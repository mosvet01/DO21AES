	...
	Если РезультатРаспознавания = "Успешно" Тогда
		...
		Попытка
			...
			Если ЭтоТекущаяВерсия Тогда
				...
				ФайлОбъект.ДополнительныеСвойства.Вставить("ЗаписьРезультатаРаспознавания", Истина);
				ФайлОбъект.Записать();
				РазблокироватьДанныеДляРедактирования(ФайлСсылка, УникальныйИдентификатор);
			...
			Если ПрограммаРаспознавания = Перечисления.ПрограммыРаспознавания.СервисРаспознавания Тогда
				...
				Если СтатусРаспознаванияТекста <> Перечисления.СтатусыРаспознаванияТекста.НужноРаспознать Тогда
					РегистрыСведений.ФайлыНаРаспознавании.ЗафиксироватьОшибкуПриРаспознавании(
						ВерсияСсылка, ОписаниеРезультатаРаспознавания);
...
// Возвращает константу ИспользоватьGhostscript.
...
Функция ПолучитьФайлВизуализации(Файл, ИдентификаторФормы, ТребуетсяЗаполнитьВизуализацию) Экспорт
	...
	ДвоичныеДанныеШтампа = СоздатьШтампПодписиСервер(ВладелецФайла, РеквФайла.ТекущаяВерсия); // возможно старая подпись - тут же на лету создаем png
	
	Если Не ЗначениеЗаполнено(ДвоичныеДанныеШтампа) Тогда
		Возврат Неопределено;
	КонецЕсли;
...
Процедура ДобавитьВизуализациюЭП(ДанныеФайла, ИдентификаторФормы) 
	...
	ДвоичныеДанныеШтампа = СоздатьШтампПодписиСервер(ДанныеФайла.Владелец, ДанныеФайла.Версия);
...
// Функция возвращает структуру, содержащую различные сведения о Файлах и их версиях.
// Параметры
...
//  * Владелец - ОпределяемыйТип.ВладелецФайлов -
...
//  * МассивСертификатовШифрования - Массив -
//  * НаЧтение - Булево -
//  * ЗанятСДругогоКомпьютера - Булево -
//  * ИмяДругогоКомпьютера - Строка -
//  * ПолноеИмяФайлаВРабочемКаталоге - Строка -
//  * ВРабочемКаталогеНаЧтение - Строка -
...
Функция ДанныеФайлов(Файлы, ИдентификаторКлиента = Неопределено) Экспорт
	
	Возврат ДанныеФайла(Файлы, Неопределено, ИдентификаторКлиента);
	
КонецФункции


// Функция - создает штамп png, если его еще нет.
//
// Параметры:
//  Документ - СправочникСсылка.ВходящиеДокументы, 
//  			СправочникСсылка.ИсходящиеДокументы,
//  			СправочникСсылка.ВнутренниеДокументы
//  ТекущаяВерсия	 - СправочникСсылка.ВерсииФайлов	 - Версия от этого файла
// 
// Возвращаемое значение:
//  ДвоичныеДанные - содержит файл png.
//
Функция СоздатьШтампПодписиСервер(Документ, ТекущаяВерсия)
	...
	Возврат Новый ДвоичныеДанные(НовыйПутьКОтметкеЭП);
