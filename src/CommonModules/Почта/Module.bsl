...
Процедура ЗаписатьОшибкуВЖурналРегистрации(Знач ТекстОшибки, Знач Данные = Неопределено) Экспорт
	...
	ЗаписьЖурналаРегистрации(
		...
		?(ОбщегоНазначения.РазделениеВключено(), УровеньЖурналаРегистрации.Предупреждение, УровеньЖурналаРегистрации.Ошибка),,
		Данные,
		ТекстОшибки);
...
Функция ИнтернетПочтаУстановитьСоединение(
	...
	ПротоколИнтернет = Неопределено) Экспорт
...
Функция УдалитьСообщенияВПочтовомЯщике(
	...
	Попытка
		...
		Если ПротоколИнтернет = ПротоколИнтернетПочты.IMAP Тогда
...
Функция ОтправитьСообщение(
	...
	Попытка
		...
		СоответствиеВозврат = Соединение.Послать(ИнтернетПочтовоеСообщение, ОбработкаТекстаИнтернетПочтовогоСообщения.НеОбрабатывать);
		
		Если СоответствиеВозврат.Количество() <> 0 Тогда
			
			СообщениеОбОшибке = "";
			
			Для Каждого КлючИЗначение Из СоответствиеВозврат Цикл
				
				СообщениеОбОшибке = СообщениеОбОшибке 
					+ СтрШаблон(НСтр("ru = '%1: %2'"), КлючИЗначение.Ключ, КлючИЗначение.Значение);
	...
	Если ПротоколВходящейПочты = "IMAP" И СохранятьКопиюВОтправленные = Истина Тогда
		...
		Попытка
			...
			Для Каждого Ящик Из Ящики Цикл
				Если    НРег(Ящик) = НРег("Sent")
					Или НРег(Ящик) = НРег("SentBox" )
					Или НРег(Ящик) = НРег("Отправленные") 
					Или НРег(Ящик) = НРег("inbox.sent")
					Или НРег(Ящик) = НРег("Sent Items")
					Тогда
