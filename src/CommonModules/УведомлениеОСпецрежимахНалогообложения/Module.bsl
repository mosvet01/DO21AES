...
Функция АдресВФорматеФИАС(Адрес, ДополнительныеПараметры = Неопределено) Экспорт
	...
	СтруктураАдресаЗаписи = РаботаСАдресами.СведенияОбАдресе(Адрес, ПараметрыСокращения);
	...
	ПолучатьНаселенныйПунктИзГорода =
		СтруктураАдресаЗаписи.Свойство("НаселенныйПункт")
		И СтруктураАдресаЗаписи.Свойство("Город")
		И НЕ ЗначениеЗаполнено(СтруктураАдресаЗаписи.НаселенныйПункт)
	...
	ВозвращаемыйАдрес.Вставить("РегионСокращение",             СтруктураАдресаЗаписи.РегионТипКраткий);
	ВозвращаемыйАдрес.Вставить("МуниципальныйРайонСокращение", СтруктураАдресаЗаписи.МуниципальныйРайонТипКраткий);
	ВозвращаемыйАдрес.Вставить("ПоселениеСокращение",          СтруктураАдресаЗаписи.ПоселениеТипКраткий);
	
	// Временно используются полные типы объектов.
	ВозвращаемыйАдрес.Вставить("НаселенныйПунктСокращение",    СтруктураАдресаЗаписи.НаселенныйПунктТипПолный);
	ВозвращаемыйАдрес.Вставить("ТерриторияСокращение",         СтруктураАдресаЗаписи.ТерриторияТипПолный);
	ВозвращаемыйАдрес.Вставить("УлицаСокращение",              СтруктураАдресаЗаписи.УлицаТипПолный);
	...
	Если ПолучатьНаселенныйПунктИзГорода Тогда
		...
		ВозвращаемыйАдрес.Вставить("НаселенныйПунктСокращение",
			?(СтруктураАдресаЗаписи.Свойство("ГородСокращение"), СтруктураАдресаЗаписи.ГородТипПолный, ""));
	КонецЕсли;
	
	// Идентификатор адресного объекта. Если есть идентификатор дома, то передается он.
	ЗначениеСвойства = "";
	...
	Если ЗначениеСвойства = "" Тогда
		...
		ИначеЕсли Сокращение = "г.о." Тогда
			ЗначениеСвойства = "2";
		ИначеЕсли Сокращение = "вн.тер." ИЛИ Сокращение = "вн.тер.г." ИЛИ Сокращение = "вн.тер. г." Тогда
			ЗначениеСвойства = "3";
	...
	Сокращения = УведомлениеОСпецрежимахНалогообложенияПовтИсп.СокращенияЭлементовФИАС();
	
	// Населенные пункты.
	Сокращение = НРег(СокрЛП(ВозвращаемыйАдрес.НаселенныйПунктСокращение));
	Сокращение = СокращениеЭлементаАдресаФИАС(Сокращение, Сокращения);
	ВозвращаемыйАдрес.Вставить("НаселенныйПунктВид", Сокращение);
	
	// Элементы планировочной структуры.
	Сокращение = НРег(СокрЛП(ВозвращаемыйАдрес.ТерриторияСокращение));
	Сокращение = СокращениеЭлементаАдресаФИАС(Сокращение, Сокращения);
	ВозвращаемыйАдрес.Вставить("ТерриторияТип", Сокращение);
	
	// Элементы улично-дорожной сети.
	Сокращение = НРег(СокрЛП(ВозвращаемыйАдрес.УлицаСокращение));
	Если Сокращение = "сквер" Тогда
		Сокращение = "с-р"; // сокращение зависит от подчиненности элемента
	Иначе
		Сокращение = СокращениеЭлементаАдресаФИАС(Сокращение, Сокращения);
	КонецЕсли;
	ВозвращаемыйАдрес.Вставить("УлицаТип", Сокращение);
	...
	Если СтруктураАдресаЗаписи.Свойство("Здание", Здание) И ТипЗнч(Здание) = Тип("Структура") Тогда
		...
		ЗданиеТип = СокращениеЭлементаАдресаФИАС(ЗданиеТип, Сокращения);
	...
	Если СтруктураАдресаЗаписи.Свойство("Корпуса", Корпуса) И ТипЗнч(Корпуса) = Тип("Массив") Тогда
		Для Каждого Корпус Из Корпуса Цикл
			Если ТипЗнч(Корпус) = Тип("Структура") Тогда
				...
				КорпусТип = СокращениеЭлементаАдресаФИАС(КорпусТип, Сокращения);
	...
	Если СтруктураАдресаЗаписи.Свойство("Помещения", Помещения) Тогда
		...
		Если Помещения.Количество() > 0 Тогда
			Если Помещения[0].Свойство("ТипПомещения", ПомещениеТип) И Помещения[0].Свойство("Номер", ПомещениеНомер) Тогда
		...
		Если Помещения.Количество() > 1 Тогда
			Если Помещения[1].Свойство("ТипПомещения", ПомещениеТип) И Помещения[1].Свойство("Номер", ПомещениеНомер) Тогда
				ПомещениеВКвартиреТип = ПомещениеТип;
	...
	ПомещениеВЗданииТип = СокращениеЭлементаАдресаФИАС(ПомещениеВЗданииТип, Сокращения);
	ПомещениеВКвартиреТип = СокращениеЭлементаАдресаФИАС(ПомещениеВКвартиреТип, Сокращения);
	...
	// Сокращения из ФИАС для обратной совместимости.
	ВозвращаемыйАдрес.Вставить("НаселенныйПунктСокращение",    СтруктураАдресаЗаписи.НаселенныйПунктТипКраткий);
	ВозвращаемыйАдрес.Вставить("ТерриторияСокращение",         СтруктураАдресаЗаписи.ТерриторияТипКраткий);
	ВозвращаемыйАдрес.Вставить("УлицаСокращение",              СтруктураАдресаЗаписи.УлицаТипКраткий);
	Если ПолучатьНаселенныйПунктИзГорода Тогда
		ВозвращаемыйАдрес.Вставить("НаселенныйПунктСокращение",
			?(СтруктураАдресаЗаписи.Свойство("ГородСокращение"), СтруктураАдресаЗаписи.ГородТипКраткий, ""));
	КонецЕсли;
...
Функция СокращениеЭлементаАдресаФИАС(ПолныйТипЭлемента, Сокращения)
	
	СокращенныйТипЭлемента = НРег(ПолныйТипЭлемента);
	
	СтрокаСокращения = Сокращения.Найти(НРег(ПолныйТипЭлемента), "Наименование");
	Если СтрокаСокращения <> Неопределено Тогда
		СокращенныйТипЭлемента = СтрокаСокращения.Сокращение;
	КонецЕсли;
	
	Возврат СокращенныйТипЭлемента;
...
Функция ИмяФайлаТребования(Требование) Экспорт
	...
	_ИмяФайлТреб = "";
	Если Требование.ВидДокумента = Перечисления.ВидыНалоговыхДокументов.ТребованиеОПредставленииДокументов Тогда 
		КонтекстЭДОСервер = ДокументооборотСКО.ПолучитьОбработкуЭДО();
