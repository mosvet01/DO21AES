	...
	Если Выборка.Количество() > 1 Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если КоличествоСтрок < МаксимальноеКоличествоСтрок Тогда
				...
				ПраваПоОбъекту = ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(Выборка.УчетнаяЗапись);
				Если ПраваПоОбъекту.Чтение Тогда
					...
					НСтр("ru = '%1: <B><FONT color=""%2"">%3</FONT></B>'"), ПредставлениеЗначения, ЦветЗначения,
						Выборка.Письмо);
...
Процедура ЗаполнитьВиджетМоиДела(Параметры)
	...
	ГрафикНагрузкиДиаграмма.ОбластьПостроения.ОриентацияМеток = 
		ОриентацияПодписейДиаграммы.Горизонтально;
