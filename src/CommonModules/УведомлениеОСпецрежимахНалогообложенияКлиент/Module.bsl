...
Процедура ВыбратьОбъект1СОтчетности_ПослеВыбора(ВыбранныйОбъект, ВходящийКонтекст) Экспорт 
	...
	Иначе
		...
		Если Раздел = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие") Тогда
			ПоказатьПредупреждение(, НСтр("ru = 'Выбрать можно только требование документов (информации)'"));
		Иначе
			ПоказатьПредупреждение(, НСтр("ru = 'Выбрать можно только ответ на требование документов (информации)'"));
		КонецЕсли;
...
Функция ВыбранаПодходящаяСсылка(ВыбранныйОбъект, ВходящийКонтекст) 
	...
	Если ЗначениеЗаполнено(ВыбранныйОбъект) Тогда 
		...
		Если ТипЗнч(ВыбранныйОбъект) = Тип("СправочникСсылка.ДокументыРеализацииПолномочийНалоговыхОрганов")
			И Раздел = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Входящие") Тогда
		
			ВидДокумента = ЭлектронныйДокументооборотСКонтролирующимиОрганамиВызовСервера.ЗначениеРеквизитаОбъекта(
				ВыбранныйОбъект, 
				"ВидДокумента");
			
			Если ВидДокумента = ПредопределенноеЗначение("Перечисление.ВидыНалоговыхДокументов.ТребованиеОПредставленииДокументов") Тогда
				ВыбранаПодходящаяСсылка = Истина;
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(ВыбранныйОбъект) = Тип("СправочникСсылка.ОписиИсходящихДокументовВНалоговыеОрганы")
			И Раздел = ПредопределенноеЗначение("Перечисление.СтраницыЖурналаОтчетность.Письма") Тогда
