...
// Формирует строковое представление периода мероприятия, приведеннего к местному времени.
//
// Параметры:
//  ДатаНачала    - Дата - Дата начала мероприятия.
//  ДатаОкончания - Дата - Дата окончания мероприятия.
// 
// Возвращаемое значение:
//  Строка - Строковое представление периода мероприятия, приведеннего к местному времени.
//
		...
		Если ТипЗнч(БизнесПроцесс.Ссылка) = Тип("БизнесПроцессСсылка.Исполнение") Тогда 
			...
			ИначеЕсли ДанныеЗаполнения.ОперацияМероприятия = "ИсполнитьПунктПротокола" Тогда
				...
				БизнесПроцесс.Наименование = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр(
					"ru = 'Исполнить п. %1 протокола мероприятия ""%2""'"),
...
Процедура НаправитьПротоколМероприятияНаИсполнение(ПараметрыИсполнения) Экспорт
	...
	Если Не ПраваПоОбъекту.Изменение Тогда
		ТекстИсключения =
			НСтр("ru = 'Недостаточно прав доступа для направления протокола мероприятия на исполнение.'");
	...
	Попытка
		...
		Если ПараметрыИсполнения.ПоВсемПунктам Тогда
			ПрерватьИсполнениеПротокола(ПараметрыИсполнения.Мероприятие,
				НСтр("ru = 'Операция ""Исполнить протокол..."". Направление протокола мероприятия на исполнение повторно.'"));
		КонецЕсли;
