...
Процедура ПереопределитьТаблицуТиповПолучателей(ТаблицаТипов, ДоступныеТипы) Экспорт
	...
	ТаблицаТипов[0].ТипПолучателей = Новый ОписаниеТипов(
		Метаданные.ОпределяемыеТипы.ПолучательРассылки.Тип);
	
	ДоступныеТипы.Очистить(); // Все типы являются дополнительными к пользователям, а не самостоятельными.
...
Процедура ПередФормированиемСпискаПолучателейРассылки(ПараметрыПолучателей, Запрос, СтандартнаяОбработка, Результат) Экспорт
	...
	Запрос.Текст =
		...
		|		ПО ПользователиВКонтейнерах.Контейнер = ТаблицаПолучателей.Получатель
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Пользователи КАК Пользователи
		|		ПО Пользователи.Ссылка = ПользователиВКонтейнерах.Пользователь
		|ГДЕ
