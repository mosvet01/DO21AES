...
Процедура ПредметыШаблонаПриАктивизацииСтроки(Форма, Объект) Экспорт
	...
	Если Форма.Элементы.Найти("ТочкиМаршрута") <> Неопределено Тогда
		...
		Если ТекущиеДанные = Неопределено Тогда
			...
			Для Каждого Элемент Из Элементы Цикл
				...
				КонецЦикла;
			КонецЦикла;
			Форма.ПодсказкаКЭтапам = НСтр("ru = 'Отображение текущего предмета:'");
			Возврат;
		Иначе
	...
	Если Форма.Элементы.Найти("ПодсказкаКЭтапам") <> Неопределено Тогда
		ТекущиеДанные = Форма.Элементы.Предметы.ТекущиеДанные;
		Если ТекущиеДанные = Неопределено Тогда
			Форма.ПодсказкаКЭтапам = НСтр("ru = 'Отображения текущего предмета:'");
			Возврат;
		Иначе
			Форма.ПодсказкаКЭтапам = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Отображение предмета ""%1"":'"),
