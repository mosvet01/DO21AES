...
#Иначе
	...
	РаботаСоСканеромКлиент.СканироватьИПоказатьДиалогПросмотра(ПараметрыВыполнения.ВладелецФайла, Неопределено, Истина);
...
Процедура ОбновитьИзФайлаНаДискеПослеУдаленияФайлаИзРабочегоКаталога(Результат, ПараметрыВыполнения) Экспорт
	...
	ПараметрыВыполнения.Вставить("РедактируетТекущийПользователь", 
		ПараметрыВыполнения.ДанныеФайла.РедактируетТекущийПользователь);
	...
	ПараметрыВыполнения.ДанныеФайла.Расширение 
		= ОбщегоНазначенияКлиентСервер.РасширениеБезТочки(ПараметрыВыполнения.РасширениеФайлаНаДиске);
	...
	ПолучитьФайлВерсииВРабочийКаталог(Обработчик, ПараметрыВыполнения.ДанныеФайла, 
		НовоеПолноеИмяФайла, ПараметрыВыполнения.ИдентификаторФормы);
...
#Если ВебКлиент Тогда
	ВернутьРезультатПослеПоказаПредупреждения(ОбработчикРезультата, 
		НСтр("ru = 'Очистка рабочего каталога возможна только в приложении (тонкий клиент).'"), Неопределено);
	Возврат;
#КонецЕсли
...
Процедура ДобавитьНаОсновеШаблонаПослеВыбораШаблона(Результат, ПараметрыВыполнения) Экспорт
	...
	ОписаниеОповещенияОЗакрытии = ОбработчикЗавершения(ПараметрыВыполнения.ОбработчикРезультата);
	СкопироватьФайл(ПараметрыВыполнения.ВладелецФайла, 
