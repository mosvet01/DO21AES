...
Процедура Проголосовать(Сообщение, ВариантОтвета) Экспорт
	...
	Если Не ЗначениеЗаполнено(Сообщение) Тогда
		ОписаниеОшибки = НСтр("ru = 'Невозможно проголосовать: не указано сообщение для голосования.'");
		ВызватьИсключение ОписаниеОшибки;
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(ВариантОтвета) Тогда
		ОписаниеОшибки = НСтр("ru = 'Невозможно проголосовать: не указан вариант голосования.'");
		ВызватьИсключение ОписаниеОшибки;
	КонецЕсли;
	...
	Если Не РеквизитыСообщения.ДобавленоГолосование Тогда
		ОписаниеОшибки = НСтр("ru = 'Невозможно проголосовать: не найдено голосование.'");
		ВызватьИсключение ОписаниеОшибки;
	КонецЕсли;
	...
	Если ТемаЗакрыта Тогда
		ОписаниеОшибки = НСтр("ru = 'Невозможно проголосовать: тема закрыта.'");
		ВызватьИсключение ОписаниеОшибки;
	КонецЕсли;
...
Функция ПолучитьРезультатГолосования(Сообщение) Экспорт
	...
	Голосование.Добавить(0, НСтр("ru = 'Всего голосов'") + ": " + ВсегоГолосов + ".");
	Возврат Голосование; 
...
// Формирует HTML представление сообщения форума.
//
// Параметры:
//  Сообщение                    - СправочникСсылка.СообщенияОбсуждений - Сообщение форума.
//  ФормироватьШапку             - Булево                               - Формировать шапку с информацией об авторе и теме.
//  УникальныйИдентификаторФормы - УникальныйИдентификатор              - Уникальный идентификатор формы.
// 
// Возвращаемое значение:
//  Строка - HTML представление сообщения форума.
//
Функция ПредставлениеСообщенияHTML(Сообщение, ФормироватьШапку, УникальныйИдентификаторФормы) Экспорт
	
	Если Не ЗначениеЗаполнено(Сообщение) Тогда
		Возврат "";
	КонецЕсли;
	
	РеквизитыСообщения = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Сообщение, "Автор, ТекстСообщения, ВладелецСообщения, ДатаСоздания");
	
	Если ФормироватьШапку Тогда
		
		ПредставлениеСообщенияHTML = 
			"<html><body>
			|<table>
			|<tr>
			|<td valign=top>
			|<table cellpadding=""0"" cellspacing=""0"">
			| <tr><td> <div style=""font-size:1pt; line-height:1px; ""> &nbsp; </div> </td></tr>  
			| <tr><td> <div style="" margin-right:6px; ""> [ФотографияАвтора] </div> </td></tr>
			| </table> 
			|</td>
			|<td width=100%>
			|<div id=""Subject"" class=v8doc-mail-header><b>[ТемаФорума]</b></div>
			|<div id=""Author"" class=v8doc-mail-header>[АвторСообщения]</div>
			|<div id=""Date"" class=v8doc-mail-information>[ДатаСообщения] &nbsp; &nbsp;</div>
			|</td>
			|</tr>
			|</table>
			|<hr>
		...
		ТекстHTMLФотографии = РаботаСФотографиями.ТекстHTMLФотографии(РеквизитыСообщения.Автор, УникальныйИдентификаторФормы);
		ПредставлениеСообщенияHTML = СтрЗаменить(ПредставлениеСообщенияHTML, "[ФотографияАвтора]", ТекстHTMLФотографии);
		
		ПредставлениеСообщенияHTML = СтрЗаменить(ПредставлениеСообщенияHTML, "[ТемаФорума]", РеквизитыСообщения.ВладелецСообщения);
		ПредставлениеСообщенияHTML = СтрЗаменить(ПредставлениеСообщенияHTML, "[АвторСообщения]", РеквизитыСообщения.Автор);
		ПредставлениеСообщенияHTML = СтрЗаменить(ПредставлениеСообщенияHTML, "[ДатаСообщения]", РеквизитыСообщения.ДатаСоздания);
	...
	ТекстСообщенияHTML = РаботаС_HTML.ПолучитьHTMLИзТекста(РеквизитыСообщения.ТекстСообщения, "UTF8", Истина);
	РаботаС_HTML.ДобавитьТегиКСсылкам(ТекстСообщенияHTML);
	ПредставлениеСообщенияHTML = СтрЗаменить(ПредставлениеСообщенияHTML, "[ТекстСообщения]", ТекстСообщенияHTML);
	
	РаботаС_HTML.ПрименитьНастройкиОтображение(
		ПредставлениеСообщенияHTML,
		РаботаСОбсуждениями.ПолучитьПерсональнуюНастройку("ШрифтОбластиЧтения"));
	
	Возврат ПредставлениеСообщенияHTML;
