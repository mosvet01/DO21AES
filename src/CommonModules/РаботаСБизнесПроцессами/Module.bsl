...
Процедура ФормаЗадачиПередЗаписьюНаСервере(Форма, Отказ, ТекущийОбъект, ПараметрыЗаписи) Экспорт
	...
	Если (ПараметрыЗаписи.Свойство("ВыполнитьЗадачу") И ПараметрыЗаписи.ВыполнитьЗадачу)
		...
		Если ПараметрыЗаписи.Свойство("ДатаИсполненияПодписанная")
			И ЗначениеЗаполнено(ПараметрыЗаписи.ДатаИсполненияПодписанная) Тогда
			
			ТекущийОбъект.ДатаИсполнения = ПараметрыЗаписи.ДатаИсполненияПодписанная;
			
		ИначеЕсли Не Форма.ДатаИсполненияУстановленаВручную Тогда
...
Процедура ОбработатьПолучениеЗадачиИзУзлаРИБ(Узел, ЭлементДанных, ПолучениеЭлемента, ОтправкаНазад) Экспорт
	...
	Если ТипЗнч(ЭлементДанных) = Тип("ЗадачаОбъект.ЗадачаИсполнителя") 
		...
		Если ЭтотУзел = УзелОбменаПроцесса Тогда // Получение задачи в мастер-узле.
			...
			ПредметыДоИзменения = ЗадачаПоСсылке.Предметы.Выгрузить();
			
			ДобавитьНовыеПредметыВЗадачу(ЗадачаПоСсылке, ЭлементДанных.Предметы);
			
			ПередатьЗаполняемыеПредметыВЗадачу(ЗадачаПоСсылке, ЭлементДанных.Предметы);
			
			Если Не ОбщегоНазначения.ДанныеСовпадают(
				ПредметыДоИзменения, ЗадачаПоСсылке.Предметы.Выгрузить()) Тогда
...
// 
// Возвращаемое значение:
//   УникальныйИдентификатор - идентификатор текущего узла (реквизит УИД в плане обмена Полный)
//
...
Процедура ДобавитьНовыеПредметыВПроцесс(Процесс, НовыеПредметы, НовыеПредметыЗадач) Экспорт
	...
	ПропущенныеПредметы = Новый Соответствие();
	
	Для Каждого СтрПредмет Из НовыеПредметы Цикл
		
		Если СтрПредмет.РольПредмета = Перечисления.РолиПредметов.Заполняемый Тогда
			ПропущенныеПредметы[СтрПредмет.ИмяПредмета] = Истина;
			Продолжить;
		КонецЕсли;
	...
	Если ДобавленыПредметы Тогда
		Для Каждого СтрПредметЗадач Из НовыеПредметыЗадач Цикл
			...
			Если ПропущенныеПредметы[СтрПредметЗадач.ИмяПредмета] = Истина Тогда
				Продолжить;
			КонецЕсли;
...
// Передает заполняемые предметы в задачу.
//
// Параметры:
//   Задача - ЗадачаОбъект.ЗадачаИсполнителя - задача объект, в которую передаются заполняемые предметы.
//   Предметы - ТаблицаЗначений, ТабличнаяЧасть - соответствует табличной части Предметы в задаче.
//
Процедура ПередатьЗаполняемыеПредметыВЗадачу(Задача, Предметы)
	...
	Для Каждого СтрПредмет Из Предметы Цикл
		...
		НайденныеСтроки = Задача.Предметы.НайтиСтроки(
			Новый Структура("ИмяПредмета", СтрПредмет.ИмяПредмета));
		Если НайденныеСтроки.Количество() = 0 Тогда
			НовСтрПредмет = Задача.Предметы.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрПредмет, СтрПредмет);
		Иначе
			ЗаполнитьЗначенияСвойств(НайденныеСтроки[0], СтрПредмет);
		КонецЕсли;
...
// Передает заполняемые предметы в процесс.
//
// Параметры:
//   Процесс - БизнесПроцессОбъект - процесс объект, в который передаются заполняемые предметы.
//   Предметы - ТаблицаЗначений, ТабличнаяЧасть - соответствует табличной части Предметы в процессе.
//   ПредметыЗадач - ТаблицаЗначений, ТабличнаяЧасть - соответствует табличной части ПредметыЗадач в процессе.
//
Процедура ПередатьЗаполняемыеПредметыВПроцесс(Процесс, Предметы, ПредметыЗадач) Экспорт
	...
	Для Каждого СтрПредмет Из Предметы Цикл
		...
		НайденныеСтроки = Процесс.Предметы.НайтиСтроки(
			Новый Структура("ИмяПредмета", СтрПредмет.ИмяПредмета));
		Если НайденныеСтроки.Количество() = 0 Тогда
			НовСтрПредмет = Процесс.Предметы.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтрПредмет, СтрПредмет);
	...
	Для Каждого СтрПредметЗадач Из ПредметыЗадач Цикл
