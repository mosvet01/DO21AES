...
Функция ЗарегистрироватьПочтуДляПервичнойЗагрузки(МобильныйКлиент, ПараметрыСинхронизации) Экспорт
	...
	Если Не ПараметрыСинхронизации.СинхронизацияПочты Тогда
		...
		РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
			НСтр("ru = 'Почта не передаётся на мобильный клиент'"),
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		НСтр("ru = 'Подготовка почты для первой загрузки'"),
		Ложь, МобильныйКлиент);
	...
	Если ПапкиПисемДляСинхронизации.Количество() = 0 Тогда
		...
		Если Не МобильныйКлиент = Неопределено Тогда
			РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
				НСтр("ru = 'Не указано ни одной папки для синхронизации'"),
				Ложь, МобильныйКлиент);
		КонецЕсли;
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		СтрШаблон(
			НСтр("ru = 'Подготовка почты для первой загрузки завершена (%1 обьектов)'"), 
			ВсегоОбъектов),
		Ложь, МобильныйКлиент);
...
Функция ЗарегистрироватьЗадачиДляПервичнойЗагрузки(МобильныйКлиент, ПараметрыСинхронизации) Экспорт 
	...
	Если Не ПараметрыСинхронизации.СинхронизацияЗадач Тогда
		...
		РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
			НСтр("ru = 'Задачи не передаются на мобильный клиент'"),
			Ложь, МобильныйКлиент);
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		НСтр("ru = 'Подготовка задач для первой загрузки'"),
		Ложь, МобильныйКлиент);
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		СтрШаблон(
			НСтр("ru = 'Подготовка задач для первой загрузки завершена (%1 обьектов)'"), 
			ВсегоОбъектов),
		Ложь, МобильныйКлиент);
...
Функция ЗарегистрироватьКалендарьДляПервичнойЗагрузки(МобильныйКлиент, ПараметрыСинхронизации) Экспорт 
	...
	Если Не ПараметрыСинхронизации.СинхронизацияКалендаря Тогда
		...
		РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
			НСтр("ru = 'Календарь не передаётся на мобильный клиент'"),
			Ложь, МобильныйКлиент);
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		НСтр("ru = 'Подготовка календаря для первой загрузки'"),
		Ложь, МобильныйКлиент);
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		СтрШаблон(
			НСтр("ru = 'Подготовка календаря для первой загрузки завершена (%1 обьектов)'"), 
			ВсегоОбъектов),
		Ложь, МобильныйКлиент);
...
Функция ЗарегистрироватьКонтрольДляПервичнойЗагрузки(МобильныйКлиент, ПараметрыСинхронизации) Экспорт
	...
	Если Не ПараметрыСинхронизации.СинхронизацияКонтроля Тогда
		...
		РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
			НСтр("ru = 'Контрольные карточки не передаются на мобильный клиент'"),
			Ложь, МобильныйКлиент);
	...
	РегистрыСведений.ПротоколРаботыСМобильнымиКлиентами.ДобавитьИнформацию(
		НСтр("ru = 'Подготовка контрольных карточек для первой загрузки'"),
		Ложь, МобильныйКлиент);
...
Процедура ВыполнитьРегистрациюДанных(МобильныйКлиент, ПараметрыСинхронизации, МассивДляРегистрации) Экспорт
	...
	ТипКлючИЗначение = Тип("КлючИЗначение");

	Для каждого Элемент Из МассивДляРегистрации Цикл

		Попытка

			Если Не ТипЗнч(Элемент) = ТипСтрока Или Не ТипЗнч(Элемент) = ТипКлючИЗначение Тогда
...
Процедура ОпределитьСписокДанныхРазделаПочтаДляРегистрации(
	...
	Для Каждого ЭлементСоставаПланаОбмена Из СоставПланаОбмена Цикл
		...
		ИначеЕсли ПолноеИмяОбъекта = "Справочник.ШаблоныТекстов" Тогда
			...
			НаборЗаписей.Отбор.Настройка.Установить(
				Перечисления.ВидыНастроекОбменаСМобильнымКлиентом.ШаблоныТекстовПисем);
