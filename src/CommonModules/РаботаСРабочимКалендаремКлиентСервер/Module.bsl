...
Функция ПолучитьТекстовоеПредставлениеПовторения(НастройкиПовторения) Экспорт
	...
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		...
		Если ЗначениеЗаполнено(НастройкиПовторения.ПовторениеПоДнямМесяца) Тогда
			...
			МассивПредставлениеДняМесяца = Новый Массив;
			МассивПовторениеПоДнямМесяца =
				ЧислоПовторениеПоДнямМесяцаВМассив(НастройкиПовторения.ПовторениеПоДнямМесяца);
			Для Каждого ЭлементПовторениеПоДнямМесяца Из МассивПовторениеПоДнямМесяца Цикл
				МассивПредставлениеДняМесяца.Добавить(
					СтрШаблон(НСтр("ru = '%1-го'"),
					ЭлементПовторениеПоДнямМесяца));
			КонецЦикла;
			
			ПредставлениеПовторения =
				ПредставлениеПовторения
				+ СимволТире
				+ СтрСоединить(МассивПредставлениеДняМесяца, ", ")
				+ " "
				+ НСтр("ru = 'числа'");
	...
	ИначеЕсли НастройкиПовторения.ЧастотаПовторения =
		...
		Если ЗначениеЗаполнено(НастройкиПовторения.ПовторениеПоДнямМесяца) Тогда
			...
			МассивПредставлениеДняГода = Новый Массив;
			МассивПовторениеПоДнямМесяца =
				ЧислоПовторениеПоДнямМесяцаВМассив(НастройкиПовторения.ПовторениеПоДнямМесяца);
			МассивПовторениеПоМесяцам =
				ЧислоПовторениеПоМесяцамВМассив(НастройкиПовторения.ПовторениеПоМесяцам);
			Для Каждого ЭлементПовторениеПоМесяцам Из МассивПовторениеПоМесяцам Цикл
				
				Для Каждого ЭлементПовторениеПоДнямМесяца Из МассивПовторениеПоДнямМесяца Цикл
					
					МесяцДень = Дата(1, ЭлементПовторениеПоМесяцам, ЭлементПовторениеПоДнямМесяца);
					МассивПредставлениеДняГода.Добавить(Формат(МесяцДень, "ДФ='d MMMM'"));
					
				КонецЦикла;
				
			КонецЦикла;
			
			ПредставлениеПовторения = ПредставлениеПовторения + СимволТире + СтрСоединить(МассивПредставлениеДняГода, ", ");
			
		Иначе
			
			МассивПредставлениеПовторения = Новый Массив;
			МассивПовторениеПоМесяцам =
				ЧислоПовторениеПоМесяцамВМассив(НастройкиПовторения.ПовторениеПоМесяцам);
			Для Каждого ЭлементПовторениеПоМесяцам Из МассивПовторениеПоМесяцам Цикл
				...
				Если ЭлементПовторениеПоМесяцам = 1 Тогда
					ПредставлениеМесяца = НСтр("ru = 'января'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 2 Тогда
					ПредставлениеМесяца = НСтр("ru = 'февраля'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 3 Тогда
					ПредставлениеМесяца = НСтр("ru = 'марта'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 4 Тогда
					ПредставлениеМесяца = НСтр("ru = 'апреля'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 5 Тогда
					ПредставлениеМесяца = НСтр("ru = 'мая'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 6 Тогда
					ПредставлениеМесяца = НСтр("ru = 'июня'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 7 Тогда
					ПредставлениеМесяца = НСтр("ru = 'июля'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 8 Тогда
					ПредставлениеМесяца = НСтр("ru = 'августа'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 9 Тогда
					ПредставлениеМесяца = НСтр("ru = 'сентября'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 10 Тогда
					ПредставлениеМесяца = НСтр("ru = 'октября'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 11 Тогда
					ПредставлениеМесяца = НСтр("ru = 'ноября'");
				ИначеЕсли ЭлементПовторениеПоМесяцам = 12 Тогда
					ПредставлениеМесяца = НСтр("ru = 'декабря'");
				Иначе
					ПредставлениеМесяца = "";
				КонецЕсли;
				
				ПредставлениеДняНеделиМесяца = СтрШаблон(НСтр("ru = '%1 %2 %3'"),
					ПредставлениеПорядкаДняНеделиМесяца,
					ПредставлениеДняНедели,
					ПредставлениеМесяца);
				
				МассивПредставлениеПовторения.Добавить(ПредставлениеДняНеделиМесяца);
				
			КонецЦикла;
			
			ПредставлениеПовторения = ПредставлениеПовторения + СимволТире + СтрСоединить(МассивПредставлениеПовторения, ", ");
