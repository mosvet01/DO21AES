...
Процедура ВыполнитьЗаменуПолейИСтрокВДокументеMSOfficeOpenXML(
	...
	// Иногда бывает, что текст поля "разрезает на куски" тэги проверки орфографии, т.е. часть текста имеет по мнению Word ошибку,
	// эти тэги их надо убирать, т.к. автозамена оставит за собой "хвост" из второго тэга, т.е. заменит только часть поля.
	// но только уже после удачной замены, т.к. обычный текст тоже может быть внутри тэгов орфограции - он должен остаться.
	ФлагНайденаПроверкаОрфографии = Ложь;
	ФлагНайденаПроверкаГрамматики = Ложь;
	...
	Пока ЧтениеXML.Прочитать() Цикл
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
			...
			Если (ФлагНайденаПроверкаОрфографии Или ФлагНайденаПроверкаГрамматики) И ФлагПолеУдачноЗаменено Тогда
				ПропуститьЭлемент = Истина;
			КонецЕсли;
			Если ЧтениеXML.Имя = "w:proofErr" И ЧтениеXML.КоличествоАтрибутов() > 0 Тогда
				Пока ЧтениеXML.ПрочитатьАтрибут() Цикл
					Если ЧтениеXML.Имя = "w:type" Тогда 
						Если ЧтениеXML.Значение = "spellStart" Тогда
							// Ставим флаг, и вместе с признаком удачной замены поля нужно пропустить все следующие тэги проверки орфограции
							// с их содержимым, до закрывающего тэга проверки spellEnd.
							ФлагНайденаПроверкаОрфографии = Истина;
							Если ФлагПолеУдачноЗаменено Тогда
								ПропуститьЭлемент = Истина;// В любом случае последний закрывающий тэг тоже надо пропустить.
							КонецЕсли;
						ИначеЕсли ЧтениеXML.Значение = "spellEnd" Тогда
							// снимаем флаг, чтобы больше не пропускать все тэги, но сам этот последний закрывающий тэг еще не пишем.
							ФлагНайденаПроверкаОрфографии = Ложь;
							Если ФлагПолеУдачноЗаменено Тогда
								ПропуститьЭлемент = Истина;// В любом случае последний закрывающий тэг тоже надо пропустить.
							КонецЕсли;
						ИначеЕсли ЧтениеXML.Значение = "gramStart" Тогда
							// с граммматикой все также, как с орфографией
							ФлагНайденаПроверкаГрамматики = Истина;
							Если ФлагПолеУдачноЗаменено Тогда
								ПропуститьЭлемент = Истина;
							КонецЕсли;
						ИначеЕсли ЧтениеXML.Значение = "gramEnd" Тогда
							ФлагНайденаПроверкаГрамматики = Ложь;
							Если ФлагПолеУдачноЗаменено Тогда
								ПропуститьЭлемент = Истина;
							КонецЕсли;
				...
				КонецЦикла;
			КонецЕсли;
			Если ПропуститьЭлемент Тогда
				// не важно сейчас нашли, или еще давно, все тэги пропускаем до конца проверки орфографии или до нового поля.
				Продолжить;
			КонецЕсли;
