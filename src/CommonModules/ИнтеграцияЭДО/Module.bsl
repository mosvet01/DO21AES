...
//
Функция АктуальныйЭлектронныйДокументПоОписанию(ОписаниеЭлектронногоДокументаОбъектаУчета) Экспорт
	...
	|ИЗ
	|	РегистрСведений.ОбъектыУчетаДокументовЭДО КАК ОбъектыУчетаДокументовЭДО
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ИмяТаблицыЭлектронныхДокументов КАК ЭлектронныйДокументЭДО
	|		ПО ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент = ЭлектронныйДокументЭДО.Ссылка
	|			И (ОбъектыУчетаДокументовЭДО.ОбъектУчета = &ОбъектУчета)
	|			И (ЭлектронныйДокументЭДО.Организация = &Организация)
	|			И (ЭлектронныйДокументЭДО.Контрагент = &Контрагент)
	|			И (ЭлектронныйДокументЭДО.ДоговорКонтрагента = &Договор)
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ВидыДокументовЭДО КАК ВидыДокументовЭДО
	|		ПО (ЭлектронныйДокументЭДО.ВидДокумента = ВидыДокументовЭДО.Ссылка)
	...
	ЭтоВходящийЭДО = (ОписаниеЭлектронногоДокументаОбъектаУчета.Направление =
		Перечисления.НаправленияЭДО.Входящий);
	ИмяТаблицыЭлектронныхДокументов = ЭлектронныеДокументыЭДО.ИмяТаблицыДокументаЭДО(ЭтоВходящийЭДО);
	ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ИмяТаблицыЭлектронныхДокументов", ИмяТаблицыЭлектронныхДокументов);
	
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.Параметры.Вставить("ОбъектУчета", ОписаниеЭлектронногоДокументаОбъектаУчета.ОбъектУчета);
	Запрос.Параметры.Вставить("Организация", ОписаниеЭлектронногоДокументаОбъектаУчета.Организация);
	Запрос.Параметры.Вставить("Контрагент", ОписаниеЭлектронногоДокументаОбъектаУчета.Контрагент);
	Запрос.Параметры.Вставить("Договор", ОписаниеЭлектронногоДокументаОбъектаУчета.Договор);
	Запрос.Параметры.Вставить("ТипДокумента", ОписаниеЭлектронногоДокументаОбъектаУчета.ТипДокумента);
	Запрос.Параметры.Вставить("ПрикладнойТипДокумента", ОписаниеЭлектронногоДокументаОбъектаУчета.ПрикладнойТипДокумента);
	...
	Если РезультатЗапроса.Пустой() Тогда
		МенеджерЭлектронныхДокументов = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ИмяТаблицыЭлектронныхДокументов);
		Возврат МенеджерЭлектронныхДокументов.ПустаяСсылка();
	КонецЕсли;
...
Функция ОписаниеДанныхОбъектаУчета(ОбъектУчета, Параметры) Экспорт
	...
	ДанныеДО = ОбменСКонтрагентамиДО.ДанныеДокументаЭДОПоВерсииФайла(
		ОбъектУчета,
		Параметры);
	
	Если ДанныеДО <> Неопределено Тогда
		Данные = Новый Структура;
		Данные.Вставить("ДеревоЭД", ДанныеДО.ДеревоЭД);
		Данные.Вставить("ДанныеДО", ДанныеДО);
		
		Результат.Данные = Данные;
		Результат.Основания.Добавить(ОбъектУчета);
		Результат.СвязанныеОбъектыУчета = ДанныеДО.СвязанныеОбъектыУчета;
...
//
Функция ТекстЗапросаОбъектовУчетаЭлектронныхДокументов(ПолучатьДокументыДО = Ложь) Экспорт
	
	Если ПолучатьДокументыДО Тогда
		
		ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ОбъектыУчетаДокументовЭДО.ЭлектронныйДокумент КАК ЭлектронныйДокумент,
		|	ВЫБОР
		|		КОГДА ОбъектыУчетаДокументовЭДО.ОбъектУчета ССЫЛКА Справочник.ВерсииФайлов
		|			ТОГДА ВЫРАЗИТЬ(ОбъектыУчетаДокументовЭДО.ОбъектУчета КАК Справочник.ВерсииФайлов).Владелец.ВладелецФайла
		|		ИНАЧЕ ОбъектыУчетаДокументовЭДО.ОбъектУчета
...
Функция ЗапросОрганизацийПолучателей(ИмяВременнойТаблицы, ИмяПараметраСсылка, Отбор = Неопределено) Экспорт
	...
	ИмяРеквизитаИННОрганизации =
		ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("ИННОрганизации");
	ИмяРеквизитаКППОрганизации =
		ОбщегоНазначенияБЭДПовтИсп.ИмяНаличиеОбъектаРеквизитаВПрикладномРешении("КППОрганизации");
...
Процедура ДополнитьДанныеДокумента(Данные, Знач ДополнительныеДанные, Знач Замещать = Ложь) Экспорт
...
Процедура ЗаполнитьСвязанныеОбъектыУчетаCML(СвязанныеОбъектыУчета, Данные) Экспорт
...
Процедура УПД2019_ИнформацияПродавца_ЗаполнитьСвязанныеОбъектыУчета(СвязанныеОбъектыУчета, Данные) Экспорт
...
Процедура ПередачаТоваров_ИнформацияПродавца_ЗаполнитьСвязанныеОбъектыУчета(СвязанныеОбъектыУчета, Данные) Экспорт
...
Процедура ПередачаУслуг_ИнформацияПродавца_ЗаполнитьСвязанныеОбъектыУчета(СвязанныеОбъектыУчета, Данные) Экспорт
...
Процедура УКД_ИнформацияПродавца_ЗаполнитьСвязанныеОбъектыУчета(СвязанныеОбъектыУчета, Данные) Экспорт
...
Процедура УКД2020_ИнформацияПродавца_ЗаполнитьСвязанныеОбъектыУчета(СвязанныеОбъектыУчета, Данные) Экспорт
	ДокументыОснованияСчетаФактуры = ДеревоЭлектронногоДокументаБЭД.ЗначениеРеквизитаВДереве(Данные, "ДокументыОснованияСчетаФактуры");
