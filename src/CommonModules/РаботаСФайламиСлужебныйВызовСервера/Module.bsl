...
//
Функция ЗанятьФайл(ДанныеФайла, СтрокаОшибки = "", УникальныйИдентификатор = Неопределено, 
...
Функция ДанныеФайла(ФайлИлиВерсияСсылка) Экспорт
	...
	ИначеЕсли ТипЗнч(ВерсияСсылка) = Тип("СправочникСсылка.ВерсииФайлов") Тогда
	...
	ИначеЕсли ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ВерсияСсылка)) Тогда 
		
		ИмяСправочникаФайлов = ВерсияСсылка.Метаданные().Имя;
		...
		ТекстЗапроса = 
			...
			|	"""" КАК Код,
			|	Файлы.Редактирует,
			|	Неопределено КАК ТекущаяВерсия,
			|	Файлы.ВладелецФайла,
			...
			|	Файлы.Автор,
			|	ЗНАЧЕНИЕ(Перечисление.СтатусыИзвлеченияТекстаФайлов.ПустаяСсылка) КАК СтатусИзвлеченияТекста,
			|	Файлы.Зашифрован,
			|	Файлы.ДатаЗаема
			|ИЗ
		...
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "%ИмяСправочника%", ИмяСправочникаФайлов);
