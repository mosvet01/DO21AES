...
Процедура ПроверитьПодписьЗавершение(Результат, Контекст) Экспорт
	...
	Если ТипЗнч(Результат) = Тип("Строка") Тогда
		ОбработатьОшибкуКриптографии(Контекст, Контекст.ВидОперации, Результат);
		Контекст.РезультатПроверки.ОписаниеОшибки = Результат;
		Контекст.РезультатПроверки.СвойстваПодписи.Комментарий = Результат;
	КонецЕсли;
	...
	Если ТипЗнч(Результат) = Тип("Булево") Тогда
		...
		Контекст.РезультатПроверки.СвойстваПодписи.ПодписьВерна = Результат;
		Контекст.РезультатПроверки.СвойстваПодписи.СертификатДействителен = Результат;
		Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиПодписи = "";
		Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиСертификата = "";
	...
	ИначеЕсли ТипЗнч(Результат) = Тип("Структура") Тогда
		...
		Если Результат.Свойство("ТекстОшибкиПроверкиПодписи") Тогда
			...
			Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиПодписи
				= Результат.ТекстОшибкиПроверкиПодписи;
		Иначе
			Контекст.РезультатПроверки.СвойстваПодписи.ПодписьВерна = Истина;
			Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиПодписи = "";
		КонецЕсли;
		
		Если Результат.Свойство("ТекстОшибкиПроверкиСертификата") Тогда
			Контекст.РезультатПроверки.СвойстваПодписи.СертификатДействителен = Ложь;
			Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиСертификата
				= Результат.ТекстОшибкиПроверкиСертификата;
		Иначе
			Контекст.РезультатПроверки.СвойстваПодписи.СертификатДействителен = Истина;
			Контекст.РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиСертификата = "";
		КонецЕсли;
...
Процедура ОбработатьОшибкуКриптографии(Контекст, ВидОперации, ТекстОшибки, Сертификат = Неопределено)
	
	ДополнительныеПараметрыОшибки = Новый Структура;
	Если Сертификат <> Неопределено Тогда
		ДополнительныеПараметрыОшибки.Вставить("Сертификат", Сертификат);
	КонецЕсли;
	ВидОшибкиКриптография = КриптографияБЭДКлиентСервер.ВидОшибкиКриптография();
	Ошибка = ОбработкаНеисправностейБЭДКлиент.НоваяОшибка(ВидОперации, ВидОшибкиКриптография,
		ТекстОшибки, ТекстОшибки, ДополнительныеПараметрыОшибки);
