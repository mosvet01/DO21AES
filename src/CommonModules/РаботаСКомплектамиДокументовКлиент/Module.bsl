...
Процедура УстановитьДоступностьКомандРаботыСЭлементамиКомплекта(Форма)
	...
	Если ТекущиеДанные = Неопределено Тогда
		...
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.ЭлементКомплектаОтправить, Ложь);
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.КонтекстноеМенюЭлементКомплектаОтправить, Ложь);
	...
	Иначе
		...
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.ЭлементКомплектаОтправить, ЭтоДокументИлиФайл);
		УстановитьДоступностьКоманды(ДоступныеПоля, Элементы.КонтекстноеМенюЭлементКомплектаОтправить, ЭтоДокументИлиФайл);
...
Процедура СоставКомплектаПодобрать(Форма, Тип = Неопределено) Экспорт
	...
	Если Тип = Неопределено Тогда
		...
		Если Найти(Форма.ПраваДоступа, "ВнутренниеДокументы.Чтение") > 0 Тогда
			Меню.Добавить("ВнутренниеДокументы", НСтр("ru = 'Подобрать внутренние документы'"));
		КонецЕсли;
		Если Найти(Форма.ПраваДоступа, "ВходящиеДокументы.Чтение") > 0 Тогда
			Меню.Добавить("ВходящиеДокументы", НСтр("ru = 'Подобрать входящие документы'"));
		КонецЕсли;
		Если Найти(Форма.ПраваДоступа, "ИсходящиеДокументы.Чтение") > 0 Тогда
			Меню.Добавить("ИсходящиеДокументы", НСтр("ru = 'Подобрать исходящие документы'"));
		КонецЕсли;
		Если Найти(Форма.ПраваДоступа, "Файлы.Чтение") > 0 Тогда
			Меню.Добавить("Файлы", НСтр("ru = 'Подобрать файлы'"));
		КонецЕсли;
		Если Меню.Количество() = 0 Тогда
			Возврат;
		КонецЕсли;
		
		Меню.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru = 'Выберите действие'"));
	Иначе 
...
Процедура СоставКомплектаПодобратьПродолжение(Результат, Параметры) Экспорт
	...
	Если ВРег(Тип) = ВРег("ВнутренниеДокументы") Тогда
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор внутренних документов в комплект'"));
	...
	ИначеЕсли ВРег(Тип) = ВРег("ВходящиеДокументы") Тогда
		...
		ПараметрыОткрытия.Вставить("ПоказыватьКомандуСоздать", Истина);
		ПараметрыОткрытия.Вставить("ЗакрыватьПриВыборе", Ложь);
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор входящих документов в комплект'"));
	...
	ИначеЕсли ВРег(Тип) = ВРег("ИсходящиеДокументы") Тогда
		...
		ПараметрыОткрытия.Вставить("ПоказыватьКомандуСоздать", Истина);
		ПараметрыОткрытия.Вставить("ЗакрыватьПриВыборе", Ложь);
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор исходящих документов в комплект'"));
	...
	ИначеЕсли ВРег(Тип) = ВРег("Файлы") Тогда
		...
		ПараметрыОткрытия.Вставить("РежимВыбора", Истина);
		ПараметрыОткрытия.Вставить("ЗакрыватьПриВыборе", Ложь);
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор файлов в комплект'"));
...
Процедура ЭлементКомплектаУдалить(Форма, Отказ = Неопределено) Экспорт
	...
	Если Форма.Элементы.СоставКомплекта.ТекущиеДанные.ПолучитьРодителя() <> Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Удалять можно только элементы текущего комплекта.'"));
		Возврат;
	КонецЕсли;
...
// Отправляет выделенные строки состава комплекта
//
// Параметры:
//   Форма - ФормаКлиентскогоПриложения - форма комплекта документа.
//
Процедура ОтправитьОбъектыКомплекта(Форма) Экспорт
	...
	ПомощникОтправитьКлиент.ОтправитьОбъекты(Предметы, Форма);
...
Процедура ДобавитьВКомплект(ВыделенныеСтроки) Экспорт
	...
	ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Выберите комплект документов'"));
...
Процедура СоздатьКомплект(ВыделенныеСтроки) Экспорт
	...
	Если СписокВидовКомплектов.Количество() = 0 Тогда
		Текст = НСтр("ru = 'Не найдено ни одного вида документов с признаком ""Является комплектом документов""'");
	...
	Иначе	
		СписокВидовКомплектов.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru = 'Выберите вид комплекта документов'"));
	КонецЕсли;
...
Функция ДобавитьЭлементКомплекта(Форма, КомплектИнфо, КомплектСсылка, Ссылка, Комментарий, СообщениеОбОшибке)
	...
	Если КомплектИнфо.Элементы.Найти(Ссылка) <> Неопределено Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Документ ""%1"" уже находится в комплекте.'"),
			Ссылка);
		Возврат Ложь;
	КонецЕсли;
	Если Ссылка = КомплектСсылка Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Документ ""%1"" не добавлен. Нельзя добавлять комплект сам в себя.'"),
			Ссылка);
		Возврат Ложь;
	КонецЕсли;
	Если Не ЭтоДокументИлиФайл(Ссылка) Тогда
		СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'Объект ""%1"" не добавлен. В комплект можно добавлять только документы или файлы.'"),
			Ссылка);
		Возврат Ложь;
	КонецЕсли;
...
Функция ЗапомнитьСостояниеРазвернут(Элемент, Контекст) Экспорт
...
Функция УстановитьСостояниеРазвернут(Элемент, Контекст) Экспорт
