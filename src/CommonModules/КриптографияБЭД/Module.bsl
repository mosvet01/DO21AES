...
Функция ПроверитьПодпись(МенеджерКриптографии, ИсходныеДанные, Подпись, КонтекстДиагностики) Экспорт
	...
	ТекстОшибки = "";
	РезультатПроверкиПодписи = ЭлектроннаяПодпись.ПроверитьПодпись(
	...
	РезультатПроверки.СвойстваПодписи.СертификатДействителен = РезультатПроверкиПодписи.СертификатДействителен;
	РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиПодписи = РезультатПроверкиПодписи.ТекстОшибкиПроверкиПодписи;
	РезультатПроверки.СвойстваПодписи.ТекстОшибкиПроверкиСертификата = РезультатПроверкиПодписи.ТекстОшибкиПроверкиСертификата;
...
Функция УстановленныеПодписиОбъектов(ПодписанныеОбъекты) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ЭлектронныеПодписи.Объект В (&ПодписанныеОбъекты)
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПодписанныйОбъект";
...
Процедура ОбновитьПодпись(Объект, Знач СвойстваПодписи) Экспорт
	...
	Попытка
		...
		Для Каждого ПодписьОбъекта Из ПодписиОбъекта Цикл
			...
			Если ПодписьОбъекта.УникальныйИдентификатор = СвойстваПодписи.УникальныйИдентификатор 
				...
				Для Каждого КлючИЗначение Из СвойстваПодписи Цикл
					Если КлючИЗначение.Ключ = "Подпись"
						...
						Или КлючИЗначение.Ключ = "УникальныйИдентификатор"
						Или КлючИЗначение.Ключ = "СтатусПроверкиЭП" Тогда
						Продолжить;
					КонецЕсли;
