...
Процедура УдалитьШаблонДействияЭтапаПроцесса(ШаблонБизнесПроцесса) Экспорт
	...
	Если ЗначениеЗаполнено(ВладелецШаблона)
		И ОбщегоНазначения.СсылкаСуществует(ВладелецШаблона)
		И Не ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(ШаблонБизнесПроцесса).Удаление Тогда
...
Функция ПечатьКарточки(МассивОбъектов, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектПечати.Схема) Тогда
		...
		ПараметрыДействий.Сортировать("ПорядокСортировки Возр");
...
// Возвращает имя действия исполнения в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияИсполненияВКомплексномПроцессе() Экспорт
	
	Возврат "Исполнение";
	
КонецФункции

// Возвращает имя вложенного процесса в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяВложенногоПроцессаВКомплексномПроцессе() Экспорт
	
	Возврат "ВложенныйПроцесс";
	
КонецФункции

// Возвращает имя действия ознакомления в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияОзнакомленияВКомплексномПроцессе() Экспорт
	
	Возврат "Ознакомление";
	
КонецФункции

// Возвращает имя действия приглашения в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияПриглашенияВКомплексномПроцессе() Экспорт
	
	Возврат "Приглашение";
	
КонецФункции

// Возвращает имя действия рассмотрения в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияРассмотренияВКомплексномПроцессе() Экспорт
	
	Возврат "Рассмотрение";
	
КонецФункции

// Возвращает имя действия регистрации в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияРегистрацииВКомплексномПроцессе() Экспорт
	
	Возврат "Регистрация";
	
КонецФункции

// Возвращает имя действия согласования в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияСогласованияВКомплексномПроцессе() Экспорт
	
	Возврат "Согласование";
	
КонецФункции

// Возвращает имя действия утверждения в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияУтвержденияВКомплексномПроцессе() Экспорт
	
	Возврат "Утверждение";
	
КонецФункции

// Возвращает имя действия подписания в комплексном процессе.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияПодписанияВКомплексномПроцессе() Экспорт
	
	Возврат "Подписание";
	
КонецФункции

// Возвращает для шаблона или процесса имя действия в комплексном процессе.
//
// Параметры:
//  Объект - ОпределяемыйТип.ШаблонДействияКомплексногоПроцесса,
//           ОпределяемыйТип.ДействиеКомплексногоПроцесса - процесс или шаблон процесса.
//
// Возвращаемое значение:
//  Строка
//
Функция ИмяДействияВКомплексномПроцессеПоОбъекту(Объект) Экспорт
	
	ТипОбъекта = ТипЗнч(Объект);
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Исполнение")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныИсполнения") Тогда
		
		Возврат ИмяДействияИсполненияВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.КомплексныйПроцесс")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныКомплексныхБизнесПроцессов") Тогда
		
		Возврат ИмяВложенногоПроцессаВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Ознакомление")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныОзнакомления") Тогда
		
		Возврат ИмяДействияОзнакомленияВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Приглашение")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныПриглашения") Тогда
		
		Возврат ИмяДействияПриглашенияВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Рассмотрение")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныРассмотрения") Тогда
		
		Возврат ИмяДействияРассмотренияВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Регистрация")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныРегистрации") Тогда
		
		Возврат ИмяДействияРегистрацииВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Согласование")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныСогласования") Тогда
		
		Возврат ИмяДействияСогласованияВКомплексномПроцессе();
	КонецЕсли;
	
	Если ТипОбъекта = Тип("БизнесПроцессСсылка.Утверждение")
		Или ТипОбъекта = Тип("СправочникСсылка.ШаблоныУтверждения") Тогда
		
		ВидПроцесса = ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(
			Объект, "ВидПроцесса");
		
		Если ВидПроцесса = Перечисления.ВидыБизнесПроцессаУтверждение.Подписание Тогда
			Возврат ИмяДействияПодписанияВКомплексномПроцессе();
		Иначе
			Возврат ИмяДействияУтвержденияВКомплексномПроцессе();
		КонецЕсли;
...
// Заполняет схему шаблона по умаолчанию в форме.
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - картчока шаблона процесса.
//
Процедура ЗаполнитьСхемуШаблонаПоУмолчаниюВФорме(Форма) Экспорт
	
	Если Форма.Объект.Схема <> 
		Справочники.СхемыКомплексныхПроцессов.СхемаШаблонаПроцессаПоУмолчанию Тогда
		
		ВызватьИсключение НСтр("ru = 'Процедура ЗаполнитьСхемуШаблонаПоУмолчаниюВФорме переназначена,
			|для применения к карточке шаблона комплексного процесса по умолчанию.'");
	КонецЕсли;
	
	СхемаОбъект = Форма.Объект.Схема.ПолучитьОбъект();
	СхемаОбъект.ЗаполнитьСхемуШаблонаПроцессаПоУмолчанию();
	...
	ПоказатьТочныеСроки = Ложь;
	ПоказатьОтносительныеСроки = Истина;
	ПроцессыСхемыДляПрерывания = Новый Соответствие;
...
// Возвращает возможные результаты действия в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияВСхеме() Экспорт
	
	ПеречислениеРезультатов = Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов;
	
	РезультатыВыполненияДействия = Новый СписокЗначений;
	РезультатыВыполненияДействия.Добавить(ПеречислениеРезультатов.ЗавершеноСЛюбымРезультатом,
		Строка(ПеречислениеРезультатов.ЗавершеноСЛюбымРезультатом));
	РезультатыВыполненияДействия.Добавить(ПеречислениеРезультатов.Отрицательно,
		Строка(ПеречислениеРезультатов.Отрицательно));
	РезультатыВыполненияДействия.Добавить(ПеречислениеРезультатов.Положительно,
		Строка(ПеречислениеРезультатов.Положительно));
	РезультатыВыполненияДействия.Добавить(ПеречислениеРезультатов.ПоложительноБезЗамечаний,
		Строка(ПеречислениеРезультатов.ПоложительноБезЗамечаний));
	РезультатыВыполненияДействия.Добавить(ПеречислениеРезультатов.ПоложительноСЗамечаниями,
		Строка(ПеречислениеРезультатов.ПоложительноСЗамечаниями));
	
	Возврат РезультатыВыполненияДействия;
	
КонецФункции

// Возвращает возможные результаты выполнения действия в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияИсполненияВСхеме() Экспорт
	
	ПеречислениеРезультатов = Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов;
	
	РезультатыВыполненияДействия = Новый СписокЗначений;
	РезультатыВыполненияДействия.Добавить(
		ПеречислениеРезультатов.ЗавершеноСЛюбымРезультатом,
		НСтр("ru = 'Завершено успешно'"));
	
	Возврат РезультатыВыполненияДействия;
	
КонецФункции

// Возвращает возможные результаты выполнения вложенного процесса в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияВложенногоПроцессаВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения ознакомления в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияОзнакомленияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения приглашения в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияПриглашенияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения рассмотрения в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияРассмотренияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения регистрации в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияРегистрацияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения согласования в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияСогласованияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения утверждения в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияУтвержденияВСхеме() Экспорт
...
// Возвращает возможные результаты выполнения подписания в схеме.
//
// Возвращаемое значение:
//  СписокЗначений - список возможных вариантов выполнения действия.
//   * Перечисления.РезультатыВыполненияДействийКомплексныхПроцессов
//
Функция РезультатыВыполненияДействияПодписанияВСхеме() Экспорт
...
Функция РезультатыВыполненияДействий() Экспорт
	...
	// Все результаты
	РезультатыВыполнения[""] = РезультатыВыполненияДействияВСхеме();
	
	РезультатыВыполнения[ИмяДействияИсполненияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияИсполненияВСхеме();
	
	РезультатыВыполнения[ИмяВложенногоПроцессаВКомплексномПроцессе()] = 
		РезультатыВыполненияВложенногоПроцессаВСхеме();
	
	РезультатыВыполнения[ИмяДействияОзнакомленияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияОзнакомленияВСхеме();
	
	РезультатыВыполнения[ИмяДействияПриглашенияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияПриглашенияВСхеме();
	
	РезультатыВыполнения[ИмяДействияРассмотренияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияРассмотренияВСхеме();
	
	РезультатыВыполнения[ИмяДействияРегистрацииВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияРегистрацияВСхеме();
	
	РезультатыВыполнения[ИмяДействияСогласованияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияСогласованияВСхеме();
	
	РезультатыВыполнения[ИмяДействияУтвержденияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияУтвержденияВСхеме();
	
	РезультатыВыполнения[ИмяДействияПодписанияВКомплексномПроцессе()] = 
		РезультатыВыполненияДействияПодписанияВСхеме();
...
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - карточка комплексного процесса/шаблона/схемы.
//
...
Процедура ФоноваяМаршрутизацияКомплексныхПроцессов() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ФоноваяМаршрутизацияКомплексныхПроцессов, Отказ);
...
Функция ДоступностьПоШаблону(Форма) 
	...
	ТипыКолонки.Добавить(Тип("СправочникСсылка.ВидыВнутреннихДокументов"));
	ТипыКолонки.Добавить(Тип("СправочникСсылка.ВидыВходящихДокументов"));
	ТипыКолонки.Добавить(Тип("СправочникСсылка.ВидыИсходящихДокументов"));
	...
	Для Каждого Предмет Из Предметы Цикл
		...
		Иначе
			СтрокаДанных = ДанныеПредметов.Добавить();
			СтрокаДанных.КомуНазначен = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(Предмет));
