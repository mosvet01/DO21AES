...
Процедура ДокументооборотПраваДоступаПриЗаписиОбъектаДоступа(Источник, Отказ) Экспорт
	...
	РегистрыСведений.РабочиеГруппыДляПерехода.ЗаписатьРГПриЗаписиОбъектаДоступа(
		Источник.Ссылка);
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("СправочникОбъект.СтруктураПредприятия") Тогда	
		...
		Если Не Источник.ЭтоНовый() Тогда
			...
			РеквизитыСсылки = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(
				Источник.Ссылка, "Родитель, Руководитель, ПометкаУдаления");
			...
			Если Источник.ПометкаУдаления <> РеквизитыСсылки.ПометкаУдаления Тогда
				Источник.ДополнительныеСвойства.Вставить("ЗначениеИзменено", Истина);
...
Процедура ДокументооборотПриЗаписиПравообразующихОбъектов(Источник, Отказ) Экспорт
	...
	ИначеЕсли ТипИсточника = Тип("СправочникОбъект.СтруктураПредприятия") Тогда
		...
		ИначеЕсли Источник.ДополнительныеСвойства.Свойство("ИзмененРуководитель") Тогда
...
Процедура ОбновитьПраваПриИзмененииРолейПользователей(РолиПользователей) Экспорт
	...
	Для Каждого КлючИЗначение Из РолиПользователей Цикл
		...
		РолиПослеИзменений = ОбщегоНазначения.СкопироватьРекурсивно(НеИзмененныеРоли);
		ОбщегоНазначенияКлиентСервер.ДополнитьМассив(РолиПослеИзменений, ДобавленныеРоли);
		Для Каждого УдаленнаяРоль Из УдаленныеРоли Цикл
			ОбщегоНазначенияКлиентСервер.УдалитьЗначениеИзМассива(РолиПослеИзменений, УдаленнаяРоль);
		КонецЦикла;
		ИзмененныеОбъекты = ОбъектыСОтличиямиВПравахРолей(НеИзмененныеРоли, РолиПослеИзменений);
