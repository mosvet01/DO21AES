...
Процедура ВыполнитьУстановкуПометкиУдаления(Форма, ИмяСписка = "Список") Экспорт
	...
	Иначе
		Если ТекущаяПометкаУдаления Тогда
			ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Снять с выделенных элементов (%1 шт) пометку на удаление?'"),
	...
	Если ТекущаяПометкаУдаления Тогда
		Если КоличествоДокументов = 1 Тогда 
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Пометка на удаление снята'"),
...
// Вызывается при начале выбора стороны в списке и открывает адресную книгу.
//
Процедура НачалоВыбораСтороны(Элемент, ВыбранноеЗначение, ДопустимыКонтрагенты, ДопустимыПользователи) Экспорт
	ПараметрыФормы = Новый Структура;
	ПараметрыФормы.Вставить("РежимРаботыФормы", 1);
	ПараметрыФормы.Вставить("УпрощенныйИнтерфейс", Истина);
	ПараметрыФормы.Вставить("ОтображатьКонтрагентов", ДопустимыКонтрагенты);
	ПараметрыФормы.Вставить("ВыбиратьКонтрагентов", ДопустимыКонтрагенты);
	ПараметрыФормы.Вставить("ОтображатьСотрудников", ДопустимыПользователи);
	ПараметрыФормы.Вставить("ВыбиратьСотрудников", ДопустимыПользователи);
...
Процедура НастроитьСписокСогласноВидуДокумента(Форма) Экспорт
	...
	Элементы = Форма.Элементы;
	ТекущиеДанные = Элементы.ВидыДокументов.ТекущиеДанные;
	Если Не ТекущиеДанные.Свойство("НаименованиеСтороны1") Тогда
		Возврат;
	КонецЕсли;
	...
	ЕстьПредопределенныеСтороны = ЗначениеЗаполнено(ТекущиеДанные.НаименованиеСтороны1);
	Если ЕстьПредопределенныеСтороны Тогда
		
		Для Номер = 1 По ПределСторон Цикл
			// Установим видимость.
			Элементы["ОтборСторона" + Номер].Видимость = Истина;
			Элементы["ОтборСторона" + Номер].Заголовок = ТекущиеДанные["НаименованиеСтороны" + Номер];
			Элементы["ОтборСторона" + Номер].ПодсказкаВвода = ТекущиеДанные["НаименованиеСтороны" + Номер];
			Элементы["Сторона" + Номер].Видимость = Истина;
			Элементы["Сторона" + Номер].Заголовок = ТекущиеДанные["НаименованиеСтороны" + Номер];
			// Приведем типы.
			Для Каждого ТипСтороны Из ТипыСторон Цикл
				Форма["Сторона" + Номер + ТипСтороны.Значение] = ТекущиеДанные["Сторона" + Номер + ТипСтороны.Значение];
				Если ТипЗнч(Форма["Сторона" + Номер]) = ТипСтороны.Ключ
					И Не ТекущиеДанные["Сторона" + Номер + ТипСтороны.Значение] Тогда
			...
			Если Форма["Сторона" + Номер] = Неопределено Тогда
				Для Каждого ТипСтороны Из ТипыСторон Цикл
					Если ТекущиеДанные["Сторона" + Номер + ТипСтороны.Значение] Тогда
		...
		// Скроем организацию и сбросим отбор.
		Элементы.ОтборОрганизация.Видимость = Ложь;
		ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(Форма.Список.Отбор,
			"Организация");
		Элементы.Организация.Видимость = Ложь;
		
		// Скроем контрагента и сбросим отбор.
