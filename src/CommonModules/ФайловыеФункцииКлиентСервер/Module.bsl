...
#Если Клиент Тогда
	...
	Описание = Новый ОписаниеПередаваемогоФайла(ИмяВременногоФайла, "");
...
// удаляет в конце "\"
Процедура УдалитьСлешЕслиНужно(НовыйПуть) Экспорт
	
	Если Прав(НовыйПуть, 1) = "\" ИЛИ Прав(НовыйПуть,1) = "/" Тогда
		НовыйПуть = Лев(НовыйПуть, СтрДлина(НовыйПуть) - 1);
	КонецЕсли;
...
Функция РасширениеБезТочки(СтрРасширение) Экспорт
	...
	Расширение = СокрЛП(СтрРасширение);
...
Функция ПолучитьСоответствиеФайловИПодписей(ИменаФайлов, РасширениеДляФайловПодписи) Экспорт
	...
	Для Каждого ИмяФайла Из ИменаФайлов Цикл
		...
		РасширениеФайлаБезТочки = ПолучитьРасширениеИмениФайла(ИмяФайла);
		Если НРег(РасширениеФайлаБезТочки) = НРег(РасширениеДляФайловПодписи) Тогда
...
// Получить строку с представлением размера файлов
Функция ПолучитьСтрокуСРазмеромФайлаДляКарточкиПапки(Знач РазмерБайт) Экспорт
	
	СтрокаРазмера = "";
	
	Если РазмерБайт >= 1024*1024*1024 Тогда // больше 1 Гб
		СтрокаРазмера = Формат(РазмерБайт / (1024*1024*1024), "ЧДЦ=0") + НСтр("ru=' Гб'");
	ИначеЕсли РазмерБайт < 1024*1024*1024  И РазмерБайт >= 1024*1024 Тогда // от 1 Мб до 1 Гб
		СтрокаРазмера = Формат(РазмерБайт / (1024*1024), "ЧДЦ=0; ЧН=0") + НСтр("ru=' Мб'");
	ИначеЕсли РазмерБайт < 1024*1024  И РазмерБайт >= 1024 Тогда // от 1 кб до 1 Мб
		СтрокаРазмера = Формат(РазмерБайт / 1024, "ЧДЦ=0; ЧН=0") + НСтр("ru=' Кб'");
	Иначе // размер меньше 1 кб
		СтрокаРазмера = Формат(РазмерБайт, "ЧДЦ=0; ЧН=0") + НСтр("ru=' байт'");
	КонецЕсли;
