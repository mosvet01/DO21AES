...
Процедура ПодготовитьГруппуПодтвержденияЭлементы(КонтекстФормы, ТекущиеЭлементыФормы)
	...
	// Получим значение если это мобильный клиент, для адаптации.
	ЭтоМобильныйКлиент = ОбщегоНазначения.ЭтоМобильныйКлиент(); // Для обновления БСП
	...
	ЕстьРазделители = СписокГрупп.ЕстьРазделители И ИспользованиеВозможно И Не ЭтоМобильныйКлиент;
	...
	Если ЭтоМобильныйКлиент Тогда
		ВременнаяГруппа.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		ВременнаяГруппа.ОтображатьЗаголовок = Ложь;
		ВременнаяГруппа.Отображение = ОтображениеОбычнойГруппы.Нет;
	КонецЕсли;
	...
	Если ЭтоМобильныйКлиент Тогда
		ЭлементФормы.Заголовок = НСтр("ru = 'Запомнить'");
	КонецЕсли;
