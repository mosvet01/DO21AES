...
// Возвращает виды внутренних электронных документов по идентификаторам.
// 
// Параметры:
//  ИдентификаторОбъектаУчета - СправочникСсылка.ИдентификаторыОбъектовМетаданных - Идентификатор объекта учета.
//  ИдентификаторКомандыПечати - Строка, Неопределено - Идентификатор команды печати.
// 
// Возвращаемое значение:
//  Массив из СправочникСсылка.ВидыДокументовЭДО - Массив ссылок на вид электронного документа.
//
Функция ВидыВнутреннихДокументов(ИдентификаторОбъектаУчета, ИдентификаторКомандыПечати = Неопределено) Экспорт
	
	Если Не ЗначениеЗаполнено(ИдентификаторОбъектаУчета) Тогда
		ВызватьИсключение НСтр("ru = 'Не заполнено значение параметра ""ИдентификаторОбъектаУчета""'");
	КонецЕсли;
	...
	|	Справочник.ВидыДокументовЭДО КАК ВидыДокументовЭДО
	|ГДЕ
	|	ВидыДокументовЭДО.ТипДокумента = ЗНАЧЕНИЕ(Перечисление.ТипыДокументовЭДО.Внутренний)
	|	И ВидыДокументовЭДО.ИдентификаторОбъектаУчета В (&ИдентификаторОбъектаУчета)
	|	И ВЫБОР
	|		КОГДА &ИдентификаторКомандыПечати = НЕОПРЕДЕЛЕНО
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ВидыДокументовЭДО.ИдентификаторКомандыПечати = &ИдентификаторКомандыПечати
	|	КОНЕЦ";
...
Функция ОписаниеСообщенияОтправителя(ОбъектУчета, ДанныеОбъектаУчета, НастройкиОтправки, ЗначенияДополнительныхПолей = Неопределено) Экспорт
	...
	Если ТипЗнч(ДанныеОбъектаУчета) = Тип("Структура") Тогда
		ДанныеДляФормирования.УникальныйИдентификатор = Строка(Новый УникальныйИдентификатор);
		ДанныеДляФормирования.ДанныеДокумента = ДанныеОбъектаУчета.ДеревоЭД;
		ДанныеДляФормирования.Вставить("ДанныеДО", ДанныеОбъектаУчета.ДанныеДО);
...
Функция ПредставлениеКарточкиСообщения(Сообщение, ОтветноеСообщение = Неопределено) Экспорт
	...
	Если Выборка.Следующий() Тогда
		...
		УстановленныеПодписи = ЭлектронныеДокументыЭДО.УстановленныеПодписиСУчетомДоверенностей(Сообщение);
		
		Если ЗначениеЗаполнено(ОтветноеСообщение) Тогда
			ОтветныеПодписи = ЭлектронныеДокументыЭДО.УстановленныеПодписиСУчетомДоверенностей(ОтветноеСообщение);
...
Процедура ПодменитьФайлНаФайлСВизуализацией(
	...
	Попытка
		Выполнить(СтрокаДляВыполнения); // АПК:488 Исполняемый код безопасен.
	Исключение	
...
Функция СоздатьДокументПоФайлу(ПараметрыСоздания, ОписаниеФайла) Экспорт
	...
	Если ПараметрыСоздания.Свойство("ТребуетсяОтветнаяПодпись")
		И ЗначениеЗаполнено(ПараметрыСоздания.ТребуетсяОтветнаяПодпись) Тогда
		
		НастройкиДокумента.ТребуетсяПодтверждение = ПараметрыСоздания.ТребуетсяОтветнаяПодпись;
	КонецЕсли;
	Если ПараметрыСоздания.Свойство("ТребуетсяИзвещениеОПолучении")
		И ЗначениеЗаполнено(ПараметрыСоздания.ТребуетсяИзвещениеОПолучении) Тогда
		
		НастройкиДокумента.ТребуетсяИзвещение = ПараметрыСоздания.ТребуетсяИзвещениеОПолучении;
	КонецЕсли;
...
Функция УстановленныеПодписиФайла(ПрисоединенныйФайл, ВидПодписи = Неопределено)
	...
	Если Не ЗначениеЗаполнено(ВидПодписи)
		...
		Для Каждого Подпись Из УстановленныеПодписи Цикл
			...
			УстановленнаяПодпись.ПорядковыйНомер = ПорядковыйНомер;
...
Процедура УдалитьПодписи(ФайлДокумента, ВидПодписи)
	...
	Для Каждого СвойстваПодписи Из Подписи Цикл
		ЭлектроннаяПодпись.УдалитьПодпись(ФайлДокумента, СвойстваПодписи.УникальныйИдентификатор);
	КонецЦикла;
...
Процедура ЗаполнитьПараметрыФормированияИсходящихДокументов(ПараметрыВыполнения, РезультатЗапросаНастроек, НаборПараметровФормирования, РезультатДействий)
	...
	Пока ВыборкаНастроек.Следующий() Цикл
		...
		Если ОбработанныеНастройки.НайтиСтроки(СтруктураПоиска).Количество() = 0 Тогда
...
Процедура ДобавитьПараметрыФормированияИсходящегоДокумента(ПараметрыВыполнения, ВыборкаНастроек, НаборПараметровФормирования, ТаблицаСвязанныхОбъектовУчета, РезультатДействий)
	...
	Если Не ЗначениеЗаполнено(НастройкиОтправки.Формат) 
		И Не (МодульОбменСГИСЭПД <> Неопределено И МодульОбменСГИСЭПД.ЭтоДокументЭПД(НастройкиОтправки.ТипДокумента)) Тогда
		ИдентификаторПечатнойФормы = ПараметрыВыполнения.ИдентификаторыПечатныхФормОбъектов[НастройкиОтправки.ОбъектУчета];
		ОписаниеФайлаПечатнойФормы = СформироватьДанныеПечатнойФормыОбъектаУчета(РезультатДействий,
			НастройкиОтправки.ОбъектУчета, НастройкиОтправки, ИдентификаторПечатнойФормы);
		
		ОписаниеСообщения = ОписаниеСообщенияОтправителяПроизвольногоФормата(НастройкиОтправки.ОбъектУчета, НастройкиОтправки,
			ОписаниеФайлаПечатнойФормы);
	ИначеЕсли НастройкиОтправки.ТипДокумента = Перечисления.ТипыДокументовЭДО.Прикладной Тогда
		ОписаниеСообщения = ЭлектронныеДокументыЭДО.ОписаниеСообщенияОтправителяПрикладногоДокумента(
			НастройкиОтправки.ОбъектУчета, НастройкиОтправки, НастройкиОтправки.ПрикладнойТипДокумента);
	Иначе
		ОписаниеСообщения = ОписаниеСообщенияОтправителя(НастройкиОтправки.ОбъектУчета, ОписаниеДанных.Данные, НастройкиОтправки,
...
Функция ЗаполнитьПодписантовИДоверенностиПоСертификату(ДанныеДляЗаполнения, Сертификат, КонтекстДиагностики) Экспорт
	...
	Для Каждого Элемент Из ДанныеДляЗаполнения Цикл
		...
		Иначе
			ПрисоединенныеФайлыМассив.Добавить(Элемент.Ключ);
		КонецЕсли;
	КонецЦикла;
	...
	|ГДЕ
	|	ПрисоединенныеФайлы.Ссылка В(&ПрисоединенныеФайлы)";
	Запрос.УстановитьПараметр("ПрисоединенныеФайлы", ПрисоединенныеФайлыМассив);

	ДвоичныеДанныеФайлов = РаботаСФайламиБЭД.ДвоичныеДанныеФайлов(ПрисоединенныеФайлыМассив, КонтекстДиагностики);
...
// Обновляет сосотояние сообщения при установке подписи.
// 
// Параметры:
...
Процедура ДобавитьОписаниеДанныхПодписи(ДанныеПодписей, СвойстваПодписи, СвойстваФайла)
	...
	Если ПустаяСтрока(ОписаниеДанныхПодписи.ИмяФайла) Тогда
		ОписаниеДанныхПодписи.ИмяФайла = ИмяФайлаПодписи(СвойстваФайла.ПолноеИмяФайла,
			СвойстваПодписи.УникальныйИдентификатор);
	КонецЕсли;
...
Функция ЗагрузитьОтветнуюПодпись(ДанныеОбъекта, ПроверенныеПодписи, КонтекстДиагностики)
	...
	Если Не ЕстьНовыеПодписи Тогда
		ДанныеОбъекта.Отказ = Истина;
		ТекстОшибки = СтрШаблон(НСтр("ru = 'Данная подпись уже существует в документообороте %1'"), ДанныеОбъекта.ИдентификаторДокументооборота);
		СинхронизацияЭДО.ОбработатьОшибкуЗагрузкиОбъекта(ДанныеОбъекта, КонтекстДиагностики, ТекстОшибки, Истина);
		Возврат Результат;
	КонецЕсли;
	...
	// Загрузим подпись в файл документа ДО
	ОбменСКонтрагентамиДОСлужебный.ЗагрузитьОтветнуюПодписьВФайлДО(СообщениеОбъект, ОтветныеПодписи);
...
Процедура ПослеЗагрузкиСообщения(РезультатЗагрузкиСообщения, КонтекстДиагностики)
	...
	Если РезультатЗагрузкиСообщения.ЭтоНовыйДокумент Тогда
		...
		// Добавим в очередь приема Документооборота
		Если ОбменСКонтрагентамиДОСлужебный.НеобходимоДобавитьВОчередь(РезультатЗагрузкиСообщения.СообщениеОбъект) Тогда
