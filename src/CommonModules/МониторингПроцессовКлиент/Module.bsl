...
//
// Параметры:
//  Владелец - ФормаКлиентскогоПриложения - Владелец открывшейся формы.
//
...
Процедура ПересчитатьПоказатели(ПоказателиПроцессов, ДатаПересчета, УдалитьСтарыеЗначения) Экспорт
	...
	Если КоличествоПоказателей = 1 Тогда
		...
		ТекстОповещения = НСтр("ru = 'Запланирован пересчет показателя'");
	...
	Иначе
		ТекстОповещения = СтрШаблон(
			НСтр("ru = 'Запланирован пересчет показателей (%1)'"),
...
Процедура Отслеживать(ПоказателиПроцессов) Экспорт
	...
	Если Результат.КоличествоОбработанныхЭлементов = 1 Тогда
		Если Результат.УстановленоОтслеживание Тогда
			ТекстОповещения = НСтр("ru = 'Установлено отслеживание показателя'");
		Иначе
			ТекстОповещения = НСтр("ru = 'Снято отслеживание показателя'");
		КонецЕсли;
	...
	Иначе
		Если Результат.УстановленоОтслеживание Тогда
			ТекстОповещения = СтрШаблон(НСтр("ru = 'Установлено отслеживание показателей (%1)'"),
				Результат.КоличествоОбработанныхЭлементов);
		Иначе
			ТекстОповещения = СтрШаблон(НСтр("ru = 'Снято отслеживание показателей (%1)'"),
				Результат.КоличествоОбработанныхЭлементов);
		КонецЕсли;
		Пояснение = Неопределено;
		НавигационнаяСсылка = Неопределено;
