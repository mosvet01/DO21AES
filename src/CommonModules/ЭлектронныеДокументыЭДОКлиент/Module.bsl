...
//
Процедура НачатьВыполнениеДействийПоЭДО(ОповещениеОЗавершении, ПараметрыВыполнения,
		КонтекстДиагностики = Неопределено, ВнешнийКонтекстДействийПоЭДО = Неопределено) Экспорт
	...
	Контекст.Вставить("ВнешнийКонтекстДействийПоЭДО", ВнешнийКонтекстДействийПоЭДО);
...
//                          См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//
Процедура ОжидатьВыполнениеДействийПоЭДО(ОповещениеОЗавершении, ДлительнаяОперация,
		КонтекстДиагностики = Неопределено, ВнешнийКонтекстДействийПоЭДО = Неопределено) Экспорт
	...
	Если ТипЗнч(ВнешнийКонтекстДействийПоЭДО) = Тип("Структура") Тогда
		ЗаполнитьЗначенияСвойств(КонтекстДействийПоЭДО, ВнешнийКонтекстДействийПоЭДО);
	КонецЕсли;
...
Процедура ВыполнитьДействияПоЭДОПослеПолученияОтпечатков(ОтпечаткиСертификатов, Контекст) Экспорт
	...
	ВыполнитьДействияПоЭДО(Контекст.ОповещениеОЗавершении, Контекст.ПараметрыВыполнения,
		Контекст.КонтекстДиагностики, Контекст.ВнешнийКонтекстДействийПоЭДО);
...
//                          См. ОбработкаНеисправностейБЭДКлиент.НовыйКонтекстДиагностики
//
Процедура ВыполнитьДействияПоЭДО(ОповещениеОЗавершении, ПараметрыВыполнения,
		КонтекстДиагностики = Неопределено, ВнешнийКонтекстДействийПоЭДО = Неопределено)
	...
	ОжидатьВыполнениеДействийПоЭДО(ОповещениеОЗавершении, ДлительнаяОперация,
		КонтекстДиагностики, ВнешнийКонтекстДействийПоЭДО);
