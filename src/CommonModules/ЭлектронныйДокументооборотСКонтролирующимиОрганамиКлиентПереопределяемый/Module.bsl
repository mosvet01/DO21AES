...
Процедура ОткрытьФормуВладельцаЭЦПНаРеквизите(ВладелецЭЦП, НаименованиеРеквизита) Экспорт
	...
	СоответствиеИмен.Вставить("ФИО",   "Наименование");
	...
	Если Форма.Элементы.Найти(ИмяРеквизитаФормы)<> Неопределено Тогда 
		Если ЗначениеЗаполнено(ИмяРеквизитаФормы) Тогда
...
Процедура ОткрытьФормуГлБухгалтера(Организация) Экспорт
	...
	Иначе
		ОткрытьФормуОрганизацииНаРеквизите(Организация, "ОтветственныеЛица");
	КонецЕсли;
...
Процедура ОткрытьФормуОрганизацииНаРеквизите(Организация,НаименованиеРеквизита) Экспорт
	...
	СоответствиеИмен.Вставить("ОтветственныеЛица",		"ГруппаОтветственныеЛица");
...
Процедура ПолучитьИсполнителяПродолжение(ВыбранноеЗначение, ВыполняемоеОповещение) Экспорт
	
	//получить физ лицо от пользователя и вернуть дальше
	ФизЛицо = ОбщегоНазначенияДокументооборотВызовСервера.ЗначениеРеквизитаОбъекта(ВыбранноеЗначение, "ФизЛицо");
	
	Если ЗначениеЗаполнено(ФизЛицо) Тогда
		ВыполнитьОбработкуОповещения(ВыполняемоеОповещение, ФизЛицо);
	Иначе
		ВызватьИсключение НСтр("ru = 'У выбранного пользователя не указано физическое лицо.
									|Обратитесь к администратору для заполнения необходимых сведений.'");
	КонецЕсли;
...
Функция СНИЛСДоступенДляРедактирования(ФизЛицо) Экспорт
	// сейчас СНИЛС не вводится в карточке ФЛ.
	Возврат Ложь;
КонецФункции
