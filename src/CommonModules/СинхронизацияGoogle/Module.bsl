...
Функция ИдентификаторПриложения() Экспорт
	...
	Возврат ОбщегоНазначенияДокументооборот.ИдентификацияПриложенияДляGoogle();
...
Процедура ВыгрузитьДанныеВУзел(Узел, ИдентификаторКалендаря, ДанныеДляВыгрузки, ДанныеАвторизации, ВремяУведомлений)
	...
	ПланыОбмена.ВыбратьИзменения(Узел, ЗаписьСообщения.НомерСообщения, ВыгруженныеЗаписиКалендаря);
...
Процедура ЗаполнитьСобытияКалендаря(ДанныеАвторизации, ОписаниеКалендаря, ДатаНачала, ДатаОкончания, СтраничныйТокен = Неопределено)
	...
	Для Каждого ТекОписаниеСобытия Из РезультатЧтенияJSON["items"] Цикл
		...
		НоваяСтрока.ИдентификаторПовторения	= ТекОписаниеСобытия["recurringEventId"];
		...
		Если НоваяСтрока.ПравилоПовторения.Количество() = 0 Тогда
			НоваяСтрока.ПравилоПовторения = РаботаСРабочимКалендаремСервер.ПравилоПовторенияНикогда();
		КонецЕсли;
