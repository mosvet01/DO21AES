...
Процедура УдалитьПомеченныеОбъекты(
		...
		Иначе
			...
			Для Каждого УспешноУдаленный Из УспешноУдаленныеВРамкахТранзакции Цикл 
				...
				Если ИндексНайденногоЭлемента <> Неопределено Тогда
					...
					Если ЭтоОбъектИзСпискаУдаляемых Тогда
						ИсходныхУдаляемыхОбработано = ИсходныхУдаляемыхОбработано - 1;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
		
		ТекущийПроцент = Окр(90 * ИсходныхУдаляемыхОбработано / УдаляемыхВсего) + 5;
	...
	// Для подсчета количества из неудаленных нужно убрать элементы, найденные по ссылкам.
	КолЗаписей = НеУдаленные.Количество();
	Для Сч = 1 По КолЗаписей Цикл
		Индекс = КолЗаписей - Сч;
		Если Удаляемые.Найти(НеУдаленные[Индекс]) = Неопределено Тогда
			НеУдаленные.Удалить(Индекс);
		КонецЕсли;
	КонецЦикла;
...
Функция УдалитьОбъект(
	...
	Для Каждого УдаляемыйПоСсылке Из УдаляемыеПоСсылкам Цикл
		НайденныйПоСсылкеУдаленУспешно = УдалитьОбъект(
			...
			НеУдаленные, Найденные, УспешноУдаленныеВРамкахТранзакции, ИсходныхУдаляемыхОбработано);
	...
	Для Каждого СтрокаСсылки Из ТаблицаСсылок Цикл
		Если Не ЗначениеЗаполнено(СтрокаСсылки.ИмяРегистраСведений) Тогда
			Продолжить;
		КонецЕсли;
		// ОтметкиВремени должны хранить ссылки удаленных объектов.
		Если СтрНачинаетсяС(СтрокаСсылки.ИмяРегистраСведений, "ОтметкиВремени") Тогда  
			Продолжить;
		КонецЕсли;
...
Процедура УдалитьДанныеСвязанныхРегистров(УдаляемыеОбъекты)
	...
	// Удалить ОтметкиВремениИзмененийСведенийПисем
	ИмяРегистра = "ОтметкиВремениИзмененияСведенийПисем";
	ПоляПоиска = Новый Массив;
	ПоляПоиска.Добавить("Письмо");
	УдалитьДанныеСвязанногоРегистра(ИмяРегистра, ПоляПоиска, УдаляемыеОбъекты);
	
	// Удалить МП_ОтправленныеСсылочныеДанные
	ИмяРегистра = "МП_ОтправленныеСсылочныеДанные";
	ПоляПоиска = Новый Массив;
	ПоляПоиска.Добавить("Ссылка");
	УдалитьДанныеСвязанногоРегистра(ИмяРегистра, ПоляПоиска, УдаляемыеОбъекты);
