...
Процедура МониторингПроцессов() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.МониторингПроцессов, Отказ);
...
Функция ПолучитьПерсональнуюНастройку(КлючНастройки) Экспорт
	...
	Иначе
		...
		ВызватьИсключение СтрШаблон(
			НСтр("ru = 'Некорректная настройка мониторинга процессов: ""%1""'"),
			КлючНастройки);
...
Процедура УстановитьПерсональнуюНастройку(КлючНастройки, Значение) Экспорт
	...
	Если КлючНастройки = "ДатаНастройкиПоказателейПоУмолчанию" Тогда
		...
		Если ТипЗнч(Значение) <> Тип("Дата") Тогда
			...
			ВызватьИсключение СтрШаблон(
				НСтр("ru = 'Некорректное значение настройки мониторинга процессов: ""%1""'"),
				КлючНастройки);
...
Функция СобытиеЖурналаРегистрации()
	...
	СобытиеЖурналаРегистрации = НСтр("ru = 'Мониторинг процессов'",
...
Функция СтруктурыВыбираемыхПолей(ИсточникСхемыЗапроса, ВариантРасчета)
	...
	Если Не ЗначениеЗаполнено(ВариантРасчета.ДанныеДляРасчета) Тогда
		ТекстОшибки = НСтр("ru = 'Не указаны данные для расчета.'");
...
Функция ВыражениеРасчета(ВариантРасчета)
	...
	Если Не ЗначениеЗаполнено(ВариантРасчета.СпособРасчета) Тогда
		ТекстОшибки = НСтр("ru = 'Не указан способ расчета.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ИмяИсточника(ТипПроцесса)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Неизвестный тип процесса.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция ИмяТипаПредмета(ТипПредмета)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Неизвестный тип предмета.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция РазвернутьОтветственных(Ответственный)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Неизвестный ответственный.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Функция РеквизитВидаПредмета(ВидПредмета)
	...
	Иначе
		ТекстОшибки = НСтр("ru = 'Неизвестный вид предмета.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ДобавитьСоединенияИсточника(ТекстИсточники, Источники, Источник, УровеньВложенности)
	...
	Если УровеньВложенности > 10 Тогда
		ВызватьИсключение НСтр("ru = 'Превышен уровень вложенности соединения в запросе.'");
	КонецЕсли;
