...
//
Процедура ПроверитьЗаполнениеСторон(СправочникОбъект, Отказ, ЭтоЗаявкаНаОплату) Экспорт 
	...
	ИначеЕсли Стороны.Количество() = 1 И Не ЭтоЗаявкаНаОплату Тогда 
		ОбщегоНазначения.СообщитьПользователю(
	...
	Если ИспользоватьУчетПоОрганизациям Тогда
		...
		Если Не ПерваяИзСторонОрганизация Тогда
			ОбщегоНазначения.СообщитьПользователю(ТекстОшибки,, ИмяПоля, "Объект", Отказ);
		КонецЕсли;
