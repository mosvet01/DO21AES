...
Процедура ЗадатьВопросПоОшибкеИзМассиваПередСтартомБизнесПроцесс(Результат, Параметры) Экспорт
	...
	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,, КодВозвратаДиалога.Нет, НСтр("ru = 'Запуск процесса'"));
	...
	Если ЗначениеЗаполнено(ПлановаяДатаОкончания) Тогда
		...
		Если ЗадатьВопрос Тогда
			...
			ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет,,
				КодВозвратаДиалога.Нет, НСтр("ru = 'Запуск процесса'"));
	...
	Если РаботаСПроектами.ИзменитьПапкуПроектов(МассивПроектов, НоваяПапка) Тогда
		...
		Если МассивПроектов.Количество() = 1 Тогда
			...
			ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проект ""%1"" перенесен в папку ""%2""'"), МассивПроектов[0], НоваяПапка);
			
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Проект перенесен.'"),
		...
		Иначе
			...
			ПолноеОписание = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Проекты (%1 шт.) перенесены в папку ""%2""'"), МассивПроектов.Количество(), НоваяПапка);
			
			ПоказатьОповещениеПользователя(
				НСтр("ru = 'Проекты перенесены.'"),
				,
				ПолноеОписание,
				БиблиотекаКартинок.Информация32);
