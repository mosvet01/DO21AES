...
//
Функция ПолучитьДокументы(КлючСинхронизации, ДатаПоследнегоЗапроса, КонтекстДиагностики) Экспорт
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД")
		...
		ПараметрыВыполненияОперации.АдресРесурса = МодульОбменСГИСЭПД.АдресРесурсаПолученияСпискаСообщений();
		РезультатОперацииЭПД = ВыполнитьОперацию(ПараметрыВыполненияОперации, КонтекстДиагностики, КлючСинхронизации);
		Если РезультатОперацииЭПД.Успех = Истина Тогда
			СписокЭлектронныхДокументовЭПД = СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатОперацииЭПД,
				ДатаПоследнегоЗапроса);	
			Для Каждого СтрокаЭПД Из СписокЭлектронныхДокументовЭПД.ЭлектронныеДокументы Цикл
				НоваяСтрокаЭД = СписокЭлектронныхДокументов.ЭлектронныеДокументы.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаЭД, СтрокаЭПД);	
				НоваяСтрокаЭД.ЭтоЭПД = Истина;
			КонецЦикла;
		КонецЕсли;
	КонецЕсли;
...
Функция СписокЭлектронныхДокументовИзОтветаОтСервиса(РезультатВыполненияОперации, ДатаПоследнегоЗапроса)
...
Функция ЕстьДоступКВебСервису1CЭДО(КонтекстДиагностики)
	...
	РезультатПроверки = СервисЭДОПовтИспНаВызов.ПроверитьДоступКВебСервису1CЭДО();
...
//  Структура:
//  * Успех - Булево
//  * Ошибка - Неопределено - при значении Успех равное истина.
