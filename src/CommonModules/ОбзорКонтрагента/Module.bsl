...
Процедура ДобавитьЗначениеСКартинкой(HTMLТекст, Поле, Картинка, МассивЗначений,
			СсылкаНаРесурс = "", ВключаяЗначение = Истина)
	...
	Если ЗначениеЗаполнено(МассивЗначений) И ЗначениеЗаполнено(СсылкаНаРесурс) Тогда
		...
		Для Ит = 0 По Количество - 1 Цикл 
			...
			ЭлементМассива = МассивЗначений.Получить(Ит);
			ТекстСсылки = ЭлементМассива;
			Представление = ЭлементМассива;
			Если ТипЗнч(ЭлементМассива) = Тип("Структура") Тогда
				// Представление передано отдельно. Например, в значении сложная ссылка на сайт,
				// а в представлении текст с коротким и понятным текстом.
				Если ЭлементМассива.Свойство("Значение") Тогда
					ТекстСсылки = ЭлементМассива.Значение;
				КонецЕсли;
				Если ЭлементМассива.Свойство("Представление") Тогда
					Представление = ЭлементМассива.Представление;
				КонецЕсли;
			КонецЕсли;
			
			РаботаС_HTML.ЗаменитьСпецСимволыHTML(ТекстСсылки);
			РаботаС_HTML.ЗаменитьСпецСимволыHTML(Представление);
			
			Если ВключаяЗначение = Истина Тогда 
				Ссылка = СсылкаНаРесурс + ТекстСсылки;
			Иначе 
...
Функция ПолучитьОбзорКонтрагента(Контрагент, ПоказыватьУдаленные) Экспорт 
	...
	Для Каждого КИ Из КонтактнаяИнформация Цикл 
		Если КИ.Вид = Справочники.ВидыКонтактнойИнформации.СайтКонтрагента Тогда 
			ЗначенияКИ = УправлениеКонтактнойИнформациейСлужебный.JSONВКонтактнуюИнформациюПоПолям(
				КИ.Значение, Перечисления.ТипыКонтактнойИнформации.ВебСтраница);
			Если ЗначенияКИ.Свойство("value") И ЗначениеЗаполнено(ЗначенияКИ.value)
				И ЗначенияКИ.Свойство("name") И ЗначениеЗаполнено(ЗначенияКИ.name) Тогда
				Сайт.Добавить(Новый Структура("Значение, Представление", ЗначенияКИ.value, ЗначенияКИ.name));
			Иначе
	...
	Если СПАРКРиски.ИспользованиеРазрешено() Тогда
		ПредставлениеИндексов = СПАРКРиски.ИндексыСПАРККонтрагента(
			Контрагент,
			ОбщегоНазначенияДокументооборотКлиентСервер.ВидКонтрагентаСпаркПоВидуКонтрагента(РеквизитыОбъекта.ЮрФизЛицо),
			Истина);
