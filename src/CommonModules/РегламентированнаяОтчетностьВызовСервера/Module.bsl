...
Функция ПолучитьСведенияОПредставителе(ФормаОрганизация, ФормаДатаПодписи, Знач МожетБытьТолькоЮРЛицо = Неопределено, Знач ИФНС = Неопределено, КПП = Неопределено) Экспорт
	...
	Если РегламентированнаяОтчетность.ПредставительЯвляетсяФизЛицом(Представитель) Тогда
		...
		ДомашнийТелефонФизЛица = Справочники.ВидыКонтактнойИнформации.ДомашнийТелефонФизическогоЛица;
		...
		// АДРЕСАНАЯ ЧАСТЬ
		// Адаптация ДО. Замена АдресМестаПроживанияФизическиеЛица.
		Данные = Новый Структура("Тип, Вид", Перечисления.ТипыКонтактнойИнформации.Адрес, Справочники.ВидыКонтактнойИнформации.ДомашнийАдресФизическогоЛица);
...
Функция ИННСоответствуетТребованиямНаСервере(Знач ИНН, Знач ТипЛица) Экспорт
	...
	//Аадптация ДО. ЮридическоеФизическоеЛицо.
