...
// Возвращает подготовленные к отправке контейнеры.
// 
// Возвращаемое значение:
// 	Массив из ДокументСсылка.ТранспортныйКонтейнерЭДО
Функция ПодготовленныеКОтправкеКонтейнеры() Экспорт
	...
	|	И ТранспортныйКонтейнерЭДО.Статус = &Статус";
	
	Запрос.УстановитьПараметр("Статус", Перечисления.СтатусыТранспортныхСообщенийБЭД.ПодготовленКОтправке);
...
// Возвращает вид ошибки, описывающий ситуацию, когда в карточке входящего документа не указан обратный 
// адрес контрагента.
// 
// Возвращаемое значение:
...
Функция ВидОшибкиНеУказанОбратныйАдресКонтрагента() Экспорт
	...
	ВидОшибки.Идентификатор = "НеУказанОбратныйАдресКонтрагента";
	ВидОшибки.ЗаголовокПроблемы = НСтр("ru = 'Не удалось сформировать транспортный контейнер ЭДО'");
	ВидОшибки.ОписаниеПроблемы = НСтр("ru = 'Не заполнен обратный адрес контрагента'");
	ВидОшибки.ОписаниеРешения = НСтр("ru = 'Рекомендуется обновить конфигурацию информационной базы корреспондента
...
Функция СформироватьДанныеОбъектов(ТранспортныеКонтейнеры, КонтекстДиагностики = Неопределено)
	...
	Для каждого ТранспортныйКонтейнер Из ТранспортныеКонтейнеры Цикл
		...
		Попытка
			...
			Если (ЗначенияРеквизитовКонтейнера.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.Неизвестный
				Или ЗначенияРеквизитовКонтейнера.Статус = Перечисления.СтатусыТранспортныхСообщенийБЭД.КРаспаковке) Тогда
