...
// Проверяет наличие учетных записей электронного документооборота для оператора.
// 
// Параметры:
// 	Оператор - Строка
// Возвращаемое значение:
// 	Булево
Функция ЕстьУчетныеЗаписиОператора(Оператор) Экспорт
	...
	|	УчетныеЗаписи";
	ИтоговыйЗапрос = ОбщегоНазначенияБЭД.СоединитьЗапросы(Запрос, Запросы);
	ИтоговыйЗапрос.УстановитьПараметр("Оператор", Оператор);
