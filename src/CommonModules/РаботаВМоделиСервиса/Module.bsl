...
Функция КонечнаяТочкаМенеджераСервисаНастроена() Экспорт
	...
	Возврат РаботаВМоделиСервисаПовтИсп.КонечнаяТочкаМенеджераСервисаНастроена()
...
//		** Значение - см. ZipАрхивы.ПрочитатьЗаписьКаталога
//
Функция ПрочитатьАрхив(Знач ИдентификаторФайла) Экспорт
	
	Возврат ПрочитатьАрхивВнутр(ИдентификаторФайла);
	
КонецФункции

// Параметры:
// 	ДанныеАрхива - см. РаботаВМоделиСервиса.ПрочитатьАрхив
// 	ИмяФайла - Строка - имя файла в архиве
// 	
// Возвращаемое значение:
//  см. РаботаВМоделиСервиса.ПрочитатьАрхив 
Функция ПрочитатьВложенныйНесжатыйАрхив(ДанныеАрхива, ИмяФайла) Экспорт
	...
	Возврат ПрочитатьАрхивВнутр(ДанныеАрхива.ИдентификаторФайла, Смещение, НайденнаяЗапись.НесжатыйРазмер);
...
Функция ИзвлечьФайл(ДанныеАрхива, ИмяФайла, Каталог) Экспорт
	...
	РазмерФайла = 30 + НайденнаяЗапись.Буфер.ПрочитатьЦелое16(28) + 0 + НайденнаяЗапись.СжатыйРазмер;
	
	ВремФайл = ПрочитатьВФайл(ДанныеАрхива, НайденнаяЗапись.СмещениеФайла, РазмерФайла);
	...
	Разница = ПравильныйРазмер - РазмерФайла;
	Если Разница > 0 Тогда
		РазмерФайла = ПравильныйРазмер;
		ПотокЧтения.Закрыть();
		ЗаписьДанных = Новый ЗаписьДанных(ВремФайл, , , , Истина);
		ЗаписьДанных.ЗаписатьБуферДвоичныхДанных(ПрочитатьВБуферДвоичныхДанных(ДанныеАрхива, НайденнаяЗапись.СмещениеФайла + РазмерФайла, Разница));
		ЗаписьДанных.Закрыть();
		ПотокЧтения = ФайловыеПотоки.ОткрытьДляЧтения(ВремФайл);
	КонецЕсли;
	
	Если РазмерФайла > 128 * 1024 * 1024 Тогда 
		ИмяАрхива = ПолучитьИмяВременногоФайла("zip");
	Иначе
		ВременныйБуфер = Новый БуферДвоичныхДанных(РазмерФайла + НайденнаяЗапись.Буфер.Размер + 22);
	...
	КонецКаталога =  ZipАрхивы.НовыйКонецКаталога(1, РазмерКаталога, СмещениеКаталога);
...
Функция ПрочитатьАрхивВнутр(Знач ИдентификаторФайла, Знач Смещение = 0, Знач Размер = Неопределено)
	
	ПолныйРазмер = ПолучитьРазмерФайлаИзХранилищаМенеджераСервиса(ИдентификаторФайла);
	...
	ДанныеАрхива.Вставить("РазмерФайла", Размер);
	...
	КонецКаталога = КонецКаталога(ДанныеАрхива, ДанныеАрхива.РазмерФайла);
	...
	Если КонецКаталога.СмещениеКаталога = ЧислоИзШестнадцатеричнойСтроки("0xFFFFFFFF") 
		...
		Локатор64 = Локатор64(ДанныеАрхива, ДанныеАрхива.РазмерФайла - КонецКаталога.Буфер.Размер);
		КонецКаталога64 = КонецКаталога64(ДанныеАрхива, Локатор64.Смещение);
		ЗаписиКаталога = ЗаписиКаталога(ДанныеАрхива, КонецКаталога64.СмещениеКаталога, КонецКаталога64.РазмерКаталога);
	Иначе
		ЗаписиКаталога = ЗаписиКаталога(ДанныеАрхива, КонецКаталога.СмещениеКаталога, КонецКаталога.РазмерКаталога);
	КонецЕсли;
	...
	Пока Смещение < Размер Цикл
		ЗаписьКаталога = ZipАрхивы.ПрочитатьЗаписьКаталога(Буфер, Смещение);
...
Функция КонецКаталога64(ДанныеАрхива, Начало)
...
Функция Локатор64(ДанныеАрхива, Конец)
...
Функция КонецКаталога(ДанныеАрхива, РазмерФайла)
	
	КонецКаталога = ПрочитатьВБуферДвоичныхДанных(ДанныеАрхива, РазмерФайла - 22, 22);
...
Функция ЗаписиКаталога(ДанныеАрхива, Начало, Размер)
	
	Возврат ПрочитатьВБуферДвоичныхДанных(ДанныеАрхива, Начало, Размер);
	
КонецФункции

Функция ПрочитатьВФайл(ДанныеАрхива, Начало, Размер)
	
	Диапазон = Новый Структура("Начало, Конец", Начало + ДанныеАрхива.Смещение, Начало + Размер - 1 + ДанныеАрхива.Смещение);
	Результат = ПередачаДанныхСервер.ПолучитьИзЛогическогоХранилища(ДанныеАрхива.ПараметрыДоступа, "files", ДанныеАрхива.ИдентификаторФайла, Диапазон);
...
Функция ПрочитатьВБуферДвоичныхДанных(ДанныеАрхива, Начало, Размер)
	...
	ИмяФайла = ПрочитатьВФайл(ДанныеАрхива, Начало, Размер);
