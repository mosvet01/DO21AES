...
Функция ШаблоныПоОбъектам(Объекты, ТипШаблона = Неопределено) Экспорт
	...
	МенеджерШаблоновОбработки = Справочники.ШаблоныСоставныхБизнесПроцессов;
	...
	Для Каждого ИмяШаблона Из ИменаШаблонов Цикл
		...
		Если ТипТекущегоШаблона = Тип("СправочникСсылка.ШаблоныСоставныхБизнесПроцессов") Тогда
			ПолучаемыеТипыШаблонов.Добавить(МенеджерШаблоновОбработки.ТипШаблонаОбработкаВнутреннегоДокумента());
			ПолучаемыеТипыШаблонов.Добавить(МенеджерШаблоновОбработки.ТипШаблонаОбработкаВходящегоДокумента());
			ПолучаемыеТипыШаблонов.Добавить(МенеджерШаблоновОбработки.ТипШаблонаОбработкаИсходящегоДокумента());
		Иначе
	...
	Для Каждого СтрокаТаблицы Из ШаблоныКандидаты Цикл
		...
		Если СтрокаТаблицы.ТипШаблона = Тип("СправочникСсылка.ШаблоныСоставныхБизнесПроцессов") Тогда
			СтрокаТаблицы.ТипШаблона = 
				ОбщегоНазначенияДокументооборот.ЗначениеРеквизитаОбъектаВПривилегированномРежиме(
				СтрокаТаблицы.Шаблон, "ТипШаблона");
		КонецЕсли;
	КонецЦикла;
		...
		Для Каждого ТипТекущегоШаблона Из ПолучаемыеТипыШаблонов Цикл
			...
			Для Каждого ШаблонСУсловием Из ШаблоныДляПроверкиУсловий Цикл
				Если ЗначениеЗаполнено(ШаблонСУсловием.Условие) Тогда
					Если РаботаСУсловиямиМаршрутизации.ПроверитьПрименимостьУсловияМаршрутизацииКОбъекту(
...
Процедура ЗаполнитьШаблоныПоУмолчанию() Экспорт
	...
	СхемаШаблона = Справочники.СхемыКомплексныхПроцессов.СхемаШаблонаПроцессаПоУмолчанию.ПолучитьОбъект();
	СхемаШаблона.ЗаполнитьСхемуШаблонаПроцессаПоУмолчанию();
	СхемаШаблона.Записать();
	...
	ПроцессОбъект = МенеджерШаблоновОбработки.ОбработкаВходящегоДокументаПоУмолчанию.ПолучитьОбъект();
	ПроцессОбъект.ЗаполнитьШаблонПоУмолчанию();
	ПроцессОбъект.Записать();
	
	ПроцессОбъект = МенеджерШаблоновОбработки.ОбработкаИсходящегоДокументаПоУмолчанию.ПолучитьОбъект();
	ПроцессОбъект.ЗаполнитьШаблонПоУмолчанию();
	ПроцессОбъект.Записать();
	
	ПроцессОбъект = МенеджерШаблоновОбработки.ОбработкаВнутреннегоДокументаПоУмолчанию.ПолучитьОбъект();
	ПроцессОбъект.ЗаполнитьШаблонПоУмолчанию();
	ПроцессОбъект.Записать();
...
Процедура ЗаполнитьПоШаблонуСоставногоБизнесПроцесса(Шаблон, БизнесПроцесс) Экспорт
	...
	Если Шаблон = Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаВнутреннегоДокументаПоУмолчанию
		Или Шаблон = Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаВходящегоДокументаПоУмолчанию
		Или Шаблон = Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаИсходящегоДокументаПоУмолчанию Тогда
		
		// Не заполняем шаблон в процессе, если это шаблон по умолчанию.
		// Считаем, что процесс создан с нуля.
	ИначеЕсли Шаблон.ШаблонВКомплексномПроцессе И ЗначениеЗаполнено(Шаблон.ИсходныйШаблон) Тогда
...
Функция ПолучитьСписокДоступныхФункций(ИменаПредметовДляФункций = Неопределено, ВключатьНедоступные = Истина) Экспорт
	...
	Если ИменаПредметовДляФункций <> Неопределено Тогда
		Если ИменаПредметовДляФункций.Количество() > 0 Тогда
			Для Каждого ИмяПредмета Из ИменаПредметовДляФункций Цикл
				...
				ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.РуководительПодразделенияДокумента(Объект, ИмяПредмета)",
					СтрШаблон(НСтр("ru = '%1.Руководитель подразделения документа'"),Строка(ИмяПредмета)));
		...
		ИначеЕсли ВключатьНедоступные Тогда
			...
			ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.РуководительПодразделенияДокумента(Объект, ИмяПредмета)",
				СтрШаблон(НСтр("ru = '%1.Руководитель подразделения документа'"),СтрокаИмениПредмета), Истина);
			...
			ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.СекретарьМероприятия(Объект, ИмяПредмета)",
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '%1.Секретарь мероприятия'"),СтрокаИмениПредмета), Истина);
	...
	ДоступныеФункции.Добавить("ШаблоныБизнесПроцессов.НепосредственныйРуководительИсполнителя(Объект)", 	 НСтр("ru = 'Непосредственный руководитель исполнителя'"));
...
// Возвращает руководителя подразделения по документу.
//
// Параметры:
//   БизнесПроцессОбъект - БизнесПроцессОбъект.<Тип бизнес процесса> - процесс, для которого
//                         вычисляется автоподстановка
