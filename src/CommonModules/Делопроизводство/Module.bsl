	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьУчетПоОрганизациям") Тогда 
		...
		Если ЗначениеЗаполнено(СправочникОбъект.НоменклатураДел) 
			...
			Если ТипЗнч(СправочникОбъект) = Тип("СправочникОбъект.ШаблоныВнутреннихДокументов")
				...
				ТекстСообщения = РедакцииКонфигурацииКлиентСервер.ОшибкаНеСовпаденияОрганизацииНоменклатурыДел(Истина);
			Иначе 
...
Функция ЗарегистрироватьПредмет(Предмет, СообщениеОбОшибке, РегистрационныйНомер = "") Экспорт 
	...
	Если РегистрационныйНомер = "" Тогда // сформировать автоматически
		Если МиграцияДанныхИзВнешнихСистемСервер.РегистрацияВ1СДО30() Тогда
			...
			РегДанные = МиграцияДанныхИзВнешнихСистемСервер.РегистрационныеДанныеИзДО30(
				ПредметОбъект, СообщениеОбОшибке);
			Если ЗначениеЗаполнено(СообщениеОбОшибке) Тогда
				ВызватьИсключение СообщениеОбОшибке;
			КонецЕсли;
			
			ЗаполнитьЗначенияСвойств(ПредметОбъект, РегДанные);	
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемыДокументооборота.МЭДО") Тогда
		...
		МодульМЭДО.УстановитьСостояниеДокументЗарегистрирован(ПредметОбъект);
		МодульМЭДО.ПриРегистрацииДокумента(ПредметОбъект);
	КонецЕсли;
...
Функция СтрокаПодписалУтвердил(Документ) Экспорт
	...
	Если РеквизитыВидаДокумента.ИспользоватьУтверждение Тогда
		...
		Если ГрифыУтверждения.Количество() > 0 Тогда 
			...
			РезультатУтверждения = ?(ГрифыУтверждения.Количество() > 0,
...
Процедура ОбновитьВидимостьРеквизитовХранения(Форма, Параметры = Неопределено) Экспорт 
	...
	Объект = Форма.Объект;
	Обязательно = Перечисления.ВариантыИспользованияПолейХранения.Обязательно;
	...
	Элементы.МестоХранения.АвтоОтметкаНезаполненного = 
		Параметры.УчитыватьМестоХранения = Обязательно И ЗначениеЗаполнено(Объект.РегистрационныйНомер);
	...
	Элементы.ОтветственныйЗаХранение.АвтоОтметкаНезаполненного = 
		Параметры.УчитыватьОтветственногоЗаХранение = Обязательно И ЗначениеЗаполнено(Объект.РегистрационныйНомер);
...
//  						и др. - Любой объект, у которого есть свойства ДатаРегистрации и ДатаСоздания
// 
// Возвращаемое значение:
