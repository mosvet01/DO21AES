...
//	ЕстьАктивныеПодзадачи - Булево - есть ли активные подзадачи
//
// Возвращаемое значение:
//   Булево   - Истина, если перенаправление прошло успешно.
//
Функция ПеренаправитьЗадачу(Задача, ВладелецФорма = Неопределено,
	ЕстьАктивныеПодзадачи = Неопределено) Экспорт
	
	Если ЕстьАктивныеПодзадачи = Неопределено Тогда
...
Процедура ПеренаправитьЗадачуПослеВыбораВариантаОбработкиПодзадач(Результат, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли Результат = "ПрерватьПодзадачи" Тогда 
		...
		Причина = НСтр("ru = 'Перенаправление родительской задачи:'");
		БизнесПроцессыИЗадачиВызовСервера.ПрерватьПодзадачи(ДополнительныеПараметры.Задача, Причина);
		ПеренаправитьЗадачуПослеПодзадачи(ДополнительныеПараметры.Задача, ДополнительныеПараметры.ВладелецФорма);
...
//
//Параметры
//	ЗадачиМнеТекущаяЗадача - ссылка на задачу
//	ОписаниеОповещения - ОписаниеОповещения, которое будет вызвано потом
//
