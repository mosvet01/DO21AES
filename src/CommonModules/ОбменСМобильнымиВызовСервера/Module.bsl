...
Процедура ОбменСМобильнымиРегистрацияИзменений() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбменСМобильнымиРегистрацияИзменений, Отказ);
...
Процедура УдалениеСообщенийИнтеграцииСМобильнымКлиентом() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбменСМобильнымиУдалениеСообщенийИнтегрированныхСистем, Отказ);
...
Процедура УдалениеСтарыхЗаписейПротоколаМобильногоКлиента() Экспорт
	...
	Отказ = Ложь;
	ОбщегоНазначения.ПриНачалеВыполненияРегламентногоЗадания(Метаданные.РегламентныеЗадания.ОбменСМобильнымиУдалениеСтарыхЗаписейПротокола, Отказ);
...
Процедура УдалениеСообщенийИнтеграцииПоКлиенту(МобильныйКлиент = Неопределено) Экспорт
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВсеПриложения, ОбменСМобильнымиСерверПовтИсп.ПолучитьВсеУзлы());
	
	ОбщегоНазначенияКлиентСервер.ДополнитьМассив(ВсеПриложения, Справочники.ПользователиМобильногоПриложения.ВсеПользователиМП());
	...
	Для Каждого Приложение Из ВсеПриложения Цикл
		...
		Запрос.УстановитьПараметр("МобильныйКлиент", Приложение);
		Запрос.Текст = 
			"ВЫБРАТЬ
	...
	Запрос.Текст = 
		...
		|		ПО СведенияОМобильныхКлиентах.МобильныйКлиент = Мобильный.Ссылка
		|ГДЕ
		|	(СведенияОМобильныхКлиентах.ДатаПоследнейАктивности < &Порог
		|			ИЛИ СведенияОМобильныхКлиентах.ДатаПоследнейАктивности = ДАТАВРЕМЯ(1, 1, 1))
		...
		|
		|ВЫБРАТЬ
		|	СведенияОМобильныхКлиентах.МобильныйКлиент
		|ИЗ
		|	РегистрСведений.СведенияОМобильныхКлиентах КАК СведенияОМобильныхКлиентах
	...
	Пока Выборка.Следующий() Цикл
		...
		Приложение = Выборка.Ссылка.ПолучитьОбъект();
		Если Не Приложение = Неопределено Тогда
			Приложение.УстановитьПометкуУдаления(Истина);
		КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ПользователиМобильногоПриложения.ПометкаУдаления";
