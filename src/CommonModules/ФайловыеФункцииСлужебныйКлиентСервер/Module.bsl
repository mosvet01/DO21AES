...
#КонецЕсли
	...
	Если ПустаяСтрока(ИзвлеченныйТекст) Тогда
		...
		СписокРасширенийXlsx = "xlsx";
		Если РасширениеФайлаВСписке(СписокРасширенийXlsx, РасширениеИмениФайла) Тогда
			Возврат ИзвлечьТекстИзФайлаXlsx(ПолноеИмяФайла, Отказ);		
		КонецЕсли;	
...
// Извлечь текст из файла Xlsx и возвратить его в виде строки.
//
Функция ИзвлечьТекстИзФайлаXlsx(ПутьКФайлу, Отказ)
...
#Если Не ВебКлиент  И Не МобильныйКлиент Тогда
	...
	Попытка
		...
		ИмяФайлаXML = ВременнаяПапкаДляРазархивирования + "/xl/sharedStrings.xml";
		...
		ИзвлеченныйТекст = ИзвлечьТекстИзExcelXML(ЧтениеXML);
		ЧтениеXML.Закрыть();
	Исключение
		// Не считаем ошибкой, т.к. например расширение OTF может быть как OpenDocument, так и шрифт OpenType.
		Отказ = Истина;
...
// Извлечь текст из объекта ЧтениеXML (прочитанного из файла xlsx  - xl\sharedStrings.xml).
Функция ИзвлечьТекстИзExcelXML(ЧтениеXML)
...
#Если Не ВебКлиент Тогда
	...
	Пока ЧтениеXML.Прочитать() Цикл
		...
		Если ЧтениеXML.ТипУзла = ТипУзлаXML.Текст Тогда
			...
			Если СтрНайти(ПоследнееИмяТега, "t") <> 0 Тогда
				ИзвлеченныйТекст = ИзвлеченныйТекст + ЧтениеXML.Значение;
			КонецЕсли;
