...
Функция ПолучитьОсновныеБизнесПроцессы(ПроектнаяЗадача, ТолькоНезавершенные = Ложь) Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ТипЗнч(Выборка.Ссылка) = Тип("БизнесПроцессСсылка.РешениеВопросовВыполненияЗадач") Тогда
			Продолжить;
		КонецЕсли;
...
Функция ЕстьНезавершенныеПредшественники(ПроектнаяЗадача) Экспорт
	...
	Запрос.Текст =
		...
		|	И ПроектныеЗадачиПредшественники.ТипЗависимости = ЗНАЧЕНИЕ(Перечисление.ТипыЗависимостейПроектныхЗадач.ОкончаниеНачало)
		|	И НЕ ПроектныеЗадачиПредшественники.Предшественник.ПометкаУдаления";
	Запрос.УстановитьПараметр("ПроектнаяЗадача", ПроектнаяЗадача);
		...
		Для Каждого Участник Из ТаблицаИсполнителей Цикл
			...
			Если ТипЗнч(ВыборкаЗаписей.Источник) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
				...
				РеквизитыЗадачи = ОбщегоНазначенияДокументооборот.ЗначенияРеквизитовОбъектаВПривилегированномРежиме(
					ВыборкаЗаписей.Источник, "РольИсполнителя, Исполнитель");
					
				ДобавитьТрудозатраты = Ложь;
				Если ЗначениеЗаполнено(РеквизитыЗадачи.РольИсполнителя) Тогда
					ДобавитьТрудозатраты = (РеквизитыЗадачи.РольИсполнителя = Участник.Исполнитель);
				Иначе
					ИсполнителиРоли = РегистрыСведений.ИсполнителиЗадач.ИсполнителиРоли(Участник.Исполнитель);
					ДобавитьТрудозатраты = ИсполнителиРоли.Найти(РеквизитыЗадачи.Исполнитель) <> Неопределено;
				КонецЕсли;
				
				Если ДобавитьТрудозатраты Тогда
					Участник.ТрудозатратыФакт = Участник.ТрудозатратыФакт + ТрудозатратыФакт;
					ТрудозатратыПрисвоены = Истина;
				КонецЕсли;
...
Функция НайтиИсполнителяВПроектнойЗадачеСУчетомВхожденияВРоль(
	...
	Если ЗначениеЗаполнено(ЗадачаИсполнителя) И ТипЗнч(ЗадачаИсполнителя) = Тип("ЗадачаСсылка.ЗадачаИсполнителя") Тогда
		...
		// Если задача персональная и ее исполнитель не находится среди исполнителей проектной задачи,
		// то ищем среди них роли, которые исполняет исполнитель задачи процесса.
		Если Не ИсполнительНайден
			И ЗначениеЗаполнено(РеквизитыЗадачи.Исполнитель)
			И Не ЗначениеЗаполнено(РеквизитыЗадачи.РольИсполнителя) Тогда
			
			РолиИсполнителя = РегистрыСведений.ИсполнителиЗадач.РолиИсполнителя(РеквизитыЗадачи.Исполнитель);
			...
			Для Каждого ИсполнительПроектнойКоманды Из ПроектнаяЗадача.Исполнители Цикл
				...
				Если РолиИсполнителя.Найти(ИсполнительПроектнойКоманды.Исполнитель) <> Неопределено Тогда
					ИсполнительНайден = Истина;
					Прервать;
				КонецЕсли;
...
Функция НайтиИсполнителяРольВПроектнойЗадаче(
	...
	ИсполнителиПроектнойЗадачи = 
		ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ПроектнаяЗадача, "Исполнители").Выгрузить();
	Если ИсполнителиПроектнойЗадачи.Количество() = 0 Тогда 
		Возврат Истина;
	КонецЕсли;
	
	ПроверяемыеИсполнители = Новый Соответствие;
	ПроверяемыеИсполнители[Исполнитель] = Истина;
	
	Если ТипЗнч(Исполнитель) = Тип("СправочникСсылка.Пользователи") Тогда
		Для Каждого РольИсполнителя Из РегистрыСведений.ИсполнителиЗадач.РолиИсполнителя(Исполнитель) Цикл
			ПроверяемыеИсполнители[РольИсполнителя] = Истина;
		КонецЦикла;
	КонецЕсли;
	
	Для Каждого СтрокаТаблицы Из ИсполнителиПроектнойЗадачи Цикл
		Если ПроверяемыеИсполнители[СтрокаТаблицы.Исполнитель] = Истина Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Функция НайтиИсполнителяРольВКомандеПроекта(
	...
	РеквизитыПроекта = ОбщегоНазначенияДокументооборот.ЗначенияРеквизитовОбъектаВПривилегированномРежиме(
		Проект, "Руководитель, ПроектнаяКоманда");
	
	ПроектнаяКоманда = РеквизитыПроекта.ПроектнаяКоманда.Выгрузить();
	Если ПроектнаяКоманда.Количество() = 0 Тогда 
		Возврат Истина;
	КонецЕсли;
	
	НоваяСтрока = ПроектнаяКоманда.Добавить();
	НоваяСтрока.Исполнитель = РеквизитыПроекта.Руководитель;
	...
	Для Каждого СтрокаТаблицы Из ПроектнаяКоманда Цикл
		Если ПроверяемыеИсполнители[СтрокаТаблицы.Исполнитель] = Истина Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПроверитьВозможностьСтартаБизнесПроцесса(Знач Объект, ВозвращаемыйМассивОшибок, ЕстьПравоЗапуска) Экспорт
	...
	Если ЗначениеЗаполнено(Объект.ПроектнаяЗадача) Тогда
		...
		Если Не ЗначениеЗаполнено(Объект.ГлавнаяЗадача)
			...
			ТекстОшибки = СтрШаблон(НСтр("ru = 'У вас нет прав на старт основного процесса по проектной задаче ""%1""'"),
...
Функция ПолучитьДанныеПроектныхЗадач(ПроектныеЗадачи) Экспорт
	...
	Запрос.Текст =
		...
		|		ИНАЧЕ ПроектныеЗадачи.Владелец.ГрафикРаботы
		|	КОНЕЦ КАК ГрафикРаботы,
		|	ПроектныеЗадачи.Владелец.АвтоматическиОтправлятьПроектныеЗадачиНаИсполнение КАК АвтоматическиОтправлятьПроектныеЗадачиНаИсполнение,
		|	ПроектныеЗадачи.Владелец.АвтоматическиРассчитыватьПланПроекта КАК АвтоматическиРассчитыватьПланПроекта,
		|	ПроектныеЗадачи.Владелец.РедактируетсяВСтороннейСистеме КАК РедактируетсяВСтороннейСистеме,
		|	ПроектныеЗадачи.Владелец.СпособПланирования КАК СпособПланирования,
	...
	Для Каждого ПроектнаяЗадача Из ПроектныеЗадачи Цикл
		...
		Результат.Вставить("АвтоматическиОтправлятьПроектныеЗадачиНаИсполнение", Ложь);
...
// Возвращает проект письма.
//
// Параметры:
//   Письмо - ДокументСсылка.ВходящееПисьмо, ДокументСсылка.ИсходящееПисьмо - ссылка на письмо.
//
// Возвращаемое значение:
//   СправочникСсылка.Проект - проект письма.
//
Функция ПолучитьПроектПисьма(Письмо) Экспорт
	
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Письмо, "Проект");
...
Функция ИзменитьПапкуПроектов(МассивПроектов, НоваяПапка) Экспорт
	...
	Для Каждого ПроектСсылка Из МассивПроектов Цикл
		Если Не ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(ПроектСсылка).Изменение Тогда 
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'У вас нет права на изменение проекта ""%1"".'"),
				Строка(ПроектСсылка));
...
Функция ПолучитьПроектныеЗадачиПользователя(ДатаНачала, ДатаОкончания, Пользователь) Экспорт
	...
	|	СрокиПроектныхЗадач.ТекущийПланНачало КАК ТекущийПланНачало,
	|	СрокиПроектныхЗадач.ТекущийПланОкончание КАК ТекущийПланОкончание,
	|	ПроектныеЗадачи.Представление КАК Представление
	|ИЗ
	|	Справочник.ПроектныеЗадачи КАК ПроектныеЗадачи
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СрокиПроектныхЗадач КАК СрокиПроектныхЗадач
	|		ПО ПроектныеЗадачи.Ссылка = СрокиПроектныхЗадач.ПроектнаяЗадача
	...
	|ГДЕ
	|	ПроектныеЗадачи.Исполнители.Исполнитель В(&Исполнители)
	...
	Исполнители = РегистрыСведений.ИсполнителиЗадач.РолиИсполнителя(Пользователь);
...
// Проверяет уникальность документа в рамках проекта
//
// Параметры:
...
//   Булево
//
Функция ДокументУникален(ДокументОбъект) Экспорт
	
	Если Не ЗначениеЗаполнено(ДокументОбъект.Проект) Или Не ЗначениеЗаполнено(ДокументОбъект.ВидДокумента) Тогда
		Возврат Истина;
	КонецЕсли;
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	<<ИмяМетаданных>>.Ссылка <> &Ссылка
		|	И <<ИмяМетаданных>>.ВидДокумента = &ВидДокумента
		|	И <<ИмяМетаданных>>.Проект = &Проект
		|	И НЕ <<ИмяМетаданных>>.ПометкаУдаления";
	Запрос.Текст = СтрЗаменить(Запрос.Текст, "<<ИмяМетаданных>>", ДокументОбъект.Метаданные().Имя);
	Запрос.УстановитьПараметр("ВидДокумента", ДокументОбъект.ВидДокумента);
	Запрос.УстановитьПараметр("Проект", ДокументОбъект.Проект);
...
// Ищет и отправляет на исполнение следующую по плану проектную задачу.
//
// Параметры:
//   ДанныеПроектнойЗадачи - Структура - данные текущей проектной задачи.
//
Процедура ПродолжитьИсполнениеПроекта(ДанныеПроектнойЗадачи)
	...
	Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПроектныеЗадачиПредшественники.Ссылка КАК СледующаяЗадача,
		|	СрокиПроектныхЗадач.ТекущийПланНачало КАК СледующаяЗадачаПланНачало,
		|	СрокиПроектныхЗадач.ТекущийПланОкончание КАК СледующаяЗадачаПланОкончание
		|ПОМЕСТИТЬ СледующиеЗадачи
		|ИЗ
		|	Справочник.ПроектныеЗадачи.Предшественники КАК ПроектныеЗадачиПредшественники
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СрокиПроектныхЗадач КАК СрокиПроектныхЗадач
		|		ПО ПроектныеЗадачиПредшественники.Ссылка = СрокиПроектныхЗадач.ПроектнаяЗадача
		|ГДЕ
		|	ПроектныеЗадачиПредшественники.Предшественник = &ПредыдущаяЗадача
		|	И НЕ ПроектныеЗадачи.ПометкаУдаления
		|	И НЕ ПроектныеЗадачи.Веха
		|	И НЕ ПроектныеЗадачи.СуммарнаяЗадача
		|	И СрокиПроектныхЗадач.НачалоФакт = ДАТАВРЕМЯ(1, 1, 1)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СледующиеЗадачи.СледующаяЗадача КАК СледующаяЗадача,
		|	СледующиеЗадачи.СледующаяЗадачаПланНачало КАК СледующаяЗадачаПланНачало,
		|	СледующиеЗадачи.СледующаяЗадачаПланОкончание КАК СледующаяЗадачаПланОкончание
		|ПОМЕСТИТЬ СледующиеЗадачиГотовыеКСтарту
		|ИЗ
		|	СледующиеЗадачи КАК СледующиеЗадачи
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПроектныеЗадачи.Предшественники КАК ПроектныеЗадачиПредшественники
		|		ПО СледующиеЗадачи.СледующаяЗадача = ПроектныеЗадачиПредшественники.Ссылка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СрокиПроектныхЗадач КАК СрокиПроектныхЗадач
		|		ПО (ПроектныеЗадачиПредшественники.Предшественник = СрокиПроектныхЗадач.ПроектнаяЗадача)
		|
		|СГРУППИРОВАТЬ ПО
		|	СледующиеЗадачи.СледующаяЗадача,
		|	СледующиеЗадачи.СледующаяЗадачаПланОкончание,
		|	СледующиеЗадачи.СледующаяЗадачаПланНачало
		|
		|ИМЕЮЩИЕ
		|	МИНИМУМ(СрокиПроектныхЗадач.ОкончаниеФакт > ДАТАВРЕМЯ(1, 1, 1)) = ИСТИНА
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СледующиеЗадачиГотовыеКСтарту.СледующаяЗадача КАК СледующаяЗадача,
		|	СледующиеЗадачиГотовыеКСтарту.СледующаяЗадачаПланНачало КАК СледующаяЗадачаПланНачало,
		|	СледующиеЗадачиГотовыеКСтарту.СледующаяЗадачаПланОкончание КАК СледующаяЗадачаПланОкончание,
		|	ПроектныеЗадачиИсполнители.Исполнитель КАК Исполнитель,
		|	ПроектныеЗадачи.Наименование КАК Наименование,
		|	ПроектныеЗадачи.КодСДР КАК КодСДР
		|ИЗ
		|	СледующиеЗадачиГотовыеКСтарту КАК СледующиеЗадачиГотовыеКСтарту
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ПроектныеЗадачи КАК ПроектныеЗадачи
		|		ПО СледующиеЗадачиГотовыеКСтарту.СледующаяЗадача = ПроектныеЗадачи.Ссылка
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ПроектныеЗадачи.Исполнители КАК ПроектныеЗадачиИсполнители
		|		ПО СледующиеЗадачиГотовыеКСтарту.СледующаяЗадача = ПроектныеЗадачиИсполнители.Ссылка
		|ИТОГИ
		|	МАКСИМУМ(СледующаяЗадачаПланНачало),
		|	МАКСИМУМ(СледующаяЗадачаПланОкончание),
	...
	Запрос.УстановитьПараметр("Проект", ДанныеПроектнойЗадачи.Проект);
	Запрос.УстановитьПараметр("ПредыдущаяЗадача", ДанныеПроектнойЗадачи.Ссылка);
	
	МассивРезультатов = Запрос.ВыполнитьПакет();
	ВГраница = МассивРезультатов.ВГраница();
	
	ДанныеПроекта = МассивРезультатов[ВГраница].Выбрать();
	Если Не ДанныеПроекта.Следующий() Тогда
		Возврат;
	КонецЕсли;
	
	ВыборкаСледующаяЗадача = МассивРезультатов[ВГраница - 1].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
	
	Пока ВыборкаСледующаяЗадача.Следующий() Цикл
		Если ВыборкаСледующаяЗадача.Исполнитель = 0 Тогда
			Если Не ЗначениеЗаполнено(ДанныеПроекта.Руководитель) Тогда
				Продолжить;
			КонецЕсли;
			РегистрыСведений.ОчередьУведомлений.ДобавитьУведомлениеПоСобытию(
				ДанныеПроекта.Руководитель,
				Перечисления.СобытияУведомлений.НеУказанИсполнительПроектнойЗадачи,
				ВыборкаСледующаяЗадача.СледующаяЗадача,
				ДанныеПроектнойЗадачи.Проект,
				СтрШаблон(НСтр("ru = 'Для продолжения автоматического выполнения плана проекта нужно указать исполнителей, и отправить задачу на исполнение вручную.
					|<b>Проектная задача:</b> <a href=""%1"">%2 %3</a>
					|<b>Плановая дата начала:</b> %4
					|<b>Плановая дата окончания:</b> %5
					|<b>Проект:</b> <a href=""%6"">%7</a>
					|<b>Руководитель проекта:</b> %8'"),
					ПолучитьНавигационнуюСсылку(ВыборкаСледующаяЗадача.СледующаяЗадача),
					ВыборкаСледующаяЗадача.КодСДР,
					ВыборкаСледующаяЗадача.Наименование,
					Формат(ВыборкаСледующаяЗадача.СледующаяЗадачаПланНачало, "ДЛФ=DDT"),
					Формат(ВыборкаСледующаяЗадача.СледующаяЗадачаПланОкончание, "ДЛФ=DDT"),
					ПолучитьНавигационнуюСсылку(ДанныеПроектнойЗадачи.Проект),
					ДанныеПроектнойЗадачи.Проект,
					ДанныеПроекта.Руководитель),
				Истина,
				Перечисления.СобытияУведомлений.НеУказанИсполнительПроектнойЗадачи);
		Иначе
			...
			БПОбъект.Заполнить(Новый Структура("ПроектнаяЗадача", ВыборкаСледующаяЗадача.СледующаяЗадача));
			БПОбъект.Записать();
			Если ВыборкаСледующаяЗадача.СледующаяЗадачаПланНачало <= ТекущаяДатаСеанса() Тогда
				...
				КонецЕсли;
			Иначе
				РегистрыСведений.ПроцессыДляЗапуска.ДобавитьПроцессДляОтложенногоСтарта(БПОбъект.Ссылка,
					ВыборкаСледующаяЗадача.СледующаяЗадачаПланНачало);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
