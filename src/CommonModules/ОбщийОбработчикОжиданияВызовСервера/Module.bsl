...
Функция ВыполнитьДействияОбработчикаОжидания(Обработчики) Экспорт
	...
	Результат.Вставить("РезультатыЗанятыеФайлы", Неопределено);
	
	РаботаСУведомлениями.ВыполнитьДействиеОбщегоОбработчикаОжидания(Обработчики, Результат);
	...
	Если Обработчики.Свойство("АвтопомещениеФайлов")
		И Обработчики.АвтопомещениеФайлов Тогда
		
		Результат.РезультатыЗанятыеФайлы 
			= РаботаСФайламиВызовСервера.ПроверитьЗанятыеФайлыОтложенно();
