...
//  ПризнакЗагрузкиДанных - Булево, УникальныйИдентификатор - если ИСТИНА, то в случае,
//                          если данных нет или они просрочены, то запустить фоновое задание
//                          загрузки данных. Если передан УникальныйИдентификатор,
...
//  КонтрагентОбъект - Объект, Неопределено - заполняется в том случае, если форма - это форма элемента
//                     справочника, а не форма документа.
//  Контрагент - ОпределяемыйТип.КонтрагентБИП, Строка - Контрагент или ИНН контрагента;
//  ВидКонтрагента - ПеречислениеСсылка.ВидыКонтрагентовСПАРКРиски - вид проверки данных контрагента;
...
//  ПризнакЗагрузкиДанных - Булево, УникальныйИдентификатор - если ИСТИНА, то в случае, 
//                          если данных нет или они просрочены, то запустить фоновое задание загрузки данных.
//                          Если передан УникальныйИдентификатор, то это идентификатор фонового задания.
//
...
Процедура ПриОткрытии(Форма, КонтрагентОбъект = Неопределено) Экспорт
	...
	// Использование Попытка/Исключение временное решение, при переходе
	// на новую редакцию Библиотеки Интернет-поддержки, будет добавлено
	// обязательное требование по размещению реквизита НастройкиСПАРКРиски
	// на формах, которые использую функциональность индексов СПАРК.
...
Процедура ПерейтиВВебВерсиюСПАРК(
	...
	Если Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ВнутренняяОшибкаСервиса") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<body><b>Не удалось подключиться к сервису 1СПАРК Риски </b>
				|<br/>
				|<br/>Не удалось подключиться к сервису 1СПАРК Риски. Сервис временно недоступен. Повторите попытку подключения позже.'"));
		
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИспользованиеЗапрещено") Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<body><b>Переход в веб-версию СПАРК недоступен </b>
				|<br/>
				|<br/>Использование сервиса 1СПАРК Риски недоступно в текущем режиме работы.'"));
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ОшибкаПодключения") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			...
			|• Брандмауэр или другое промежуточное ПО (антивирусы и т.п.) блокируют попытки программы подключиться к Интернету;<br/>
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.ИнтернетПоддержкаНеПодключена")
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<body><b>Переход в веб-версию СПАРК недоступен </b>
				|<br/>
				|<br/>Для работы с сервисом 1СПАРК Риски необходимо подключить Интернет-поддержку пользователей. Обратитесь к администратору.'"));
		...
		Иначе
			...
			СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = '<body><b>Переход в веб-версию СПАРК недоступен </b>
						|<br/>
						|<br/>Недостаточно лимита для выполнения перехода.
	...
	ИначеЕсли Результат.ВидОшибки = ПредопределенноеЗначение("Перечисление.ВидыОшибокСПАРКРиски.НеПодлежитПроверке") Тогда
		...
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
			НСтр("ru = '<body><b>Переход в веб-версию СПАРК недоступен </b>
				|<br/>
				|<br/>Контрагент не подлежит проверке, переход в веб-версию невозможен.'"));
		
	ИначеЕсли ЗначениеЗаполнено(Результат.ВидОшибки) Тогда
		
		СообщениеОбОшибке = СтроковыеФункцииКлиент.ФорматированнаяСтрока(
				НСтр("ru = '<body><b>Не удалось перейти в веб-версию СПАРК </b>
					|<br/>
					|<br/>При получении ссылки для перехода возникли ошибки. Обратитесь к администратору.'"));
