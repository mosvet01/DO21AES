	...
	ИначеЕсли Команда.Имя = "ЭДОСформироватьПроизвольныйДокумент" Тогда
		...
		Иначе
			...
			Для Каждого НастройкаПечатнойФормы Из НастройкиПечатныхФорм Цикл
				...
				ДвоичныеДанныеФайла = ИнтерфейсДокументовЭДОКлиентСервер.ДанныеТабличногоДокумента(
					ТабличныйДокумент, ТипФайлаТабличногоДокумента.PDF);
				
				ПараметрыДокумента.Вставить("ДвоичныеДанныеФайла", ДвоичныеДанныеФайла);
...
Процедура ОформитьГруппуСостоянияИСтатусыМЧД(Форма, Подписана, Верна, Отозвана, ДатаОтзыва, СтатусВРеестреФНС) Экспорт
	...
	Если Отозвана И (ДатаОтзыва > ОбщегоНазначенияКлиент.ДатаСеанса()) Тогда
		ТекстЗаголовка = НСтр("ru = 'Будет отозвана %1'");
		Элементы.ДекорацияПометкаОтозвана.Заголовок = СтрШаблон(ТекстЗаголовка, Формат(ДатаОтзыва, "ДЛФ=Д"));
