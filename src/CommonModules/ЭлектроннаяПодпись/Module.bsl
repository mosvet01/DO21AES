...
Процедура ДобавитьПодпись(Объект, Знач СвойстваПодписи, ИдентификаторФормы = Неопределено,
	...
	Попытка
		...
		Иначе
			ОбъектДанных = Объект;
		КонецЕсли;
		ОбъектДанных.ДополнительныеСвойства.Вставить(
...
Процедура ОбновитьПодпись(Объект, Знач СвойстваПодписи, ОбновитьПоПорядковомуНомеру = Ложь) Экспорт
	...
	Попытка
		...
		Для Каждого ПодписьОбъекта Из ПодписиОбъекта Цикл
			...
			Если ОбновитьПоПорядковомуНомеру И ПодписьОбъекта.УникальныйИдентификатор = СвойстваПодписи.УникальныйИдентификатор 
				...
				ОбновляемаяПодпись.ДатаПодписи = ПодписьОбъекта.ДатаПодписи;
				ОбновляемаяПодпись.УстановившийПодпись = ПодписьОбъекта.УстановившийПодпись;
				...
				Если ОбновитьПоПорядковомуНомеру Тогда
					Для Каждого КлючИЗначение Из СвойстваПодписи Цикл
						Если КлючИЗначение.Ключ = "Сертификат"
							...
							Или КлючИЗначение.Ключ = "УникальныйИдентификатор" Тогда
							Продолжить;
						КонецЕсли;
...
//   * СвойстваПодписи - см. ЭлектроннаяПодписьКлиентСервер.НовыеСвойстваПодписи
//   
Функция УсовершенствоватьПодписьОбъекта(ПодписанныйОбъект, УникальныйИдентификатор, ТипПодписи, ДобавитьАрхивнуюМеткуВремени = Ложь,
			ИдентификаторФормы = Неопределено, МенеджерКриптографии = Неопределено) Экспорт
			
	Результат = Новый Структура("Успех, ТекстОшибки, СвойстваПодписи", Ложь);
		
	СвойстваПодписи = УстановленныеПодписи(ПодписанныйОбъект, УникальныйИдентификатор)[0];
	...
	Результат.СвойстваПодписи.УникальныйИдентификатор = УникальныйИдентификатор; 
...
Функция ПерсональныеНастройки() Экспорт
	...
	ПерсональныеНастройки.Вставить("СохранятьДоверенность", Ложь);
...
Процедура ДобавитьСтрокуПодписи(ОбъектДанных, СвойстваПодписи, СообщениеЖурналаРегистрации)
	...
	НоваяЗапись.УникальныйИдентификатор = ИдентификаторПодписи;
