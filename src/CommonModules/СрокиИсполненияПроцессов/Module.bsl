...
//
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма изменения параметров процесса.
//
...
Функция ПодсказкаВводаПоляСрокИсполнения(Форма) Экспорт
	...
	Если Форма.ИспользоватьДатуИВремяВСрокахЗадач Тогда
		Результат = НСтр("ru = 'дата, дни, часы, минуты'");
	Иначе
		Результат = НСтр("ru = 'дата, дни'");
	КонецЕсли;
	
	Если Не ЗначениеЗаполнено(Форма.ДатаОтсчетаДляРасчетаСроков) Тогда
		
		Результат = СокрЛП(СтрЗаменить(Результат, НСтр("ru = 'дата,'"), ""));
