...
// Помещает в историю закрытые визы исполнителей, которые были сформированы
// процессами согласования.
//
// Параметры:
//  Документ - СправочникСсылка.ИсходящиеДокументы,
//             СправочникСсылка.ВнутренниеДокументы
//  Исполнители - ДанныеФормыКоллекция - таблица исполнителей процесса согласования.
//
Процедура ПоместитьВИсториюЗакрытыеВизыИсполнителейПоДокументу(Документ, Исполнители) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	АктивныеВизы = ПолучитьАктивныеВизыДокумента(Документ);
	
	ИсточникиВиз = Новый Массив;
	Для Каждого АктивнаяВиза Из АктивныеВизы Цикл
		Если ЗначениеЗаполнено(АктивнаяВиза.Источник)
			И ТипЗнч(АктивнаяВиза.Источник) = Тип("БизнесПроцессСсылка.Согласование") Тогда
		
			ИсточникиВиз.Добавить(АктивнаяВиза.Источник);
		КонецЕсли;
	КонецЦикла;
	ЗавершенностьИсточников = ОбщегоНазначения.ЗначенияРеквизитовОбъектов(
		ИсточникиВиз, "Завершен");
		
	КэшИсполнителей = Новый Соответствие();
	Для Каждого СтрокаТаблицы Из Исполнители Цикл
		КэшИсполнителей[СтрокаТаблицы.Исполнитель] = Истина;
	КонецЦикла;
		
	Визы = Новый Массив;
	Для Каждого АктивнаяВиза Из АктивныеВизы Цикл
		
		Если Не ЗначениеЗаполнено(АктивнаяВиза.РезультатСогласования)
			И ЗавершенностьИсточников[АктивнаяВиза.Источник] <> Истина Тогда
			
			Продолжить;
		КонецЕсли;
		
		ИсполнительВизы = АктивнаяВиза.Исполнитель;
		Если ЗначениеЗаполнено(АктивнаяВиза.РольИсполнителя) Тогда
			ИсполнительВизы = АктивнаяВиза.РольИсполнителя;
		КонецЕсли;
		Если КэшИсполнителей[ИсполнительВизы] = Неопределено Тогда		
			Продолжить;
		КонецЕсли;
		
		Визы.Добавить(АктивнаяВиза.Ссылка);
	КонецЦикла;	
...
Функция ПечатьЛистаСогласованияИзДокумента(МассивОбъектов, ОбъектыПечати, История = Ложь, КодЯзыка = Неопределено) Экспорт 
	...
	Если КодЯзыка = Неопределено Тогда
		КодЯзыка = МультиязычностьСервер.КодОсновногоЯзыка();
	КонецЕсли;
	
	Если История = Истина Тогда 
		Макет = УправлениеПечатью.МакетПечатнойФормы("БизнесПроцесс.Согласование.ПФ_MXL_ЛистСогласованияСИсторией", КодЯзыка);
	Иначе 
		Макет = УправлениеПечатью.МакетПечатнойФормы("БизнесПроцесс.Согласование.ПФ_MXL_ЛистСогласования", КодЯзыка);
	КонецЕсли;
