...
Процедура ОткрытьФайлПриложением(ДанныеФайла, ИмяОткрываемогоФайла, УникальныйИдентификатор = Неопределено)
	...
	Если НРег(ДанныеФайла.Расширение) = НРег("xml") Тогда
		...
		ПараметрыФормыПросмотраЭД.Вставить("ДокументДО", ДанныеФайла.Владелец);
		ПараметрыФормыПросмотраЭД.Вставить("ФайлДО", ДанныеФайла.Ссылка);
		ПараметрыФормыПросмотраЭД.Вставить("ВерсияФайлаДО", ДанныеФайла.Версия);
		ПараметрыФормыПросмотраЭД.Вставить("ИдентификаторФормыВладельца", УникальныйИдентификатор);
		
		ОткрытьФорму("Обработка.ИнтерфейсДокументовЭДОДокументооборот.Форма.ПросмотрФайлаПакетаЭДО",
...
//
// Параметры
//  ДанныеФайлов  - Массив из Структура -  с данными файла
//  УникальныйИдентификатор - уникальный идентификатор формы
//
...
Функция ДобавитьИзФайловойСистемыСРасширениемСинхронно(ПараметрыВыполнения) Экспорт
	...
	Если ЭтоДобавлениеОригинала Тогда
		...
		ЭтоРасширениеСканКопии = ДелопроизводствоКлиентСервер.ЭтоРасширениеСканКопии(
			ФайлКлиента.Расширение, КоличествоРасширенийСканКопий);
...
Процедура ДобавитьИзФайловойСистемыБезРасширенияПослеЗагрузкиФайла(Помещен, Адрес, ВыбранноеИмяФайла, ПараметрыВыполнения) Экспорт
	...
	Попытка
		...
		СведенияОФайле.ВремяИзменения = ТекущаяДата();
		СведенияОФайле.ВремяИзмененияУниверсальное = УниверсальноеВремя(ТекущаяДата());
