...
Процедура ПриОпределенииОбщихПараметровБазовойФункциональности(ОбщиеПараметры) Экспорт
	...
	Если ЭтоВерсияЗащищенногоПрограммногоКомплекса(СистемнаяИнформация.ВерсияПриложения) Тогда
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = СистемнаяИнформация.ВерсияПриложения;
	Иначе
		ОбщиеПараметры.МинимальнаяВерсияПлатформы = "8.3.17.2760; 8.3.18.1959; 8.3.19.1726; 8.3.20.2184; 8.3.21.1624; 8.3.22.1709";
	КонецЕсли;
	...
	Если СВЗарегистрирована Тогда 
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				ОбсужденияДокументооборот.СобытиеЖурналаРегистрации(),
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				...
				УровеньЖурналаРегистрации.Ошибка,,,
				ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			Параметры.Вставить("ИдентификаторыСВКаналовПользователяСтроками", Новый Массив);
		КонецПопытки;
