...
Процедура ПриПолученииОписанияВариантаНастройки(ОписаниеВарианта, ИдентификаторНастройки, ПараметрыКонтекста) Экспорт
	...
	ИдентификаторНастройки = "ОбменУХДО20";
	
	ОписаниеВарианта.ИмяКонфигурацииКорреспондента = "УправлениеХолдингом";
	Синоним = НСтр("ru = '1С:Управление холдингом, ред. 3'");
	ОписаниеВарианта.ИмяФайлаНастроекДляПриемника = "Настройки_обмена_ДОКОРП_УХ";
	
	ОписаниеВарианта.КраткаяИнформацияПоОбмену =
		НСтр("ru = 'Позволяет синхронизировать данные между конфигурациями ""1С:Управление холдингом"" и ""1С:Документооборот"".
	...
	Иначе
		ОписаниеВарианта.ПодробнаяИнформацияПоОбмену = "ПланОбмена.ОбменУправлениеХолдингомДокументооборот20.Форма.ПодробнаяИнформация";
	КонецЕсли;
	...
	ОписаниеВарианта.Отборы.Вставить("ВыгружатьЗаявкиНаОперации");
	ОписаниеВарианта.Отборы.Вставить("ЗаявкиНаОперации",
		Новый Структура("ВидДокумента, ТипОперации"));
	
	ОписаниеВарианта.Отборы.Вставить("ОграничиватьВыгрузку");
	ОписаниеВарианта.Отборы.Вставить("Стороны", Новый Структура("Сторона"));
	
	ОписаниеВарианта.Отборы.Вставить("ВыгружатьПроектыИПроектныеЗадачи");
...
Функция ОписаниеОграниченийПередачиДанных(Отборы, ВерсияКорреспондента, ИдентификаторНастройки) Экспорт
	...
	// Заявки на операции.
	Если Отборы.ВыгружатьЗаявкиНаОперации Тогда
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Заявки на расходование денежных средств выгружаются'");
		ВидыДокументов = Отборы.ЗаявкиНаОперации.ВидДокумента;
		Если ВидыДокументов.Количество() > 0 Тогда
			ТекстОписания = ТекстОписания + " " + НСтр("ru = 'по видам'") + ": ";
			Разделитель = "";
			Для каждого ВидДокумента из ВидыДокументов Цикл
		...
		ТекстОписания = ТекстОписания + ".";
	Иначе
		ТекстОписания = ТекстОписания + Символы.ПС + НСтр("ru = 'Заявки на расходование денежных средств не выгружаются.'");
	КонецЕсли;
	
	Если Отборы.ВыгружатьПроектыИПроектныеЗадачи Тогда
