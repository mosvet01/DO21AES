...
Функция ПрочитатьОбъектДанныхИнформационнойБазы() Экспорт
	...
	Если ПотокЧтения.ТипУзла = ТипУзлаXML.НачалоЭлемента Тогда
		...
		Если ПотокЧтения.Имя = "Artefacts" Тогда
			...
			Пока ПотокЧтения.ТипУзла <> ТипУзлаXML.КонецЭлемента Цикл
				...
				Исключение
					...
					ОшибкиАртефактов.Добавить(СтрШаблон("%1 (%2):
						|%3",
						ИмяЭлемента, ТипАртефакта,
		...
		////
		Если ЗначениеЗаполнено(ОшибкиАртефактов) Тогда
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'Выгрузка загрузка данных. Загрузка объекта с ошибкой'"),
				УровеньЖурналаРегистрации.Ошибка,,
				Строка(ТекущийОбъект),
				СтрСоединить(ОшибкиАртефактов, "
				|-------------------------------------------------------"));
			ОшибкиАртефактов.Очистить();
		КонецЕсли;
		////
