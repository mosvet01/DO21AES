...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемыДокументооборота.МЭДО") Тогда
		РеквизитыМЭДО = Новый Массив();
		РеквизитИспользоватьМЭДО = Новый РеквизитФормы(
			"ИспользоватьМЭДО", Новый ОписаниеТипов("Булево"), "", "МЭДО", Истина);
		РеквизитыМЭДО.Добавить(РеквизитИспользоватьМЭДО);
		ИзменитьРеквизиты(РеквизитыМЭДО);
		Поле = Элементы.Вставить("ИспользоватьМЭДО", Тип("ПолеФормы"), Элементы.ГруппаПравая, Элементы.ГруппаВалюта);
		Поле.Вид = ВидПоляФормы.ПолеФлажка;
		Поле.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Право;
		Поле.Подсказка = НСтр(
			"ru = 'При включении опции ""МЭДО"" (межведомственный электронный документооборот), 
			|одновременно включатся опции, необходимые для ее корректной работы:
			|- Грифы доступа
			|- Использовать синхронизацию с интегрированными системами
			|- Электронные подписи
			|- Вставлять штамп ЭП (автовизуализация ЭП)'");
		Поле.ОтображениеПодсказки = ОтображениеПодсказки.Кнопка;
		Поле.ПутьКДанным = "ИспользоватьМЭДО";
		ЭтотОбъект.ИспользоватьМЭДО = Константы.ИспользоватьМЭДО.Получить();
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ОбщегоНазначенияКлиент.ПодсистемаСуществует("СтандартныеПодсистемыДокументооборота.МЭДО") Тогда
		Если ЭтотОбъект.ИспользоватьМЭДО Тогда 
			Опции = ВыключенныеОпцииОтКоторыхЗависитМЭДО();
			Если Опции <> "" Тогда
				ТекстПредупреждения = СтрШаблон(
					НСтр("ru = 'Одновремено с опцией ""МЭДО"" (межведомственный электронный документооборот) автоматически
					|включаются другие (другая) необходимые для МЭДО опции:
					|%1'"),
					Опции);
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемыДокументооборота.МЭДО") Тогда
		...
		Константы.ИспользоватьМЭДО.Установить(ЭтотОбъект.ИспользоватьМЭДО);
		Если ЭтотОбъект.ИспользоватьМЭДО Тогда
