...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого ЭлементОтбора Из Отборы Цикл
		Для Каждого ЭлементКоллекции Из БыстрыеОтборы Цикл
			Если ЭлементОтбора.Параметр = ЭлементКоллекции.Параметр
					И ЭлементОтбора.Тип = ЭлементКоллекции.Тип Тогда
...
Процедура ВходящиеЭДПриПолученииДанныхНаСервере(ИмяЭлемента, Настройки,
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ИнтеграцияЭДО.ТекстЗапросаОбъектовУчетаЭлектронныхДокументов(Истина);
...
#Область ОбработчикиСобытийЭлементовТаблицыФормыОбластьПросмотра
...
&НаСервереБезКонтекста
Функция ПредставлениеОтраженияВУчете(Знач ТекущийЭлектронныйДокумент, ТекущийДокументУчета, 
	Знач ДанныеВходящегоЭлектронногоДокумента)
	...
	Если ЗначениеЗаполнено(ОснованияЭД) Тогда
		...
		Если КоличествоОснований = 1 Тогда
			Представление = Строка(ОснованияЭД[0].ОбъектУчета);
			ТекущийДокументУчета = ОснованияЭД[0].ОбъектУчета;
		Иначе
			ШаблонСтроки = НСтр("ru = ';%1 документ;;%1 документа;%1 документов;%1 документов'");
			Представление = СтрокаСЧислом(ШаблонСтроки, КоличествоОснований, ВидЧисловогоЗначения.Количественное);
		КонецЕсли;
		...
		Если ТипЗнч(ТекущийЭлектронныйДокумент) = Тип("ДокументСсылка.ЭлектронныйДокументВходящийЭДО") Тогда
			...
			ИначеЕсли ДанныеВходящегоЭлектронногоДокумента.КонтрольОтраженияВУчете = 1 Тогда
				Если ДанныеВходящегоЭлектронногоДокумента.ТипДокумента = Перечисления.ТипыДокументовЭДО.КонтрактЕИС Тогда
					Представление = НСтр("ru = 'Создать контракт'");
				Иначе
					Представление = НСтр("ru = 'Создать документы'");
				КонецЕсли;
			ИначеЕсли ДанныеВходящегоЭлектронногоДокумента.КонтрольОтраженияВУчете = 2 Тогда
				Представление = НСтр("ru = 'Провести документы'");
			Иначе
...
Процедура ОбновитьОбластьПросмотра()
	...
	ДанныеПросмотра = ДанныеОбластиПросмотра(ТекущийЭлектронныйДокумент, УникальныйИдентификатор, 
		ФормированиеПредставления, ЭлементСхемыРегламента, Статус, ПрочтениеДолжноБытьУстановлено,
		ЕстьОтборПоНепрочтенным);
	
	ТекущийДокументТабличныйДокумент = ДанныеПросмотра.ПредставлениеДокумента;
	
	ДанныеВходящегоЭлектронногоДокумента = Элементы.ВходящиеЭД.ТекущиеДанные;
	ТекущийДокументУчетаПредставление = ПредставлениеОтраженияВУчете(ТекущийЭлектронныйДокумент, ТекущийДокументУчета,
		ДанныеВходящегоЭлектронногоДокумента);
	...
	Элементы.ГруппаФайл.Видимость = ЗначениеЗаполнено(ДанныеПросмотра.ФайлСсылка);
	Элементы.ТекущийДокументУчетаПредставление.Видимость = ЗначениеЗаполнено(ТекущийДокументУчетаПредставление);
	
	Если Элементы.ТекущийДокументУчетаПредставление.Видимость Тогда
		Элементы.ТекущееСостояниеЭДОУчет.Видимость = Истина;
		Элементы.ТекущееСостояниеЭДОФайл.Видимость = Ложь;
	ИначеЕсли Элементы.ГруппаФайл.Видимость И Не Элементы.ТекущийДокументУчетаПредставление.Видимость Тогда
		Элементы.ТекущееСостояниеЭДОУчет.Видимость = Ложь;
		Элементы.ТекущееСостояниеЭДОФайл.Видимость = Истина;
	ИначеЕсли Не Элементы.ГруппаФайл.Видимость И Не Элементы.ТекущийДокументУчетаПредставление.Видимость Тогда
		Элементы.ТекущееСостояниеЭДОУчет.Видимость = Ложь;
		Элементы.ТекущееСостояниеЭДОФайл.Видимость = Ложь;
	КонецЕсли;
	...
	ПолучитьСостояниеДокументаЭДО(ТекущийЭлектронныйДокумент, ТекущееСостояниеЭДО);
...
&НаСервереБезКонтекста
Функция ДанныеОбластиПросмотра(ТекущийЭлектронныйДокумент, УникальныйИдентификатор, 
	ФормированиеПредставления, ЭлементСхемыРегламента, Статус, ПрочтениеДолжноБытьУстановлено,
	ЕстьОтборПоНепрочтенным)
	
	СтруктураВозврат = Новый Структура(
		"ДокументУчетаСсылка, ДокументУчетаПредставление, 
		|ФайлСсылка, ФайлНомерИконки, ФайлПредставление,
		|ПредставлениеДокумента");
	
	Основания = ИнтеграцияЭДО.ОбъектыУчетаЭлектронныхДокументов(ТекущийЭлектронныйДокумент);
	Если Основания.Количество() > 0 Тогда
		
		СтруктураВозврат.ДокументУчетаСсылка = Основания[0].ОбъектУчета;
		СтруктураВозврат.ДокументУчетаПредставление = Строка(Основания[0].ОбъектУчета);
		Если Не ЗначениеЗаполнено(Основания[0].ОбъектУчета) Тогда
			СтруктураВозврат.ДокументУчетаПредставление = НСтр("ru = 'Нет'");
		КонецЕсли;	
	...
	ДанныеЭлектронногоДокумента = ИнтерфейсДокументовЭДО.ДанныеФормыПросмотраЭлектронногоДокумента(
		ТекущийЭлектронныйДокумент, ЭтоВходящийЭДО);
		
	Если ДанныеЭлектронногоДокумента.ДанныеЭлементовСхемы.Количество() = 0 Тогда
		Возврат СтруктураВозврат;
	КонецЕсли;
	
	ЭлементСхемыРегламента = Новый Структура("АдресОписанияСообщения, ВидДокумента, ДатаИзмененияСтатуса,
	|ДополнительнаяИнформация, Доступность, Наименование, Направление, ПрисоединенныйФайл, 
	|Сообщение, Статус, ТипДокумента, ТипЭлементаРегламента"); 
	...
	Если ОбщегоНазначения.ПодсистемаСуществует("ЭлектронноеВзаимодействие.ОбменСГИСЭПД") Тогда
		...
		МодульОбменСГИСЭПД.ПриОпределенииИнформацииОтправителя(ТекущийЭлектронныйДокумент.ТипРегламента,
								ДанныеЭлектронногоДокумента.ДанныеЭлементовСхемы,
	...
	Если Не ДокументРаспознан
		...
		СтруктураВозврат.ФайлНомерИконки = 
			ИнтеграцияБСПБЭД.ИндексПиктограммыФайла(Расширение);
		...
		ПараметрыДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ТекущийЭлектронныйДокумент, 
		...
		ОбщегоНазначенияБЭД.ВывестиОбластьВТабличныйДокумент(ПредставлениеПроизвольногоДокумента, ОбластьШапка, 
			"ОбластьПараметры");
		СтруктураВозврат.ПредставлениеДокумента = ПредставлениеПроизвольногоДокумента;
		
	Иначе
		
		Если Основания.Количество() = 0 Тогда
			СтруктураВозврат.ДокументУчетаПредставление = НСтр("ru = 'Не указан'");
		КонецЕсли;	
...
&НаСервереБезКонтекста
Процедура ПолучитьСостояниеДокументаЭДО(ТекущийЭлектронныйДокумент, ТекущееСостояниеЭДО)
	...
	ТекущееСостояниеЭДО = ПредставлениеСостояния;
...
Процедура ВыполнитьСформироватьПодписатьИОтправить()
	...
	НаборОбъектовУчета = МассивОбъектовУчета(Элементы.СформироватьЭД.ВыделенныеСтроки);
	ДополнительныеПараметры = Новый Структура;
...
Процедура СоздатьОтборПоРазделу(Знач Раздел, Знач НастройкиОтображения,
	...
	ИначеЕсли Раздел = "Распаковать" Тогда
		...
		Значение = Новый Массив;
		Значение.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыТранспортныхСообщенийБЭД.Неизвестный"));
		Значение.Добавить(ПредопределенноеЗначение("Перечисление.СтатусыТранспортныхСообщенийБЭД.КРаспаковке"));
...
Процедура ПоказатьКомандыРаздела(Знач Раздел)
	...
	КомандыРазделов.Вставить("Утвердить", "Обновить, ЗакрытьПринудительно, Перенаправить, УтвердитьЭД, ОтклонитьДокумент, ВходящиеЭДКонтекстноеМенюУтвердитьЭД, КомандыВходящиеЭД");
	КомандыРазделов.Вставить("Подписать", "Обновить, ЗакрытьПринудительно, Перенаправить, ПодписатьВх, ОтклонитьДокумент, КомандыВходящиеЭД");
	...
	КомандыРазделов.Вставить("Исходящие", "Обновить, ЗакрытьПринудительно, СоздатьПроизвольныйДокумент, Перенаправить, УтвердитьПодписатьИОтправитьИсх, КомандыИсходящиеЭД");
...
&НаСервереБезКонтекста
Функция МассивОбъектовУчета(Знач МассивКлючей)
...
Процедура ВыполнитьОтправкуПриглашенийВАрхив()
	...
	Если ВыбранныеИдентификаторы.Количество() > 0 Тогда
		...
		Счетчик = ОтправитьПриглашенияВАрхив(ВыбранныеИдентификаторы);

		Если Счетчик > 0 Тогда
			Строка = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(НСтр("ru = ';%1 приглашение;;%1 приглашения;%1 приглашений;%1 приглашения'"), Счетчик);
...
Процедура УстановитьВидимостьДоступностьПоПереходуНаНовуюВерсию()
	...
	Элементы.КомандаСоздатьПроизвольныйДокумент.Видимость = Ложь;
