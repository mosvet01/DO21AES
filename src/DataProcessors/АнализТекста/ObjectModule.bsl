...
// Разерешает неоднозначность аннотаций времени и даты.
//
// Параметры:
...
//
// Параметры:
//  АгрегартнаяАннотация	 - Структура	 - Агрегатная аннотация.
...
//
Функция УточнитьАгрегатнуюАннотацию(АгрегартнаяАннотация, УточняющиеПредложения, Текст)
	
	НоваяАгрегатнаяАннотация = АгрегартнаяАннотация;
	Если АгрегартнаяАннотация.Аннотации["Время"] = Неопределено Тогда
...
Функция АннотацияДатаНачала(Аннотация, ДатаТекста)
	...
	// Порядок следования условия определяет приоритет распознования даты.
	...
	ИначеЕсли АннотацияСоответствуетШаблону(Аннотация, ШаблонДата19(), Словари) Тогда
		...
		ДатаДень = Токены[0].Лемма;
		ДатаМесяц = Токены[2].Лемма;
		ДатаГод = НормализованыйГод(ДатаДень, ДатаМесяц, ДатаТекста);
	...
	ИначеЕсли АннотацияСоответствуетШаблону(Аннотация, ШаблонДата20(), Словари) Тогда
		// Пример: "двадцать первое октярбря"
		Токены = СловарьПолучитьТокены(Словари[0]);
...
Функция АннотацияВремяНачала(Аннотация, ДатаТекста)
	...
	Словари = Неопределено;
	// Порядок следования условия определяет приоритет распознования времени.
...
//
Функция НормализованыйГод(Знач ДатаДень, ДатаМесяц, ДатаТекста)
...
Функция ДниПрилагательное4()
	...
	ДниПрилагательное4.Добавить("чертвертым");
...
// Формитует список аббревиатур с точкой на конце.
//
// Возвращаемое значение:
...
// Формитует список символов-индикаторов конца.
//
// Возвращаемое значение:
...
// Формитует список символов, которые могут сопутствовать символам-индикаторам конца предложения.
//
// Возвращаемое значение:
...
// Фомирует список пунктуационных знаков.
//
// Возвращаемое значение:
...
// Фомирует список символов, не являющихся токенами.
//
// Возвращаемое значение:
...
// Аннотация представляет собой часть текста, описанную начальным индексом, конечным ндексом и именем.
//
// Возвращаемое значение:
...
// Агретная аннотация представляет собой совокупность вспомогательных аннотаций.
//
// Возвращаемое значение:
...
//
Функция ПолучитьТокены(Текст) Экспорт
...
//                    unknown - это любая смешнная последовательность из букв и цифр,
//                    находящаяся между двумя проблеами, либо между пробелом и символом пунктуации.
...
// Ищет все токены в переданом тексте.
//
// Параметры:
