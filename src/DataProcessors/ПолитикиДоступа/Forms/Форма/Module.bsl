...
Процедура РазрешенияОбщиеПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	...
	Если Элементы.ЗначенияДоступаОбщие.ТекущаяСтрока = Неопределено Тогда
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбран разрез доступа'"));
		Возврат;
	КонецЕсли;
		...
		Если Не Результат.Пустой() Тогда
			...
			Пока Выборка.Следующий() Цикл
				Если Выборка.Добавить Тогда
					...
					Запись.Записать();
				Иначе
					НаборЗаписей = РегистрыСведений.РазрешенияДоступаИсключительные.СоздатьНаборЗаписей();
...
Процедура ЗаполнитьРазрешенияПоУмолчанию(Команда)
	...
	ТекстВопроса = НСтр("ru = 'Разрешения для текущего разреза будут удалены и заполнены по умолчанию. Продолжить?'");
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНетОтмена);
