...
Функция ТекстПредупрежденияОПроверке()
	...
	Текст = НСтр("ru = 'Сейчас начнется проверка отчетов на специальном сервисе в Интернете.
	      |
          |Если отчеты содержат персональные сведения физ. лиц, убедитесь, что получено соглашение на передачу этих данных.'");
...
Процедура ВыполнитьПредварительнуюПроверкуНаСервере(ВходящийКонтекст)
	...
	Для каждого ОрганизацияОрганы Из ОрганизацииОрганы Цикл
		...
		Для каждого ВидОргана Из Органы Цикл
			...
			Если ВидОргана = Перечисления.ТипыКонтролирующихОрганов.ПФР Тогда
				ПроверитьЧтоСоглашениеЗаключено(КонтекстЭДОСервер, ТекущаяОрганизация, ВидОргана, Ошибки);
			КонецЕсли;
...
&НаСервереБезКонтекста
Процедура ПроверитьЧтоСоглашениеЗаключено(КонтекстЭДОСервер, ТекущаяОрганизация, ВидОргана, Ошибки)
	
	СоглашениеЗаключено = КонтекстЭДОСервер.ОрганизацияПодключенаКЭДОсПФР(ТекущаяОрганизация);
	Если НЕ СоглашениеЗаключено Тогда
		Ошибка = ДлительнаяОтправкаКлиентСервер.СообщениеНеЗаключеноСоглашениеСПФР();
...
Процедура ИзменитьОформлениеСчетчика(Форма)
	...
	Форма.Элементы.Счетчик.Заголовок = Новый ФорматированнаяСтрока(
		Подстрока1,
		Символы.ПС,
		Подстрока1_1,
		Подстрока2,
