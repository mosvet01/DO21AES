...
Функция ДанныеДляТестовогоКонтура1С()
	...
	Текст = "ВЫБРАТЬ
	        ...
	        // Адаптация ДО. ФИО вместо 3х полей.
	        ...
	        |	Организации.ЮрФизЛицо = &ФизЛицо КАК ЭтоИП,
	        ...
	        |		ПО (ФизическиеЛица.Ссылка = Организации.ИндивидуальныйПредприниматель)
	        |ГДЕ
	...
	// Адаптация ДО. ЮридическоеФизическоеЛицо.ФизическоеЛицо.
	ЗапросУчетныхЗаписей.УстановитьПараметр("ФизЛицо", ПредопределенноеЗначение("Перечисление.ЮрФизЛицо.ФизЛицо"));
	...
	Пока ВыборкаУчетныхЗаписей.Следующий() Цикл
		...
		Для Каждого Колонка Из РезультатЗапроса.Колонки Цикл
			Если Колонка.Имя ="ФИО" Тогда
				// Адаптация ДО.
				// Развернем ФИО в Фамилия, Имя, Отчество.
				ЧастиИмени = ФизическиеЛицаКлиентСервер.ЧастиИмени(СокрЛП(ВыборкаУчетныхЗаписей.ФИО));
				Для Каждого КлючИЗначение Из ЧастиИмени Цикл
					СтруктураУчетнойЗаписи.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
				КонецЦикла;
				
			ИначеЕсли Колонка.Имя <> "УчетнаяЗапись" Тогда
