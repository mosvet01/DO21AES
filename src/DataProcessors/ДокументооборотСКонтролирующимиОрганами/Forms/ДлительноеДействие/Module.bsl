...
Процедура ВывестиПоясняющийТекстИЗаголовокПриЗавершеннойОтправке(ЗавершенУспешно = Ложь, ДополнительноеСообщение = "")
	...
	Если ЗавершенУспешно И ВидОбъекта.ЭтоЗаявлениеВПФР Тогда
		Заголовок = НСтр("ru = 'Заявление отправлено в ПФР'");
	...
	Если ВидОбъекта.ЭтоЗаявлениеВПФР И ЗавершенУспешно Тогда
		...
		ЭтоНаПодключениеИлиНаСертификат = Вид = ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаПодключение") 
			ИЛИ Вид = ПредопределенноеЗначение("Перечисление.ВидыЗаявленийНаЭДОВПФР.НаСертификат");
		
		Если ЭтоНаПодключениеИлиНаСертификат Тогда
			ТекстПредупреждения = НСтр("ru = 'Отправка документов по электронным трудовым книжкам в ПФР станет возможна после получения положительного результата рассмотрения заявления.'") + Символы.ПС + Символы.ПС;
		Иначе
			ТекстПредупреждения = "";
		КонецЕсли;
	...
	Иначе
		...
		ПоясняющийТекст = СтрЗаменить(ПоясняющийТекст, "%Орган",        " в " + НаименованиеКонтролирующегоОргана);
...
Функция ПоясняющийТекстПриВыполняющейсяОтправке(Форма, БезПроцентов, Проценты)
	...
	ТекстНадписи = СтрЗаменить(ТекстНадписи, "%Вид",          Вид);
