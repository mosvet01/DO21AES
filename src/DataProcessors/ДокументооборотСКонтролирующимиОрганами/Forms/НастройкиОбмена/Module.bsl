...
Процедура ОбновитьНастройкиОбменаОрганизацииПослеПолученияНастроекОбмена(Результат, ДополнительныеПараметры) Экспорт
	...
	Иначе
		...
		Элементы.ОбменСПФР.Заголовок = Новый ФорматированнаяСтрока(Надпись,,Цвет,,"ПФР");
	...
	ФТСБанкРоссииМОКоличествоСтрок =
		...
		+ ?(Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии, 1, 0)
	...
	Элементы.БанкРоссии.Видимость = Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии ИЛИ ОсталосьСкрываемыхСтрок = 0;
	Элементы.БанкРоссии.Заголовок = ?(Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии,
		НСтр("ru = 'Банк России'") + ":", " ");
	Элементы.ОбменСБанкомРоссии.Видимость = Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии
		ИЛИ ОсталосьСкрываемыхСтрок = 0;
	Элементы.ОбменСБанкомРоссии.Заголовок = ?(Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии,
		Новый ФорматированнаяСтрока(
		...
		"БанкРоссии"), " ");
	Если НЕ Результат.ПоддерживаетсяСдачаОтчетностиВБанкРоссии И ОсталосьСкрываемыхСтрок > 0 Тогда
		ОсталосьСкрываемыхСтрок = ОсталосьСкрываемыхСтрок - 1;
	КонецЕсли;
	...
	Если ЭлементНастроекОбмена.ЭлектроннаяПодписьВМоделиСервиса 
