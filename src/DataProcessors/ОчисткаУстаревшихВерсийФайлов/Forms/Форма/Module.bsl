...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ПолучитьФункциональнуюОпцию("УдалятьНеактивныеВерсии") Тогда 
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не включена опция ""Автоматически очищать устаревшие версии файлов"".'"));
	КонецЕсли;
...
Процедура ЗаполнитьДиаграмму()
	...
	Диаграмма.Очистить();
	Элементы.Диаграмма.Заголовок = НСтр("ru= 'Размер версий(Мб) в зависимости от срока устаревания(дней)'");
	
	Серия = Диаграмма.Серии.Добавить(НСтр("ru= 'Очищается версий'"));
	Серия.Текст = НСтр("ru= 'Очищается версий'");
	...
	Для Каждого Строка Из ТаблицаРезультатаЗапроса Цикл
		...
		Если Строка.Размер <> 0 Тогда
			...
			Точка = Диаграмма.Точки.Добавить(НСтр("ru= 'Очищается'"));
...
Процедура ЗаполнитьОписаниеСрока()
	...
	ОписаниеСрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Будет очищено файлов версий: %1 объемом: %2, что составляет %3'"),
		ЧислоВерсий,
		ФайловыеФункцииКлиентСервер.ПолучитьСтрокуСРазмеромФайлаДляКарточкиПапки(Размер), 
		ПроцентОтБазыСтрока);
		
	ОписаниеСрока = ОписаниеСрока +  НСтр("ru='% от общего объема всех файлов.'");
...
Процедура ВыполнитьОчистку(Команда)
	...
	Если Не ЗначениеЗаполнено(ПутьСохраненияУдаляемыхВерсий) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан путь сохранения очищаемых версий'"), , "ПутьСохраненияУдаляемыхВерсий");
		Возврат;
	КонецЕсли;	
	
	Если Не ЗначениеЗаполнено(СрокХраненияНеактивныхВерсий) Тогда
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(НСтр("ru = 'Не указан срок хранения неактивных версий'"), , "СрокХраненияНеактивныхВерсий");
		Возврат;
	КонецЕсли;	
	...
	ПоказатьВопрос(
		ОписаниеОповещения,
		НСтр("ru = 'Выполнить очистку устаревших версий?'"),
...
Процедура ВыполнитьОчисткуПродолжение(Ответ, Параметры) Экспорт
	...
	Для НомерПорции = 0 По ЧислоТактов Цикл
		...
		Состояние(НСтр("ru='Выполняется очистка версий файлов с истекшим сроком хранения...'"), Процент);
...
Процедура ВыполнитьУдалениеСервер(МассивРезультатов, ВладелецВерсии, ЧислоФайлов, ЧислоВерсий, РазмерВерсий)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(НСтр("ru='Ошибка очистки неактивных версий файлов'", Метаданные.ОсновнойЯзык.КодЯзыка), 
