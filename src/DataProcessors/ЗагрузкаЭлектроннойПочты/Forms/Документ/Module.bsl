...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("ТипДокумента") Тогда
		ВызватьИсключение НСтр("ru = 'Не определен тип создаваемого документа.'");
		Возврат;
	КонецЕсли;
	...
	Если ТипДокумента = "ВнутреннийДокумент" Тогда
		...
		Заголовок = НСтр("ru = 'Создание внутреннего документа'");
	...
	ИначеЕсли ТипДокумента = "ВходящийДокумент" Тогда
		...
		Заголовок = НСтр("ru = 'Создание входящего документа'");
	...
	ИначеЕсли ТипДокумента = "ИсходящийДокумент" Тогда
		...
		Заголовок = НСтр("ru = 'Создание исходящего документа'");
...
Функция ПолучитьМассивСтруктурФайлов()
	...
	Для Каждого ФайлыСтрока Из Файлы.ПолучитьЭлементы() Цикл
		...
		Для Каждого ЭПСтрока Из ФайлыСтрока.ПолучитьЭлементы() Цикл
			...
			ЭПИнфо.Вставить("ПодписьВерна", ЭПСтрока.ПодписьВерна);
			ЭПИнфо.Вставить("СертификатДействителен", ЭПСтрока.СертификатДействителен);
			ЭПИнфо.Вставить("ТекстОшибкиПроверкиПодписи", ЭПСтрока.ТекстОшибкиПроверкиПодписи);
			ЭПИнфо.Вставить("ТекстОшибкиПроверкиСертификата", ЭПСтрока.ТекстОшибкиПроверкиСертификата);
			ЭПИнфо.Вставить("ПодписьЗагруженаИзФайла", Истина);
...
&НаКлиенте
Процедура ПеренестиФайлВПодпись(ФайлыСтрока, ЭПСтрока, Сертификат = Неопределено, РезультатПроверки)
...
Процедура ФайлыПеретаскиваниеПослеПроверкиПодписи(Результат, ДополнительныеПараметры) Экспорт
	...
	ДвоичныеДанныеЭП = ПолучитьДвоичныеДанныеФайла(СтрокаЭП);
	Сертификат = ПолучитьСертификатИзПодписи(ДвоичныеДанныеЭП, ДополнительныеПараметры.МенеджерКриптографии);
	ПеренестиФайлВПодпись(СтрокаФайла, СтрокаЭП, Сертификат, Результат);
...
Процедура ПриСозданииНаСервереРедакцииКонфигурации()	
	...
	Элементы.ВнутреннийДокументОрганизация.Заголовок = РедакцииКонфигурацииКлиентСервер.Организация();
	Элементы.ВнутреннийДокументОрганизация.Подсказка = РедакцииКонфигурацииКлиентСервер.Организация();
	
	Элементы.ВходящийДокументОрганизация.Заголовок = РедакцииКонфигурацииКлиентСервер.Организация();
	Элементы.ВходящийДокументОрганизация.Подсказка = РедакцииКонфигурацииКлиентСервер.Организация();
	
	Элементы.ИсходящийДокументОрганизация.Заголовок = РедакцииКонфигурацииКлиентСервер.Организация();
	Элементы.ИсходящийДокументОрганизация.Подсказка = РедакцииКонфигурацииКлиентСервер.Организация();
