...
Процедура СоздатьДокументИзСообщенияПослеЗаполненияРеквизитовДокумента(
	...
	Для каждого ФайлИнфо Из ПараметрыЗаполнения.Файлы Цикл
		Если Не ЭтоАдресВременногоХранилища(ФайлИнфо.Адрес) Тогда
			...
			Иначе
				ВызватьИсключение НСтр("ru = 'Ошибка загрузки файла.'");
			КонецЕсли;
		КонецЕсли;
	...
	Состояние(НСтр("ru = 'Создается документ. Пожалуйста, подождите...'"));
...
Функция СоздатьДокументИзСообщенияНаСервере(ИмяСправочника, ПараметрыЗаполнения)
	...
	Попытка
		...
		Если ПустаяСтрока(Документ.Заголовок) Тогда
			Документ.Заголовок = НСтр("ru = 'Письмо без темы'");
		КонецЕсли;
		...
		Если ИмяСправочника = "ВходящиеДокументы"
			...
			Для Каждого ФайлИнфо Из ПараметрыЗаполнения.Файлы Цикл
				Если Не ЭтоАдресВременногоХранилища(ФайлИнфо.Адрес) Тогда
					ВызватьИсключение НСтр("ru = 'Файл не найден во временном хранилище.'");
				КонецЕсли;
				...
				Если ИмяФайла = "1C-53898-2010" И НРег(РасширениеФайла) = "xml" Тогда
					...
					Комментарий = НСтр("ru = 'Сообщение взаимодействия СЭД по ГОСТ Р 53898-2010'");
...
Функция СоздатьФайлСВерсией(Параметры) 
	...
	ПараметрыРаспознавания = ПолучитьПараметрыРаспознавания(
		ОбщегоНазначения.ИдентификаторОбъектаМетаданных(ТипЗнч(Параметры.ВладелецФайла)));
...
// Возвращает параметры распознавания.
//
// Параметры:
...
//
&НаСервере
Функция ПолучитьПараметрыРаспознавания(Владелец)
	...
	Если РаботаСФайламиВызовСервера.ПолучитьИспользоватьРаспознавание()
		...
		И РаботаСФайламиВызовСервера.СтатусРаспознаванияПоУмолчаниюДляФайловВладельца(Владелец) =
