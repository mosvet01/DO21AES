...
Функция Put(Сообщение)
	...
	Попытка
		...
		НовоеСообщение.ФорматСообщения = Справочники.ФорматыСообщенийСВД.СообщениеПоГОСТ53898ВложенныеФайлы;
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если Выборка.Следующий() Тогда
				...
				Если МассивФайлов.Количество() <> 1 Тогда
					Если МассивФайлов.Количество() = 0 Тогда
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'СВД.ОтправкаПолучателю.WS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,
							Метаданные.Справочники.СерверныеСообщенияСВД,
							Выборка.Ссылка,
							НСтр("ru = 'Отсутствует файл сообщения.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
					Иначе	
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'СВД.ОтправкаПолучателю.WS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,
							Метаданные.Справочники.СерверныеСообщенияСВД,
							Выборка.Ссылка,
							НСтр("ru = 'Количество файлов сообщения больше одного.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()));
					КонецЕсли;
				Иначе
					...
					Иначе
						...
						ТипОбъектаXDTO = ФабрикаXDTO.Тип("http://www.1c.ru/medo", "Header");
			...
			Исключение
				...
				ОтменитьТранзакцию();
				Информация = ИнформацияОбОшибке();
				ИмяСобытия = НСтр("ru = 'СВД.ОтправкаПолучателю.WS'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка());
				Уровень = УровеньЖурналаРегистрации.Ошибка;
				ОбъектМетаданных = Метаданные.Справочники.СерверныеСообщенияСВД;
				Данные = Выборка.Ссылка;
				Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'При передаче сообщения получателю возникла ошибка ""%1""'",
...
// Создает XDTO объект указанного типа из пространства имен "http://www.1c.ru/medo"
Функция СоздатьОбъект(ТипОбъекта) Экспорт
	
	Возврат ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://www.1c.ru/medo", ТипОбъекта));
...
Функция СформироватьСообщениеОбОшибке(ИдВходящегоСообщения, КодОшибки, ТекстОшибки, Кому)
	...
	Сообщение.from_organization = НСтр("ru = 'Сервер СВД 1С:Документооборот'"); // Организация-отправитель
