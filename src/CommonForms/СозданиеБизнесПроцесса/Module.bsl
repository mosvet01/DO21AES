...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(ВладелецШаблона)
		...
		Если ЗначениеЗаполнено(Параметры.ТипЭлементаСхемыПроцесса) Тогда
			...
			Команды.СоздатьПроцесс.Подсказка = НСтр("ru = 'Выбрать это действие'");
			
			Элементы.СоздатьПроцесс.Заголовок = НСтр("ru = 'Выбрать действие'");
			Элементы.ДеревоШаблоновКонтекстноеМенюСоздатьПроцесс.Заголовок = НСтр("ru = 'Выбрать это действие'");
	...
	Если ЗначениеЗаполнено(ТипыПроцессов) Тогда
		...
		Команды.СоздатьПроцесс.Заголовок = НСтр("ru = 'Готово'");
	...
	Для Каждого СтрПроцесс Из МассивНезаполненныхПроцессов Цикл
		...
		Если НесколькоСвоихВариантов Тогда
			...
			Если СтрПроцесс.Имя = "КомплексныйПроцесс" Тогда
				...
				НоваяСтрока.Наименование = НСтр("ru = 'Новый комплексный процесс'");
			...
			ИначеЕсли СтрПроцесс.Имя = "ОбработкаВнутреннегоДокумента" Тогда
				...
				НоваяСтрока.Наименование = НСтр("ru = 'Новая обработка внутреннего документа'");
			...
			ИначеЕсли СтрПроцесс.Имя = "ОбработкаВходящегоДокумента" Тогда
				...
				НоваяСтрока.Наименование = НСтр("ru = 'Новая обработка входящего документа'");
				ОписаниеПроцесса = НСтр("ru = 'Создание нового процесса обработки документа для заполнения вручную.
					|
			...
			ИначеЕсли СтрПроцесс.Имя = "ОбработкаИсходящегоДокумента" Тогда
				...
				НоваяСтрока.Наименование = НСтр("ru = 'Новая обработка исходящего документа'");
				ОписаниеПроцесса = НСтр("ru = 'Создание нового процесса обработки документа для заполнения вручную.
					|
		...
		Иначе
			НоваяСтрока.Наименование = НСтр("ru = 'Новый'");
...
Процедура ПостроитьДеревоВариантов()
	...
	Если Не СозданиеБезПредмета Тогда
		ДобавитьГруппуВДеревоШаблонов(Дерево, НСтр("ru = 'Рекомендованные шаблоны'"),
			МассивРекомендованныхШаблонов);
	КонецЕсли;
	
	ДобавитьГруппуВДеревоШаблонов(Дерево, НСтр("ru = 'Общие шаблоны'"), МассивОбщихШаблонов);
	ДобавитьГруппуВДеревоШаблонов(Дерево, НСтр("ru = 'Новые процессы'"), МассивНезаполненныхПроцессов);
	ДобавитьГруппуВДеревоШаблонов(Дерево, НСтр("ru = 'Другие шаблоны'"), МассивОстальныхШаблоны, Ложь);
...
Функция СформироватьСводкуПоГруппеЭлементов(ИмяГруппы)
	...
	Если Найти(ИмяГруппы, НСтр("ru = 'Рекомендованные шаблоны'")) <> 0 Тогда
		Текст = НСтр("ru = 'Шаблоны процессов, привязанные к данному виду документа.'");
	ИначеЕсли Найти(ИмяГруппы, НСтр("ru = 'Общие шаблоны'")) <> 0 Тогда
		Текст = НСтр("ru = 'Шаблоны процессов, не привязанные ни к одному из видов документов.'");
	ИначеЕсли Найти(ИмяГруппы, НСтр("ru = 'Новые процессы'")) <> 0 Тогда
		Текст = НСтр("ru = 'Ссылки на создание новых, незаполненных процессов.'");
	ИначеЕсли Найти(ИмяГруппы, НСтр("ru = 'Другие шаблоны'")) <> 0 Тогда
