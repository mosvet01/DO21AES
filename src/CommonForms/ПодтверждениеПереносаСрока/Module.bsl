...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Перенос срока исполнения процессов'");
	...
	Иначе
		Заголовок = НСтр("ru = 'Перенос срока исполнения задач'");
	...
	Эл = БизнесПроцессыИЗадачиСервер.ЭлементУсловногоОформленияПоПредставлению(
		УсловноеОформление, НСтр("ru = 'Срок процесса изменен'"));
	...
	Если Параметры.ЗапрашиватьПричинуПереносаСроков Тогда
		...
		Если Не ЗначениеЗаполнено(ПричинаПереносаСрока) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не указана причина переноса срока.'"),,
