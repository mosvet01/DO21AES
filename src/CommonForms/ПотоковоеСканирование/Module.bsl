...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ИспользоватьGhostscript Или Не ИспользоватьImageMagickДляРаспознаванияPDF Тогда	
		...
		Элементы.ИспользоватьImageMagickДляПреобразованияВPDF.Видимость = Ложь;
...
Процедура ВыполнитьЗагрузкуИзКаталога()
	...
	ИспользоватьGhostscript = РаботаСФайламиВызовСервера.ИспользоватьGhostscript();
...
Функция РаспознатьШтрихКодОдногоФайла(ИмяФайлаКартинки, ПрерватьПроцессСканирования)
	...
	Если НРег(Файл.Расширение) = НРег(".pdf") И ИспользоватьImageMagickДляПреобразованияВPDF Тогда
