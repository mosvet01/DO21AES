...
Процедура СоздатьПредмет(Команда)
	...
	ИначеЕсли ТипПредмета = "Документ.ВходящееПисьмо" Тогда
		ВызватьИсключение НСтр("ru = 'Нельзя создать входящее письмо, можно выбрать только существующее'");
	...
	ИначеЕсли ТипПредмета = "Справочник.СообщенияОбсуждений" Тогда
		ВызватьИсключение НСтр("ru = 'Можно выбрать только существующее сообщение обсуждений'");
	...
	ИначеЕсли ТипПредмета = "Справочник.ПроектныеЗадачи" Тогда
		ВызватьИсключение НСтр("ru = 'Можно выбрать только существующую проектную задачу'");
	Иначе
		ВызватьИсключение НСтр("ru = 'Некорректный тип предмета'");
	КонецЕсли;
...
Процедура УстановитьЗаголовкиПодсказок()
	...
	Если ЗначениеЗаполнено(ШаблонДокумента) Тогда
		ЗаголовокСоздать = ЗаголовокСоздать + " " 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'по шаблону ""%1""'"), Строка(ШаблонДокумента));
	КонецЕсли;
	Если ЗначениеЗаполнено(ПредметОснование) Тогда
		ЗаголовокСоздать = ЗаголовокСоздать + " " 
			+ СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'на основании предмета ""%1""'"), Строка(ИмяПредметаОснование));
	КонецЕсли;
	ЗаголовокСоздать = ЗаголовокСоздать + ".";
	...
	ЗаголовокВыбрать = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выбрать существующий %1 из списка.'"),
		НРег(ТипПредметаПредставление));
...
Процедура УстановитьПредмет(НовыйПредмет, ПараметрыДобавления)
	...
	Если ЗначениеЗаполнено(Предмет) Тогда
		...
		Если РаботаСРабочимиГруппами.ПоОбъектуВедетсяАвтоматическоеЗаполнениеРабочейГруппы(ОбъектДляРасширенияРГ)
