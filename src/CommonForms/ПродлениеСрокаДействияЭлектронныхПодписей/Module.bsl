...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		Если ТипЗнч(Параметры.Подпись) = Тип("Структура") Тогда
			Если ЗначениеЗаполнено(Параметры.Подпись.ПодписанныйОбъект) Тогда
				...
				УникальныйИдентификаторПодписи = Параметры.Подпись.УникальныйИдентификатор; 
	...
	СкрытьПодписанныйОбъект = Неопределено;
	Параметры.Свойство("СкрытьПодписанныйОбъект", СкрытьПодписанныйОбъект);
	
	Если СкрытьПодписанныйОбъект = Истина Тогда
		Элементы.ПодписанныйОбъект.Видимость = Ложь;
	КонецЕсли;
...
Процедура ЗаполнитьПодпись()
	...
	Пока Выборка.Следующий() Цикл
		Массив.Добавить(Новый Структура("ПодписанныйОбъект, УникальныйИдентификатор",
...
Процедура ОпределитьТипПодписейОбъекта()
	...
	УстановленныеПодписи = ЭлектроннаяПодпись.УстановленныеПодписи(
		ПодписанныйОбъект,
		?(ЗначениеЗаполнено(УникальныйИдентификаторПодписи), УникальныйИдентификаторПодписи, Неопределено));
