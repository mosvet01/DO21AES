...
Процедура ЗаполнитьОбзорПроцесса(HTMLТекст, Процесс) Экспорт
	...
	Иначе
		...
		Действия.Сортировать("НомерСтроки Возр");
...
Процедура ОбработатьПолучениеПроцессаИзУзлаРИБ(Узел, ЭлементДанных) Экспорт
	...
	Если ЭтотУзел = ЭлементДанных.УзелОбмена Тогда // Получение процесса в мастер-узле.
		...
		// Передаем изменения предметов в текущий процесс
		
		НоваяТаблицаПредметов = ЭлементДанных.Предметы.Выгрузить();
		
		Если Не ОбщегоНазначения.ДанныеСовпадают(
			ПроцессПоСсылке.Предметы.Выгрузить(), НоваяТаблицаПредметов) Тогда
			
			ПроцессПоСсылке.Предметы.Загрузить(НоваяТаблицаПредметов);
			
			ПроцессПоСсылке.ПредметыЗадач.Загрузить(ЭлементДанных.ПредметыЗадач.Выгрузить());
		...
		Иначе
			...
			ЭтапыПоСсылке = ПроцессПоСсылке.Этапы.Выгрузить();
			ЭтапыПоСсылке.Сортировать("НомерСтроки Возр");
			ЭтапыВЭлементеДанных = ЭлементДанных.Этапы.Выгрузить();
			ЭтапыВЭлементеДанных.Сортировать("НомерСтроки Возр");
			
			Если Не ОбщегоНазначения.ДанныеСовпадают(
				ЭтапыПоСсылке, ЭтапыВЭлементеДанных) Тогда
...
Функция ВариантыОтветовДляВыполненияЗадачиПоПочте(
	...
	Иначе
		ВызватьИсключение 
			НСтр("ru = 'Неожиданная точка маршрута задачи.'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
