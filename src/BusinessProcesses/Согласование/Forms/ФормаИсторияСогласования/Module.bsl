			...
			Если СтрокаИтерации.НомерИтерации = БизнесПроцесс.НомерИтерации Тогда 
				...
				Для Каждого Строка Из БизнесПроцесс.Исполнители Цикл
					...
					Если ЗначениеЗаполнено(Строка.ЗадачаИсполнителя) Тогда 
						...
						Если ЗначениеЗаполнено(Строка.ЗадачаИсполнителя.РольИсполнителя) Тогда 
							СтрокаДереваИсполнитель.ИсполнительПроцесса = Строка.ЗадачаИсполнителя.РольИсполнителя;
						Иначе					
							СтрокаДереваИсполнитель.ИсполнительПроцесса = Строка.ЗадачаИсполнителя.Исполнитель;
						КонецЕсли;
					...
					Иначе
						...
						СтрокаДереваИсполнитель.ИсполнительПроцесса = Строка.Исполнитель;
					КонецЕсли;	
				КонецЦикла;	
...
Процедура ИсторияИсполнителяИсполнительПриИзменении(Элемент)
	...
	ТекущиеДанные.ИсполнительПроцесса = ТекущиеДанные.Исполнитель;
...
Процедура ОК(Команда)
	...
	Для Инд1 = 0 По КоличествоИсполнителей-2 Цикл
		...
		Для Инд2 = Инд1+1 По КоличествоИсполнителей-1 Цикл
			...
			Если ЗначениеЗаполнено(Строка2.РезультатСогласования) Тогда
				Продолжить;
			КонецЕсли;
			
			Если Строка1.ИсполнительПроцесса = Строка2.ИсполнительПроцесса Тогда 
				
				ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
					НСтр("ru = 'Исполнитель ""%1"" указан дважды в списке согласующих!'"), 
					Строка(Строка1.ИсполнительПроцесса));
