...
Процедура ОбработатьПолучениеПроцессаИзУзлаРИБ(Узел, ЭлементДанных) Экспорт
	...
	Если ЭтотУзел = ЭлементДанных.УзелОбмена Тогда // Получение процесса в мастер-узле.
		...
		ПредметыДоИзменения = ПроцессПоСсылке.Предметы.Выгрузить();
		
		РаботаСБизнесПроцессами.ДобавитьНовыеПредметыВПроцесс(
			ПроцессПоСсылке, ЭлементДанных.Предметы, ЭлементДанных.ПредметыЗадач);
		
		РаботаСБизнесПроцессами.ПередатьЗаполняемыеПредметыВПроцесс(
			ПроцессПоСсылке, ЭлементДанных.Предметы, ЭлементДанных.ПредметыЗадач);
		
		Если Не ОбщегоНазначения.ДанныеСовпадают(
			ПредметыДоИзменения, ПроцессПоСсылке.Предметы.Выгрузить()) Тогда
...
Функция ВозможноВыполнениеЗадачиПроцессаПоПочте(РеквизитыЗадачи) Экспорт
	...
	Если РеквизитыЗадачи.ТочкаМаршрута = ТочкиМаршрута.Рассмотреть Тогда
		
		Предметы = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыЗадачи.Ссылка, "Предметы");
		...
		Для Каждого Предмет Из Предметы Цикл
			...
			Если ТипЗнч(Предмет.Предмет) = Тип("СправочникСсылка.ВнутренниеДокументы")
				...
				ВидДокумента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					Предмет.Предмет, "ВидДокумента");
				
				ПодписыватьРезолюцииЭП = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
					ВидДокумента, "ПодписыватьРезолюцииЭП");
