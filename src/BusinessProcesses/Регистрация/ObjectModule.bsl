...
Процедура ПриОткрепленииПредмета(Документ = Неопределено) Экспорт
	...
	Если ДелопроизводствоКлиентСервер.ЭтоСсылкаНаДокумент(Документ) Тогда 
		...
		// Установим состояние "Проект" в случае прерывания процесса регистрации
		Если ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(Документ) Тогда
			СостояниеПредмета = Перечисления.СостоянияДокументов.НаРегистрации;
		Иначе
			СостояниеПредмета = Перечисления.СостоянияДокументов.Проект;
		КонецЕсли;
...
Процедура КомпенсироватьСостояниеПредметов() Экспорт
	...
	ДатаПрерывания = ТекущаяДатаСеанса();
