...
Процедура ЗаполнитьПроекты()
	...
	Иначе
		...
		Если ЕстьПроекты Тогда
			Элементы.ДекорацияНетЗадачи.Заголовок = НСтр("ru = 'В ваших проектах нет активных задач.'");
		Иначе	
			Элементы.ДекорацияНетЗадачи.Заголовок = НСтр("ru = 'Нет активных проектов, у которых вы являетесь руководителем.'");
		КонецЕсли;	
		
	КонецЕсли;	
	
	ОбновитьИнформациюОбОтсутствиях();
	
КонецПроцедуры

&НаСервере
Процедура ОбновитьИнформациюОбОтсутствиях()
	
	СотрудникиСтрок = Новый Массив;
	СтрокиСотрудников = Новый Массив;
	
	УзлыКОбработке = Новый Массив;
	УзлыКОбработке.Добавить(ДеревоПользователей);
	Пока УзлыКОбработке.Количество() > 0 Цикл
		
		УзелКОбработке = УзлыКОбработке.Получить(0);
		
		Для Каждого СтрокаДерева Из УзелКОбработке.ПолучитьЭлементы() Цикл
			
			Если СтрокаДерева.НомерКартинки = 3
				И ЗначениеЗаполнено(СтрокаДерева.Пользователь)
				И ТипЗнч(СтрокаДерева.Пользователь) = Тип("СправочникСсылка.Пользователи") Тогда
				
				СтрокиСотрудников.Добавить(СтрокаДерева);
				СотрудникиСтрок.Добавить(СтрокаДерева.Пользователь);
				
			КонецЕсли;
			
			УзлыКОбработке.Добавить(СтрокаДерева);
	...
	Для Каждого СтрокаСотрудника Из СтрокиСотрудников Цикл
		...
		ОписаниеОтсутствия = ОписанияТекущихОтсутствийСотрудников[СтрокаСотрудника.Пользователь];
		Если ОписаниеОтсутствия <> Неопределено Тогда
			СтрокаСотрудника.НомерКартинки = 5;
		КонецЕсли;
