...
Функция ПредставлениеПолучателей(НаименованияПолучателей)
	...
	Иначе
		...
		ТекстПолучатели = СтроковыеФункцииКлиентСервер.СтрокаСЧисломДляЛюбогоЯзыка(
			НСтр("ru=';%1 получателю;;%1 получателям;%1 получателям;%1 получателям'"), КоличествоПолучателей);
...
Процедура СообщитьОЗавершенииПодготовкиФайловДляБанка()
	...
	ШаблонТекстаПредупреждения = НСтр("ru='В информационной базе сохранены печатные формы отчета %1 для передачи в составе заявки 1С:Кредит'");
