...
Процедура ДобавитьСтроку(ИмяОбласти)
	...
	Для Каждого КЗ Из ДопСтрокиТекущейСтраницы[НомерДобавляемойСтроки-1].Значение Цикл
		Обл = ПредставлениеУведомления.Области.Найти(КЗ.Ключ + РазделительНомераСтроки + Формат(НомерДобавляемойСтроки, "ЧГ="));
		Если Обл <> Неопределено Тогда 
			Элементы.ПредставлениеУведомления.ТекущаяОбласть = ПредставлениеУведомления.Область(Обл.Верх, 3, Обл.Верх, 3);
			Прервать;
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры
...
Процедура ДобавитьФайлЗавершение(Результат, АдресФайла, ВыбИмяФайла, Парам) Экспорт
	...
	Если ВРег(Прав(ВыбИмяФайла, 4)) <> ".PDF" Тогда
		
		ТекстПредупреждения = НСтр(
			"ru='Файл приложения должен иметь расширение PDF!'");
	КонецЕсли;
...
Функция ТаблицаЗаменИменМногострочныхЧастей()
	...
	Замены.Вставить("МнгСтр2", "ОбщПолож");
	Замены.Вставить("МнгСтр3", "ОбщПолож");
	Замены.Вставить("МнгСтр4", "НДС");
	Замены.Вставить("МнгСтр5", "НДС");
	Замены.Вставить("МнгСтр6", "НалИмущ");
	Замены.Вставить("МнгСтр7", "Приб");
	Замены.Вставить("МнгСтр8", "Приб");
	Замены.Вставить("МнгСтр9", "Приб");
	Замены.Вставить("МнгСтр10", "Приб");
	Замены.Вставить("МнгСтр11", "Приб");
	Возврат Замены;
КонецФункции
