...
Функция МакетОтчетаСодержащийМСЧ(ПолноеИмяМСЧ)
	...
	Если ПолноеИмяМСЧ = "МногострочнаяЧастьВ010107" Тогда
		Возврат Отчеты[Объект.ИмяОтчета].ПолучитьМакет("Форма2020_4_Страница4");
		
	ИначеЕсли ПолноеИмяМСЧ = "МногострочнаяЧастьД010102" Тогда
		Возврат Отчеты[Объект.ИмяОтчета].ПолучитьМакет("Форма2020_4_Страница6");
		
	ИначеЕсли ПолноеИмяМСЧ = "МногострочнаяЧастьД010202" Тогда
		Возврат Отчеты[Объект.ИмяОтчета].ПолучитьМакет("Форма2020_4_Страница7");
...
Процедура ПечатьБРОНаСервере(ЕстьВыходЗаГраницы)
	...
	СтруктураРеквизитовУведомления = Новый Структура("СписокПечатаемыхЛистов",
		Отчеты[ИДОтчета(ИмяФормы)].СформироватьСписокЛистов(Объект, ЕстьВыходЗаГраницы));
