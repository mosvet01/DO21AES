...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ПояснениеКВыборуБухОтчетности.Заголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ТекстДекорации, ПреставлениеПериодаБухотчетности);
...
Процедура ПредставлениеБухОтчетностиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ТекстЗаголовка = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		НСтр("ru = 'Выбор бухгалтерской отчетности за %1'"), ПреставлениеПериодаБухотчетности);
...
Функция ПреставлениеПериодаБухОтчетности(ДатаКонцаПериодаОтчета)
	...
	Если НомерКвартала = 4 Тогда
		ПреставлениеПериода = СтрокаЧГ0(ГодОтчета) + НСтр("ru = ' г.'");
	Иначе
		ПреставлениеПериода = СтрокаЧГ0(НомерКвартала) + НСтр("ru = ' квартал '")
		                    + СтрокаЧГ0(ГодОтчета) + НСтр("ru = ' г.'");
	КонецЕсли;
...
Процедура Инициализация(БезОткрытияФормы = Ложь) Экспорт
	...
	Если СтруктураРеквизитовФормы.мСохраненныйДок = Неопределено Тогда
		...
		СтруктураРеквизитовФормы.ВерсияФормата = "5.12.05";
		...
		Для Каждого СтрокаПоСчету Из ДанныеСекции.Строки Цикл
			...
			Для Каждого Субконто1 Из СтрокаПоСчету.Строки Цикл
				Если ЗначениеЗаполнено(Субконто1.КорСчет) Тогда
					Область_СтрокаТаблицыКорСчет = Шаблон.ПолучитьОбласть("СтрокаТаблицыКорСчет");
...
Процедура ДобавитьФайл()
	...
	ОписаниеОповещения = Новый ОписаниеОповещения("ДобавитьФайлЗавершение", ЭтотОбъект);
...
Функция ДобавитьФайлНаСервере(ОписаниеФайла)
	...
	ПолноеИмяФайла = ?(ЗначениеЗаполнено(ОписаниеФайла.ПолноеИмя), ОписаниеФайла.ПолноеИмя, ОписаниеФайла.Имя);
...
Процедура ВыбратьОткрытьТипизированныйФайл(ИмяОбласти)
	...
	Если ЗначениеЗаполнено(ТипФайла) Тогда
		...
		Иначе
			...
			ОписаниеОповещения = Новый ОписаниеОповещения(
				"ВыбратьТипизированныйФайлЗавершение", ЭтотОбъект, ДополнительныеПараметры);
...
Функция УниверсальнаяВыгрузкаXMLНаСервере(ДвоичныеДанныеФайла, ИмяФайлаПредставления, ПостфиксПредставлений)
	...
	СформированныеФайлы = СформироватьФайлыПредставленийРазделов(ВременныйКаталогПакета, ПостфиксПредставлений);
...
Процедура ВывестиСтраницуВУзел_ОСВ(Компоновщик, СтраницаРаздела, ПараметрыВыгрузки)
	...
	ВывестиХешСумму(Компоновщик, "ОСВ", ПараметрыВыгрузки);
...
Процедура ВывестиУзел_ОСВПоСчетам(Компоновщик, ПараметрыВыгрузки, СтруктураВыгрузки)
	...
	НачПериода = НачалоМесяца(СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета);
	КонПериода = СтруктураРеквизитовФормы.мДатаКонцаПериодаОтчета;
	...
	Если ВыгружаемыеСчета.Количество() > 0 Тогда
		...
		Для Каждого ВыгружаемыйСчет Из ВыгружаемыеСчета Цикл
			Если ДействуетСВерсииФормата("5.11.01") Тогда
				ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки);
			Иначе
...
&НаСервере
Процедура ВывестиСтраницуВУзел_ОСВПоСчетам_ОСВПоСчету_5_11_01(Компоновщик, ВыгружаемыйСчет, ПараметрыВыгрузки)
...
Процедура ВывестиУзел_ОперБанкСч(Компоновщик, ПараметрыВыгрузки)
	...
	Если НЕ ДействуетСВерсииФормата("5.11.01") Тогда
		ОтчетностьВБанкиСлужебный.ВывестиУзел_БухгалтерскиеСчета(Компоновщик);
	КонецЕсли;
...
Процедура НастроитьОтчетПоСчетам(ИмяРаздела)
	...
	ФормаНастройкиСпискаСчетов = ПолучитьФорму("Отчет.БухгалтерскаяОтчетностьВБанк.Форма.НастройкаСпискаСчетов", ПараметрыФормы, ЭтотОбъект);
