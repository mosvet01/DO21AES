...
Функция ЭтоОбластьОКСМ(Область)
	Если ((Область.Имя = "ИО_КодРег" Или Область.Имя = "ФЛ_КодРег") 
		И (ТекущееИДНаименования = "СвУчПрям" Или ТекущееИДНаименования =  "СвУчКосв" Или ТекущееИДНаименования =  "ИнфУчаст")) Тогда
...
Процедура ДеревоСтраницПриАктивизацииСтроки(Элемент)
	Если УИДТекущаяСтраница <> Элемент.ТекущиеДанные.УИД Тогда 
		...
		Если ТекущееИДНаименования = "СвУчПрям"
			Или ТекущееИДНаименования = "СвУчКосв" Тогда 
			ДоступностьПолейФЛИОРО();
		ИначеЕсли ТекущееИДНаименования = "ИнфУчаст" Тогда 
...
Процедура ЗагрузитьИзXMLНаСервере(ПараметрыЗагрузкиXML)
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.СвУчПрям Цикл 
		ТекСтр = Стр.Значение;
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.СвУчКосв Цикл 
		ТекСтр = Стр.Значение;
		Для Каждого КЗ Из ТекСтр Цикл 
			Если СтрНачинаетсяС(КЗ.Ключ, "РО_") И ЗначениеЗаполнено(КЗ.Значение) Тогда 
				ТекСтр.ТипУчастника = "1";
	...
	Для Каждого Стр Из ДанныеМногостраничныхРазделов.ИнфУчаст Цикл 
		ТекСтр = Стр.Значение;
		Для Каждого КЗ Из ТекСтр Цикл 
			Если СтрНачинаетсяС(КЗ.Ключ, "РО_") И ЗначениеЗаполнено(КЗ.Значение) Тогда 
