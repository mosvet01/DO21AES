...
Процедура КонвертироватьДеревоНастроек(УсловияДоступности)
	...
	// Удаление строк не подходящих по условиям доступности.
	ДеревоНастроек.Колонки.Добавить("ПреобразоватьВМассив", Новый ОписаниеТипов("Булево"));
	ОтборВсехСтрок = Новый Структура("ПреобразоватьВМассив", Ложь);
	СтрокиДереваЗначений = ДеревоНастроек.Строки.НайтиСтроки(ОтборВсехСтрок, Истина);
	ДеревоНастроек.Колонки.Удалить("ПреобразоватьВМассив");
	
	Для Каждого СтрокаДереваЗначений Из СтрокиДереваЗначений Цикл
		Если ЗначениеЗаполнено(СтрокаДереваЗначений.УсловияДоступности) Тогда
			Если УсловияДоступности.Найти(СтрокаДереваЗначений.УсловияДоступности) = Неопределено Тогда
				СтрокаДереваЗначений.Родитель.Строки.Удалить(СтрокаДереваЗначений);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
