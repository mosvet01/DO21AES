		...
		Для Каждого ДопСтрока Из ЭтотОбъект[ЭлементСтруктуры.Ключ] Цикл
			...
			Для НомерГрафы = 1 По мСтруктураКолвоКолонокРазделов[ЭлементСтруктуры.Ключ] Цикл
				...
				И НЕ ЗначениеЗаполнено(ДопСтрока[КолонкаТаблицыИмя]) Тогда
					...
					ДопСтрока[КолонкаТаблицыИмя] = ЭтотОбъект["СтруктураДанных" + ИмяТекРаздела][КолонкаТаблицыИмя + "_" + НомСтр];
...
Функция ИзвлечьСтруктуруXML()
	...
	Макет = ОбъектОтчета(ИмяФормы).ПолучитьМакет("СхемаВыгрузки501_Кв3");
...
Функция ПроверитьВозможностьВыгрузки(ПараметрыВыгрузки)
	...
	Если НЕ ЗначениеЗаполнено(ПараметрыВыгрузки.Период) Тогда
		РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Формат выгрузки допускает выгрузку отчетов только за I квартал, полугодие, 9 месяцев!", "Баланс", "ДатаБаланса");
		СтатусПроверки = Ложь;
	КонецЕсли;
...
Функция ЗначениеСоответствуетФормату(Стр)
	...
	Если Стр.Формат = "Т" ИЛИ Стр.Формат = "T" Тогда // учтем оба варианта: кириллицу и латиницу
		...
		Если ДлинаЗначения < Стр.МинРазмерность Тогда
			СтатусПроверки = Ложь;
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Длина значения показателя """ + РегламентированнаяОтчетность.ПредставлениеПоказателя(Стр) + """ меньше минимально возможной (" + ДлинаЗначения + " < " + Стр.МинРазмерность + ")", "Баланс", "");
		КонецЕсли;
		Если ДлинаЗначения > Стр.МаксРазмерность Тогда
			СтатусПроверки = Ложь;
			РегламентированнаяОтчетность.СообщитьВТаблицуСообщений(ЭтотОбъект, "Длина значения показателя """ + РегламентированнаяОтчетность.ПредставлениеПоказателя(Стр) + """ больше максимально возможной (" + ДлинаЗначения + " > " + Стр.МаксРазмерность + ")", "Баланс", "");
		КонецЕсли;
	КонецЕсли;
	Возврат СтатусПроверки;
