...
Процедура ЗаполнитьСписок()
	...
	ПроектыРазрешены = Ложь;
	Если ПравоДоступа("Чтение", Метаданные.Справочники.Проекты) Тогда
		ПроектыРазрешены = Истина;
	КонецЕсли;
	
	Запрос = Новый Запрос;
	Если ПроектыРазрешены Тогда 
		Запрос.Текст =
		"ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ЕСТЬNULL(ВсеПроекты.Ссылка, Значение(Справочник.Проекты.ПустаяСсылка)) КАК Проект,
		|	ЕСТЬNULL(ВсеПроекты.Представление, &ИмяГруппыБезПроекта) КАК Представление,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Проекты КАК ВсеПроекты
		|		ПО ВсеПроекты.Ссылка = ОбщиеРеквизитыДокументов.Документ.Проект
...
Процедура ПриИзмененииПараметровЭкранаНаСервере()
	...
	Если ЭтоМобильныйСБольшимЭкраном И Не ЭтоМобильныйВПортретномРежиме Тогда
		НастроитьФормуДляМобильногоВАльбомномРежиме();
	Иначе
		НастроитьФормуДляМобильногоВПортретномРежиме();
	КонецЕсли
	
КонецПроцедуры

&НаСервере
Процедура НастроитьФормуДляМобильногоВПортретномРежиме()
	
	// Скроем область предпросмотра.
	Элементы.ГруппаМиникарточка.Видимость = Ложь;
	
КонецПроцедуры

&НаСервере
Процедура НастроитьФормуДляМобильногоВАльбомномРежиме()
	...
	// Переместим файлы на отдельную страницу по аналогии с "Документы".
	Элементы.ГруппаФайлыМК.Видимость = Истина;
	Элементы.Переместить(Элементы.Файлы, Элементы.ГруппаФайлыМК);
	
	// Переместим связи на отдельную страницу по аналогии с "Документы".
	Элементы.ГруппаСвязиМК.Видимость = Истина;
	Элементы.Переместить(Элементы.ДеревоСвязей, Элементы.ГруппаСвязиМК);
	
	ЗаголовокСодержаниеФайлы = НСтр("ru = 'Описание'");
	ЗаголовокЗадачиСвязи = НСтр("ru = 'Задачи'");
	
	// Настройка страниц миникарточки.
	Элементы.СписокЗадачи.Шапка = Ложь;
	Элементы.Переместить(Элементы.ЗадачиГруппа, Элементы.СписокЗадачиГруппа);
	Элементы.ГруппаЗадачиСвязи.ЦветФона = МобильныйКлиентВызовСервера.ЦветФонаГруппыСписка();
	Элементы.ЗадачиНаименование.Высота = 3;
	Элементы.ЗадачиИсполнитель.ЦветТекста = МобильныйКлиентВызовСервера.ЦветТекстаИсполнительАвтор();
	Элементы.ЗадачиИсполнитель.Шрифт = МобильныйКлиентВызовСервера.ШрифтТекстаИсполнительАвтор();
	
	Элементы.Файлы.Шапка = Ложь;
	Элементы.ГруппаФайлыМК.ЦветФона = МобильныйКлиентВызовСервера.ЦветФонаГруппыСписка();
	Элементы.СоздатьФайл.Отображение = ОтображениеКнопки.Текст;
	Элементы.СоздатьФайл.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	
	Элементы.ДеревоСвязей.Шапка = Ложь;
	
	Элементы.ВывестиСписок.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Элементы.ОбновитьСписокФайлов.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Элементы.ПоказыватьУдаленныеФайлы.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	
	Элементы.ОткрытьФайл.Видимость = Ложь;
	Элементы.Редактировать.Видимость = Ложь;
	Элементы.ЗакончитьРедактирование.Видимость = Ложь;
	Элементы.Напечатать.Видимость = Ложь;
