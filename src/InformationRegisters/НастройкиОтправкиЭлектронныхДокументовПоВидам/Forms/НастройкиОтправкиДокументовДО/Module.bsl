...
Процедура ПодготовитьФормуНаСервере(ПараметрыФормы)
	...
	НастройкиОтправкиДО.Колонки.Добавить("ПредставлениеВидаДокумента");
	Для Каждого Строка Из НастройкиОтправкиДО Цикл
		Строка.ПредставлениеВидаДокумента = Строка(Строка.ВидДокумента);
	КонецЦикла;
	НастройкиОтправкиДО.Сортировать("ПредставлениеВидаДокумента");
	...
	Для Каждого СтрокаИсходящего Из ИсходящиеДокументыДО Цикл
		ЗаполнитьПредставлениеНастроекОтправкиВСтроке(СтрокаИсходящего, ФорматыЭД);
	КонецЦикла;
...
Процедура ОткрытьНастройкиОтправкиДО()
	...
	ПараметрыФормы.Вставить("ВидДокументаЭДО", ТекущиеДанные.ВидДокументаЭДО);
	...
	ОткрытьФорму("РегистрСведений.НастройкиОтправкиДокументовПоЭДО.Форма.НастройкаОтправкиДокументовДО",
		ПараметрыФормы, ЭтаФорма, , , , ОбработчикЗакрытия, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура ОбработатьЗакрытиеФормыНастроекОтправки(Результат, ДополнительныеПараметры) Экспорт
	...
	ЗаполнитьПредставлениеНастроекОтправкиВСтроке(СтрокаНастроек, ФорматыЭД);
...
Функция ЗаписатьНастройки()
	...
	ИсходящиеДокументыДокументооборот = ИсходящиеДокументыДО.Выгрузить();
	
	ИсходящиеДокументыДокументооборот.Колонки.Добавить("Отправитель");
	ИсходящиеДокументыДокументооборот.ЗаполнитьЗначения(Организация, "Отправитель");
	
	ИсходящиеДокументыДокументооборот.Колонки.Добавить("Получатель");
	ИсходящиеДокументыДокументооборот.ЗаполнитьЗначения(Контрагент, "Получатель");
	...
	Попытка
		...
		Запись.ИспользоватьУПД = Истина;
		Запись.ИспользоватьУКД = Истина;
		Запись.Записать();
		...
		НаборЗаписей.Отбор.Отправитель.Установить(Организация);
		НаборЗаписей.Отбор.Получатель.Установить(Контрагент);
		НаборЗаписей.Загрузить(ИсходящиеДокументыДокументооборот);
		НаборЗаписей.Записать();
...
&НаКлиентеНаСервереБезКонтекста
Процедура ЗаполнитьПредставлениеНастроекОтправкиВСтроке(СтрокаТаблицы, ФорматыЭД)
	
	СтрокиНастроек = Новый Массив;
	
	ПустойВидДокумента = ПредопределенноеЗначение("Справочник.ВидыДокументовЭДО.ПустаяСсылка");
	
	СтрокиНастроек.Добавить(?(СтрокаТаблицы.ВидДокументаЭДО = ПустойВидДокумента,
		НСтр("ru = 'Автоматически'"),
		СтрокаТаблицы.ВидДокументаЭДО));
	
	Если ЗначениеЗаполнено(СтрокаТаблицы.Формат) Тогда
		СтрокиФормата = ФорматыЭД.НайтиСтроки(Новый Структура("ИдентификаторФормата", СтрокаТаблицы.Формат));
		
		Если СтрокиФормата.Количество() > 0 Тогда
			СтрокиНастроек.Добавить(
				СтрШаблон(НСтр("ru = ' (%1)'"),СтрокиФормата[0].ПредставлениеФормата));
		КонецЕсли;
	КонецЕсли;
	
	СтрокиНастроек.Добавить(НСтр("ru = ', '"));
	
	СтрокиНастроек.Добавить(?(СтрокаТаблицы.ТребуетсяОтветнаяПодпись,
		НСтр("ru = 'с ответной подписью'"),
		НСтр("ru = 'без ответной подписи'")));
	
	СтрокаТаблицы.НастройкиОтправкиПредставление = СтрСоединить(СтрокиНастроек, "");
...
Процедура УстановитьТекущуюСтроку(Форма)
	...
	ТекущаяСтрока = Форма.Элементы.ИсходящиеДокументыДО.ТекущаяСтрока;
	Если Форма.ИсходящиеДокументыДО.Количество() > 0
		И (ТекущаяСтрока = Неопределено
			Или Форма.ИсходящиеДокументыДО.НайтиПоИдентификатору(ТекущаяСтрока) = Неопределено) Тогда
		
		Форма.Элементы.ИсходящиеДокументыДО.ТекущаяСтрока = Форма.ИсходящиеДокументыДО[0].ПолучитьИдентификатор();
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьПоПереходуНаНовуюВерсию()
	...
	Элементы.НадписьМиграцияНаНовуюВерсию.Заголовок =
		НСтр("ru = 'Обмен с контрагентами по ЭДО выполняется и настраивается в новой версии программы. Настройки отправки необходимо производить в новой версии программы.'");
