...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЗначениеЗаполнено(Пользователь) Тогда		
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Поле ""Кому передан"" не заполнено.'"),,
			"Пользователь",,Отказ);
	КонецЕсли;
			...
			Для Инд2 = Инд1 + 1 По КоличествоДокументов - 1 Цикл 			
				Если Документы[Инд1].Документ = Документы[Инд2].Документ  Тогда 
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Документ ""%1"" указан дважды в списке передаваемых документов'"),
	...
	Для Каждого Строка Из Документы Цикл
		...
		Если ТипЭкземпляра = Перечисления.ТипыЭкземпляров.Оригинал
		   ...
		   И КоличествоЭкземпляров > 0 Тогда
			...
			ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Указанный номер экземпляра ""%1"" превышает количество экземпляров документа ""%2""'"),
				НомерЭкземпляра,
				Строка.Документ);
			...
			Если Не НайденнаяСтрока.Возвращен Тогда
				...
				Если ТипЗнч(НайденнаяСтрока.Пользователь) = Тип("СправочникСсылка.КонтактныеЛица") Тогда
					...
					Если ДокументооборотПраваДоступа.ПолучитьПраваПоОбъекту(Пользователь).Чтение Тогда 
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Невозможно отразить передачу документа ""%1"", так как он не был возвращен контактным лицом %2'"),
							Строка.Документ,
							Строка(Пользователь));
					Иначе
						ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Невозможно отразить передачу документа ""%1"", так как он не был возвращен другим лицом'"),
							Строка.Документ);
					КонецЕсли;	
				Иначе
					ТекстСообщения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Невозможно отразить передачу документа ""%1"", так как он не был возвращен пользователем %2'"),
						Строка.Документ,
						Строка(Пользователь));
				КонецЕсли;	
	...
	Если Документы.Количество() = 0 Тогда
		ТекстСообщения = НСтр("ru = 'Нет документов к передаче'");
...
Процедура ПодборДокументов(Команда)
	...
	Меню.Добавить("ВнутренниеДокументы", НСтр("ru = 'Внутренний документ'"));
	Меню.Добавить("ВходящиеДокументы",   НСтр("ru = 'Входящий документ'"));
	Меню.Добавить("ИсходящиеДокументы",  НСтр("ru = 'Исходящий документ'"));
	
	ОписаниеОповещения = Новый ОписаниеОповещения(
		"ПодборДокументовПродолжение",
		ЭтотОбъект);

	Меню.ПоказатьВыборЭлемента(ОписаниеОповещения, НСтр("ru = 'Выбор типа данных'"));
...
Процедура ПодборДокументовПродолжение(Результат, Параметры) Экспорт 
	...
	Если Тип = "ВнутренниеДокументы" Тогда
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор внутренних документов'"));
	...
	ИначеЕсли Тип = "ВходящиеДокументы" Тогда
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор входящих документов'"));
	...
	ИначеЕсли Тип = "ИсходящиеДокументы" Тогда
		...
		ПараметрыОткрытия.Вставить("Заголовок", НСтр("ru = 'Подбор исходящих документов'"));
