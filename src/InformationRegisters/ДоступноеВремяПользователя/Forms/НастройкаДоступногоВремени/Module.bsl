...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДоступностьПользователя = РаботаСРабочимКалендаремСервер.ПолучитьНастройкуДоступностиПользователя(Пользователь);
	...
	Если ДоступностьПользователя <> Перечисления.ДоступностьПользователя.ДоступенПоРасписанию 
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьГрафикиРаботы") Тогда
			...
			Для Каждого ЭлементРабочееВремя Из ГрафикПользователя.РабочееВремя Цикл
				
				Для ДеньНедели = 1 По 5 Цикл
					...
					ЭлементНастройкиДоступногоВремени.ВремяНачала = ЭлементРабочееВремя.ВремяНачала;
					ЭлементНастройкиДоступногоВремени.ВремяОкончания = ЭлементРабочееВремя.ВремяОкончания;
				КонецЦикла;
...
Процедура ЗаписатьНастройкиДоступногоВремениИзРеквизитовФормы()
	...
	УстановитьНастройкуДоступностиПользователя =
		РаботаСРабочимКалендаремСервер.УстановитьНастройкуДоступностиПользователя(
			Пользователь,
...
Функция ДобавитьПромежутокВремениКВремениСтрокой(ВремяСтрокой, ВремяНачала, ВремяОкончания)
	...
	ДобавитьЗначениеКСтрокеЧерезРазделитель(ВремяСтрокой, ", ", ПромежутокВремениСтрокой);
