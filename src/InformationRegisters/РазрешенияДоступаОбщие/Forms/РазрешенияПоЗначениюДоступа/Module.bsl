...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Заголовок = НСтр("ru = 'Политики доступа'") + " (" + Параметры.ЗначениеДоступа + ")"; 
	...
	Для Сч = 0 По КолЗаписей-1 Цикл
		...
		Если УровеньДоступа = ПредопределенноеЗначение("Справочник.УровниДоступа.Регистрация") Тогда
			ДобавитьЭлемент = РазрешенаРегистрация;
		КонецЕсли;
...
Процедура ТаблицаРазрешенийПередУдалением(Элемент, Отказ)
	...
	Если Элемент.ТекущиеДанные.Унаследован Тогда
		ТекстПредупреждения =
			НСтр("ru = 'Это разрешение нельзя удалить, т.к. оно унаследовано от вышестоящего элемента.'");
