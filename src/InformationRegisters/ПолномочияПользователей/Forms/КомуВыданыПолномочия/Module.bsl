...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Полномочия") И ЗначениеЗаполнено(Параметры.Полномочия) Тогда 
		...
		Заголовок = СтрШаблон(НСтр("ru = 'Кому выданы полномочия ""%1""'"), Полномочия);
...
Процедура УдалитьЗаписьОПолномочиях()
	...
	ПоказатьВопрос(ОбработчикЗавершения, НСтр("ru = 'Полномочия будут удалены. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
