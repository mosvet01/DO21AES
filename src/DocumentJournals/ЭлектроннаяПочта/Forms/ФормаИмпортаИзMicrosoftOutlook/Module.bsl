...
Процедура Загрузить(Команда)
	...
	Состояние(НСтр("ru = 'Выполняется загрузка писем из Microsoft Outlook. Пожалуйста подождите...'"));
	...
	СтрокаСообщения = 
		НСтр("ru = 'Загрузка писем из Microsoft Outlook завершена. Загружено исходящих - %1, входящих - %2, пропущено - %3.'");
...
Функция ЗагрузитьПисьмаВПапку()
	...
	Если КоличествоСообщений = 0 Тогда
		Результат.СообщениеОбОшибке = НСтр("ru = 'Нет писем для загрузки'");
		Возврат Результат;
	КонецЕсли;
...
Процедура ОтобразитьСостояние(ПараметрыЗагрузки)
	...
	КонецЕсли;
	Состояние(
		НСтр("ru = 'Выполняется загрузка писем из Microsoft Outlook...'"),
		Прогресс,
		СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
