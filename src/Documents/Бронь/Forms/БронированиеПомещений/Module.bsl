...
Процедура УстановитьОтображаемуюДату(НоваяОтображаемаяДата)
	...
	ВидПоляОтображаемойДатыПолеВвода = Ложь;
	
	Если ЭтоМобильныйКлиент Тогда
		Если МобильныйКлиентВызовСервера.ЭтоМобильныйВПортретномРежиме() Или
			Не МобильныйКлиентВызовСервера.ЭтоМобильныйСБольшимЭкраном() Тогда 
				ВидПоляОтображаемойДатыПолеВвода = Истина;
		КонецЕсли;	
	КонецЕсли;
	
	ОтображаемаяДата = НоваяОтображаемаяДата;
	Если Не ВидПоляОтображаемойДатыПолеВвода Тогда
...
Процедура МК_НастроитьЭлементыФормы()
	...
	// Скрыть команды.
	Элементы.ГруппаКоманднаяПанельОсновная.Видимость = Ложь;
	Элементы.ФормаСправка.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	
	// Отборы.
	Элементы.МК_ГруппаОтборы.Видимость = Истина;
	
	// Отбор даты и времени.
	Элементы.ГруппаЗаголовокОтбораДатаИВремя.Видимость = Ложь;
	Элементы.Переместить(Элементы.ГруппаОтборДатаИВремя, Элементы.МК_ОтборДатаИВремя);
	Элементы.ГруппаВремя.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
	Элементы.ВремяНачала.Ширина = 4;
	Элементы.ВремяОкончания.Ширина = 4;
	
	// Отбор территории.
	Элементы.ГруппаЗаголовокОтбораТерритории.Видимость = Ложь;
	Элементы.Переместить(Элементы.Вместимость, Элементы.МК_ОтборТерритории);
	Элементы.Переместить(Элементы.ГруппаОтборТерритории, Элементы.МК_ОтборТерритории);
	
	// Планировщик.
	Элементы.СтраницаНеНайденыПомещения.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
...
Процедура МК_НастроитьФормуВПортретномРежиме()
	...
	Элементы.ОтображаемаяДата.Вид = ВидПоляФормы.ПолеВвода;
	Элементы.ОтображаемаяДата.Заголовок = НСтр("ru = 'Дата события:'");
	Элементы.ОтображаемаяДата.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Лево;
	Элементы.МК_ОтборДатаИВремя.Скрыть();
...
Процедура МК_НастроитьФормуВАльбомномРежиме()
	...
	Элементы.ОтображаемаяДата.Вид = ВидПоляФормы.ПолеКалендаря;
	Элементы.МК_ОтборДатаИВремя.Показать();
