...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ОтражениеВходящихЭДО" Тогда
		
		Если ТипЗнч(Параметр) = Тип("Структура")
			И Параметр.Свойство("ОбработанныеДокументы")
			И Параметр.ОбработанныеДокументы.Получить(Объект.Ссылка) <> Неопределено Тогда
...
&НаКлиенте
Процедура НадписьДокументДОНажатие(Элемент)
	
	Если ЗначениеЗаполнено(ДокументДО) Тогда
		ПоказатьЗначение(, ДокументДО);
	Иначе
		ОбменСКонтрагентамиДОСлужебныйКлиент.СоздатьДокументДОПоВходящемуЭДО(Объект.Ссылка);
	КонецЕсли;
...
&НаКлиенте
Процедура ОткрытьФормуПодбораОбъектовУчета(Результат, ДополнительныеПараметры = Неопределено) Экспорт
	...
	ИнтерфейсДокументовЭДОКлиент.ОткрытьПодборОбъектовУчетаЭлектронногоДокумента(Объект.Ссылка);
...
Процедура НастроитьВидимостьИНаименованиеДействийПоСостояниюЭДО()
	...
	Если Не ОбменСКонтрагентамиДОСлужебный.ДоступныКомандыПоДокументуЭДОИзИнтерфейсаБЭД(Объект.Ссылка) Тогда
		Элементы.СоздатьПроизвольныйДокумент.Видимость = Ложь;
		Элементы.Перенаправить.Видимость = Ложь;
		Элементы.ОбновитьСостояниеЭД.Видимость = Ложь;
		Элементы.Подписать.Видимость = Ложь;
		Элементы.ПодписатьДокумент.Видимость = Ложь;
		Элементы.Отправить.Видимость = Ложь;
		Элементы.ОтправитьДокумент.Видимость = Ложь;
		...
		Элементы.УтвердитьДокумент.Видимость = Ложь;
		Элементы.ПринятьАннулирование.Видимость = Ложь;
		Элементы.ПринятьАннулированиеДокумент.Видимость = Ложь;
		Элементы.ОтклонитьАннулирование.Видимость = Ложь;
		Элементы.ОтклонитьАннулированиеДокумент.Видимость = Ложь;
		Элементы.Отклонить.Видимость = Ложь;
		Элементы.ОтклонитьДокумент.Видимость = Ложь;
		Элементы.Аннулировать.Видимость = Ложь;
		Элементы.АннулироватьДокумент.Видимость = Ложь;
		Элементы.Завершить.Видимость = Ложь;
		Элементы.ЗавершитьДокумент.Видимость = Ложь;
		Элементы.ОтправитьВАрхив.Видимость = Ложь;
		Элементы.ОтправитьВАрхивДокумент.Видимость = Ложь;
		Элементы.ВернутьВРаботу.Видимость = Ложь;
		Элементы.ВернутьВРаботуДокумент.Видимость = Ложь;
		Элементы.ОтправитьПовторно.Видимость = Ложь;
		Элементы.ДействияСДокументом.Видимость = Ложь;
	КонецЕсли;
...
Функция АдресДанныхСертификата(НомерСтроки)
	...
	ДвоичныеДанныеСертификата = Подписи[НомерСтроки].Сертификат;
...
Процедура УстановитьВидимостьДоступностьПоПереходуНаНовуюВерсию()
	...
	Элементы.НадписьМиграцияНаНовуюВерсию.Заголовок =
		НСтр("ru = 'Обмен с контрагентами по ЭДО выполняется и настраивается в новой версии программы. Действия по документу необходимо производить в новой версии программы.'")
...
&НаСервере
Процедура НастроитьЭлементыОтраженияВДО()
	
	Элементы.ГруппаОтражениеВУчете.Видимость = Ложь;
	Элементы.ГруппаНевозможноОтразитьВУчете.Видимость = Ложь;
	
	ДокументыДО = ОбменСКонтрагентамиДОСлужебный.ДокументыДОЭлектронныхДокументов(
		ОбщегоНазначенияКлиентСервер.ЗначениеВМассиве(Объект.Ссылка));
	
	ДокументДО = ДокументыДО[Объект.Ссылка];
	
	Если ЗначениеЗаполнено(ДокументДО) Тогда
		Элементы.НадписьДокументДО.Заголовок = ДокументДО;
	Иначе
		Элементы.НадписьДокументДО.Заголовок = НСтр("ru = 'Отразить в 1С:Документооборот'");
	КонецЕсли;
