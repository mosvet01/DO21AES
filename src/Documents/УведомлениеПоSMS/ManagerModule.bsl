...
Процедура Добавить(Телефон, Знач Текст, Пользователь) Экспорт
	...
	Если ЗначениеЗаполнено(ОграничениеКоличестваВДеньSMS) 
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Достигнуто ограничение количества SMS в день для пользователя %1.'"),
			Пользователь);
	...
	ИначеЕсли ЗначениеЗаполнено(ОграничениеКоличестваВМесяцSMS) 
		...
		ТекстОшибки = СтрШаблон(
			НСтр("ru = 'Достигнуто ограничение количества SMS в месяц для пользователя %1.'"),
			Пользователь);
		ВызватьИсключение ТекстОшибки;
	ИначеЕсли ЗначениеЗаполнено(ОграничениеКоличестваВсегоВДеньSMS) 
		...
		ТекстОшибки = НСтр("ru = 'Достигнуто ограничение количества SMS в день.'");
		ВызватьИсключение ТекстОшибки;
	ИначеЕсли ЗначениеЗаполнено(ОграничениеКоличестваВсегоВМесяцSMS) 
		...
		ТекстОшибки = НСтр("ru = 'Достигнуто ограничение количества SMS в месяц.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Если ДлинаТекста > РазмерSMS Тогда
...
Процедура ОбновитьСтатусДоставки(УведомлениеSMS) Экспорт
	...
	Исключение
		ТекстОшибки = СтрШаблон(
				НСтр("ru = 'Ошибка при обновления состояния уведомления SMS: %1'",
...
Процедура ОтправитьSMS()
	...
	Пока Выборка.Следующий() Цикл
		...
		Исключение
			ТекстОшибки = СтрШаблон(
					НСтр("ru = 'Ошибка при передаче уведомления провайдеру SMS: %1'",
						ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
					ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(
				РаботаСУведомлениями.СобытиеЖурналаРегистрации(),
