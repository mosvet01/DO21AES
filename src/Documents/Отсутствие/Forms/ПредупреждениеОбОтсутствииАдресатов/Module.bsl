...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИначеЕсли КоличествоЗаместителей = 1 Тогда
		Элементы.ГруппаПереключатель.Видимость = Ложь;
		Элементы.ДобавитьЗаместителей.Заголовок = НСтр("ru = 'Добавить заместителя'");
		КлючНазначенияИспользования = "СЗаместителями";
	Иначе
		Элементы.ГруппаПереключатель.Видимость = Ложь;
		Элементы.ДобавитьЗаместителей.Заголовок = СтрШаблон(
			НСтр("ru = 'Добавить заместителей (%1)'"),
	...
	Если КоличествоОтсутствующих = 1 Тогда
		Элементы.УдалитьОтсутствующих.Заголовок = НСтр("ru = 'Удалить отсутствующего'");
		Элементы.ВариантОтправки.СписокВыбора[1].Представление = НСтр("ru = 'Удалить отсутствующего'");
	Иначе
		Элементы.УдалитьОтсутствующих.Заголовок = НСтр("ru = 'Удалить отсутствующих'");
		Элементы.ВариантОтправки.СписокВыбора[1].Представление = НСтр("ru = 'Удалить отсутствующих'");
	КонецЕсли;
...
Процедура ПоказатьОтсутствующихЗаместителей(Команда)
	...
	ПараметрыФормы.Вставить("ТекстПредупреждения", НСтр("ru = 'Некоторые заместители адресатов письма сейчас отсутствуют.'"));
...
Функция ВсеАдресатыОтсутсвуют(Адресаты)
	...
	Для Каждого Адресат Из Адресаты Цикл
		...
		Если Адресат.ТипАдреса <> НСтр("ru = 'Кому:'") Тогда
