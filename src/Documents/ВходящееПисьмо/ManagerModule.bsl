...
Функция СформироватьТекстПриглашения(ТекстHTML, Тема)
	...
	Если КогдаКонец = Неопределено Тогда
		...
		МассивПолей.Добавить(Новый Структура("Имя,Значение", НСтр("ru='Время'"), 
	...
	ИначеЕсли КогдаНачало <> Неопределено И КогдаКонец <> Неопределено Тогда
		...
		МассивПолей.Добавить(Новый Структура("Имя,Значение", НСтр("ru='Время'"), СтрокаДаты));
	...
	ТекстHTML = НСтр("ru='<h3>Приглашение</h3>'");
