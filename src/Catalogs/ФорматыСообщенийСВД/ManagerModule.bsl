	...
	Если ФорматСообщения = Справочники.ФорматыСообщенийСВД.СообщениеПоГОСТ53898ВложенныеФайлы
		...
		Исключение
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'СВД. Формирование сообщения на сервере СВД.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
	...
	Если Не РезультатЗапроса.Пустой() Тогда
		...
		Если ФорматСообщенияСсылка = Справочники.ФорматыСообщенийСВД.СообщениеПоГОСТ53898ВложенныеФайлы
			...
			Если СообщениеСВД.Файлы.Количество() = 1 или ИмяКорневогоЭлемента <> "Header"  Тогда
				...
				Если Не ПустаяСтрока(ИмяВременногоФайла) Тогда
					Попытка
						...
						Если ТипОбъектаXDTO = Неопределено Тогда
							...
							СообщениеОбОшибке = НСтр("ru = 'Схема сообщения не обеспечивает соответствие стандарту ГОСТ 53898'");          
							КодОшибки = 1000; 
						КонецЕсли;    
					...
					Исключение
						...
						СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Схема сообщения не обеспечивает соответствие стандарту  ГОСТ 53898 : ""%1""'"),
					...
					Исключение
						ЗаписьЖурналаРегистрации(
							НСтр("ru = 'СВД. Записать сообщения на сервере СВД.'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
							УровеньЖурналаРегистрации.Ошибка,
					...
					Если Не ОшибочноеСообщение Тогда    
						...
						Если Не ЗначениеЗаполнено(ПользовательКому) Тогда
							ОшибочноеСообщение = Истина;
							СообщениеОбОшибке = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
							НСтр("ru = 'Получатель ""%1"" не найден среди абонентов сервера СВД'"),
							ПользовательКомуСтр);    
				...
				Иначе
					ОшибочноеСообщение = Истина;
					СообщениеОбОшибке = НСтр("ru = 'В сообщении отсутствует зона сообщения ""Header""'");
...
Функция СформироватьСообщениеОбОшибке(ИдВходящегоСообщения, КодОшибки, ТекстОшибки, Кому, ПространствоИмен)
	...
	Сообщение.standart = НСтр("ru='Стандарт системы управления документами'");
	...
	Сообщение.from_organization = НСтр("ru = 'Сервер СВД 1С:Документооборот'"); 
