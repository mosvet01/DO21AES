...
Процедура РабочийКаталогВладельцаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		ИмяКаталога = ОбщегоНазначенияКлиентСервер.ДобавитьКонечныйРазделительПути(
			ИмяКаталога, ОбщегоНазначенияКлиент.ТипПлатформыКлиента());
		...
		Исключение
			...
			ТекстОшибки 
				= СтрШаблон(НСтр("ru = 'Неверный путь или отсутствуют права на запись в каталог ""%1""'"),
				ИмяКаталога);
...
Процедура РабочийКаталогВладельцаОчистка(Элемент, СтандартнаяОбработка)
	...
	РабочийКаталогУнаследованный = РабочийКаталогРодителя + Объект.Наименование 
		+ ФайловыеФункцииКлиентСервер.ПолучитьСлеш(ОбщегоНазначенияКлиент.ТипПлатформыКлиента());
...
Процедура ЗаполнитьИнформациюОСодержимом()
	...
	КоличествоВерсий = ?(ЧислоВерсий > 0, СтрШаблон(НСтр("ru='%1 (%2)'"), 
	...
	КоличествоФайлов = ?(ЧислоФайлов > 0, СтрШаблон(НСтр("ru='%1 (%2)'"), 
