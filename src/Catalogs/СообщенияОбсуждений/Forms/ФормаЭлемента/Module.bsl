...
Процедура УстановитьРежимЧтения()
	...
	Если УстановитьРежимЧтения Тогда
		...
		Если ЭтоМобильныйКлиент Тогда
			МобильныйКлиентКлиентСервер.АдаптироватьHtmlПодЭкранПриНеобходимости(ТекстСообщенияHTML);
		КонецЕсли;
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.ДобавитьФайл.Отображение = ОтображениеКнопки.Текст;
	Элементы.ДобавитьФайл.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;

	МК_НастроитьКнопку(Элементы.ДобавитьГолосование);
	МК_НастроитьКнопку(Элементы.ДобавитьРабочуюГруппу);
	МК_НастроитьКнопку(Элементы.ЗакрытьТему);
	МК_НастроитьКнопку(Элементы.ОткрытьТему);
	
	Элементы.ВставитьКартинкуИзБуфера.Видимость = Ложь;
	Элементы.ВставитьКартинкуИзБуфераОдин.Видимость = Ложь;
	...
	МК_НастроитьКнопку(Элементы.ОткрытьФайл);
	МК_НастроитьКнопку(Элементы.Редактировать);
	МК_НастроитьКнопку(Элементы.ЗакончитьРедактирование);
	...
	Элементы.Проголосовать.Отображение = ОтображениеКнопки.Текст;
	Элементы.Проголосовать.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	
	МК_НастроитьКнопку(Элементы.Прокомментировать);
	МК_НастроитьКнопку(Элементы.ОтменитьСвойГолос);
	МК_НастроитьКнопку(Элементы.ДобавитьВариантГолосования);
	МК_НастроитьКнопку(Элементы.УдалитьВариантГолосования);
	МК_НастроитьКнопку(Элементы.ИзменитьГолосование);
	МК_НастроитьКнопку(Элементы.УдалитьГолосование);
	
	// Страница "Рабочая группа".
	Элементы.РабочаяГруппаТаблица.Шапка = Ложь;
	Элементы.РабочаяГруппаТаблица.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;

КонецПроцедуры

&НаСервере
Процедура МК_НастроитьКнопку(Кнопка)
