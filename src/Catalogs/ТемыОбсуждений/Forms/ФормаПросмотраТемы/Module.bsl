...
Процедура СписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "Родитель" Тогда
		...
		Если ТекущиеДанные <> Неопределено Тогда
			Элементы.Список.ТекущаяСтрока = ТекущиеДанные.Родитель;
		КонецЕсли;
...
Процедура ОбработкаОжиданияПрочтения()
	...
	Если ТекущиеДанныеСообщения <> Неопределено Тогда
		...
		Если ТекущиеДанныеСообщения.Свойство("ТекстСообщения")
			...
			Если ЭтоМобильныйКлиент Тогда
				МобильныйКлиентКлиентСервер.АдаптироватьHtmlПодЭкранПриНеобходимости(ТекущееСообщениеТекст);
			КонецЕсли;
...
Процедура ОбновитьОтображениеОбластиЧтенияСервер(ОбновитьДанные = Истина)
	...
	ИначеЕсли ОтображениеОбластиЧтения =
		...
		Элементы.СтраницаПросмотрСФайламиИГолосованием.Группировка =
			ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;	
		Элементы.ГруппаСписок.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
...
Функция ПолучитьТекущиеДанные()
	...
	ИначеЕсли Не РежимОтображенияСообщенийДеревом
		И ЗначениеЗаполнено(Элементы.Список.ТекущаяСтрока)
		И ТипЗнч(Элементы.Список.ТекущаяСтрока) = Тип("СправочникСсылка.СообщенияОбсуждений") Тогда
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.СписокСтрокаСсылок.Видимость = Ложь;
	Элементы.ДеревоСтрокаСсылок.Видимость = Ложь;
	Элементы.ДатаСообщения.Видимость = Ложь;
	Элементы.МК_ДатаСообщения.Видимость = Истина;
	Элементы.Родитель.Видимость = Ложь;
	Элементы.Прочтен.Видимость = Ложь;
	
	// Уменьшим количество строк в сообщении.
	Элементы.ДеревоСообщенийТекстСообщенияСписок.Высота = 3;
	Элементы.ТекстСообщенияСписок.Высота = 3;
	
	Элементы.Автор.Ширина = 0;
	Элементы.Автор.РастягиватьПоГоризонтали = Истина;
	
	МК_ДобавитьСтраницыПросмотра();
	Элементы.ГруппаСписок.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
...
Процедура МК_НастроитьФормуВАльбомномРежиме()
	...
	Элементы.ДеревоСообщений.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	Элементы.Список.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	МобильныйКлиентКлиентСервер.АдаптироватьHtmlПодЭкранПриНеобходимости(ТекущееСообщениеТекст);
	Элементы.ВложенияСообщения.Шапка = Ложь;
...
&НаСервере
Процедура МК_ДобавитьСтраницыПросмотра()
	
	// Добавим страницы для просмотра.
	МК_СтраницыПросмотр = Элементы.Добавить(
		"МК_СтраницыПросмотр", Тип("ГруппаФормы"), Элементы.СтраницаПросмотр);
	МК_СтраницыПросмотр.Вид = ВидГруппыФормы.Страницы;
		
	МК_СтраницаПросмотрСообщение = Элементы.Добавить(
		"МК_СтраницаПросмотрСообщение", Тип("ГруппаФормы"), МК_СтраницыПросмотр);
	МК_СтраницаПросмотрСообщение.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаПросмотрСообщение.Заголовок = НСтр("ru = 'Сообщение'");
		
	Элементы.Переместить(Элементы.ТекущееСообщениеТекст, МК_СтраницаПросмотрСообщение);
	
	// Добавим страницы для просмотра с файлами.
	МК_СтраницыПросмотрСФайлами = Элементы.Добавить(
		"СтраницыПросмотрСФайламиМК", Тип("ГруппаФормы"), Элементы.СтраницаПросмотрСФайлами);
	МК_СтраницыПросмотрСФайлами.Вид = ВидГруппыФормы.Страницы;
	
	// Сообщение.
	МК_СтраницаСообщениеПросмотрСФайлами = Элементы.Добавить(
		"МК_СтраницаСообщениеПросмотрСФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСФайлами);
	МК_СтраницаСообщениеПросмотрСФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаСообщениеПросмотрСФайлами.Заголовок = НСтр("ru = 'Сообщение'");
		
	Элементы.Переместить(Элементы.ТекущееСообщениеТекстСФайлами, МК_СтраницаСообщениеПросмотрСФайлами);
	
	// Файлы.
	МК_СтраницаФайлыПросмотрСФайлами = Элементы.Добавить(
		"МК_СтраницаФайлыПросмотрСФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСФайлами);
	МК_СтраницаФайлыПросмотрСФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаФайлыПросмотрСФайлами.Заголовок = НСтр("ru = 'Файлы'");
		
	Элементы.Переместить(Элементы.ВложенияСообщения, МК_СтраницаФайлыПросмотрСФайлами);
	
	// Добавим страницы для просмотра с голосованием.
	МК_СтраницыПросмотрСГолосованием = Элементы.Добавить(
		"МК_СтраницыПросмотрСГолосованием", Тип("ГруппаФормы"), Элементы.СтраницаПросмотрСГолосованием);
	МК_СтраницыПросмотрСГолосованием.Вид = ВидГруппыФормы.Страницы;
	
	// Сообщение.
	МК_СтраницаСообщениеПросмотрСГолосованием = Элементы.Добавить(
		"МК_СтраницаСообщениеПросмотрСГолосованием", Тип("ГруппаФормы"), МК_СтраницыПросмотрСГолосованием);
	МК_СтраницаСообщениеПросмотрСГолосованием.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаСообщениеПросмотрСГолосованием.Заголовок = НСтр("ru = 'Сообщение'");
		
	Элементы.Переместить(Элементы.ТекущееСообщениеТекстСГолосованием, МК_СтраницаСообщениеПросмотрСГолосованием);
	
	// Голосование.
	МК_СтраницаГолосованиеПросмотрСФайлами = Элементы.Добавить(
		"МК_СтраницаГолосованиеПросмотрСФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСГолосованием);
	МК_СтраницаГолосованиеПросмотрСФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаГолосованиеПросмотрСФайлами.Заголовок = НСтр("ru = 'Голосование'");
		
	Элементы.Переместить(Элементы.ГруппаГолосование, МК_СтраницаГолосованиеПросмотрСФайлами);
	
	// Добавим страницы для просмотра с голосованием и файлами.
	МК_СтраницыПросмотрСГолосованиемИФайлами = Элементы.Добавить(
		"МК_СтраницыПросмотрСГолосованиемИФайлами", Тип("ГруппаФормы"), Элементы.СтраницаПросмотрСФайламиИГолосованием);
	МК_СтраницыПросмотрСГолосованиемИФайлами.Вид = ВидГруппыФормы.Страницы;
	
	// Сообщение.
	МК_СтраницаСообщениеПросмотрСГолосованиемИФайлами = Элементы.Добавить(
		"МК_СтраницаСообщениеПросмотрСГолосованиемИФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСГолосованиемИФайлами);
	МК_СтраницаСообщениеПросмотрСГолосованиемИФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаСообщениеПросмотрСГолосованиемИФайлами.Заголовок = НСтр("ru = 'Сообщение'");
		
	Элементы.Переместить(Элементы.ТекущееСообщениеТекстСФайламиИГолосованием, МК_СтраницаСообщениеПросмотрСГолосованиемИФайлами);
	
	// Голосование.
	МК_СтраницаГолосованиеПросмотрСГолосованиемИФайлами = Элементы.Добавить(
		"МК_СтраницаГолосованиеПросмотрСГолосованиемИФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСГолосованиемИФайлами);
	МК_СтраницаГолосованиеПросмотрСГолосованиемИФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаГолосованиеПросмотрСГолосованиемИФайлами.Заголовок = НСтр("ru = 'Голосование'");
		
	Элементы.Переместить(Элементы.ГруппаГолосованиеСФайлами, МК_СтраницаГолосованиеПросмотрСГолосованиемИФайлами);
	
	// Файлы.
	МК_СтраницаФайлыПросмотрСГолосованиемИФайлами = Элементы.Добавить(
		"МК_СтраницаФайлыПросмотрСГолосованиемИФайлами", Тип("ГруппаФормы"), МК_СтраницыПросмотрСГолосованиемИФайлами);
	МК_СтраницаФайлыПросмотрСГолосованиемИФайлами.Вид = ВидГруппыФормы.Страница;
	МК_СтраницаФайлыПросмотрСГолосованиемИФайлами.Заголовок = НСтр("ru = 'Файлы'");
	
	Элементы.Переместить(Элементы.ВложенияСообщенияСГолосованием, МК_СтраницаФайлыПросмотрСГолосованиемИФайлами);
