...
Процедура ЗаполнитьШаблонПоУмолчанию() Экспорт
	...
	Если Ссылка <> Справочники.ШаблоныИсполнения.ПоУмолчанию Тогда
	...
	ИсходныйШаблон = Справочники.ШаблоныИсполнения.ПустаяСсылка();
	КоличествоИтераций = 1;
	Комментарий = "";
	КомплексныйПроцесс = БизнесПроцессы.КомплексныйПроцесс.ПустаяСсылка();
	Контролер = Справочники.Пользователи.ПустаяСсылка();
	...
	Проверяющий = НСтр("ru = 'Автор процесса'");
	...
	ТрудозатратыПланПроверяющего = 0;
