...
Процедура НастроитьФормуДляШаблонаПоУмолчанию()
	...
	Если Объект.Ссылка <> Справочники.ШаблоныИсполнения.ПоУмолчанию Тогда
		Возврат;
	КонецЕсли;
	...
	Элементы.ГруппаОШаблонеПереход.Видимость = Ложь;
	Элементы.Важность.Видимость = Ложь;
	Элементы.ДобавлятьНаименованиеПредмета.Видимость = Ложь;
	Элементы.ИсполнителиИспользоватьУсловия.Видимость = Ложь;
...
Процедура ПоказатьНезаполненныеПоляНеобходимыеДляСтарта()
	...
	Для Каждого НезаполненноеПоле Из СписокНезаполненныхПолей Цикл
		...
		Если НезаполненноеПоле = "ОтветственныйИсполнитель" Тогда
			ТекстСообщения = НСтр("ru = 'Не указан ответственный исполнитель.'");
			ПутьКДанным = "Объект.Исполнители";
		ИначеЕсли НезаполненноеПоле = "ОбычныйИсполнитель" Тогда
			ТекстСообщения = НСтр("ru = 'Не указан исполнитель.'");
			ПутьКДанным = "Объект.Исполнители";
		Иначе
