...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ПолучитьФункциональнуюОпцию("ИспользоватьМЭДО") Тогда
		ЕстьМЭДО = ИнтеграцияСМЭДО.ЕстьДоступКРаботеСМЭДО();
	КонецЕсли;
	Элементы.СостояниеМЭДО.Видимость = ЕстьМЭДО;
	Элементы.ОтборСостояниеМЭДО.Видимость = ЕстьМЭДО;
	
	ВозможныеСостоянияДокументовМЭДО = Перечисления.СостоянияДокументовМЭДО.СостоянияВходящегоДокумента();
...
Процедура ПриЗагрузкеДанныхИзНастроекНаСервере(Настройки)
	...
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элементы.ОтборСостояниеМЭДО, СостояниеМЭДО);
...
Процедура ВидыДокументовПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоВидамДокументов")
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.МК_ГруппаВидыДокументов.Скрыть();
		КонецЕсли;
...
Процедура ВопросыДеятельностиПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоВопросамДеятельности")
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.МК_ГруппаВопросыДеятельности.Скрыть();
		КонецЕсли;
...
Процедура КонтрагентыПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоКонтрагентам")
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.МК_ГруппаКонтрагенты.Скрыть();
		КонецЕсли;
...
Процедура СписокНоменклатураДелПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоНоменклатуреДел")
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ЭтоМобильныйКлиент Тогда
				Элементы.МК_ГруппаНоменклатураДел.Скрыть();
			КонецЕсли;
...
Процедура ДелаТомаПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоДеламТомам")
		...
		Если ТекущиеДанные <> Неопределено Тогда
			...
			Если ЭтоМобильныйКлиент Тогда
				Элементы.МК_ГруппаДелаТома.Скрыть();
			КонецЕсли;
...
Процедура ПроектыПриАктивизацииСтроки(Элемент)
	...
	Если ВидПросмотра = ПредопределенноеЗначение("Перечисление.ВидыПросмотраСпискаОбъектов.ПоПроектам")
		...
		Если ЭтоМобильныйКлиент Тогда
			Элементы.МК_ГруппаПроекты.Скрыть();
		КонецЕсли;
...
Процедура ОтборСостояниеМЭДОПриИзменении(Элемент)
	...
	ОбщегоНазначенияДокументооборотКлиентСервер.ПоказатьСкрытьКнопкуОчисткиОтбора(Элемент, СостояниеМЭДО);
	
КонецПроцедуры

&НаКлиенте
Процедура ОтборСостояниеМЭДОНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	
	СтандартнаяОбработка = Ложь;
	ДанныеВыбора = ВозможныеСостоянияДокументовМЭДО;
...
Процедура ПереключитьВидПросмотра()
	...
	РаботаСоСпискамиДокументов.ПереключитьВидПросмотра(ЭтотОбъект);
...
Процедура МК_НастроитьЭлементыФормы()
	...
	// Команды формы.
	Элементы.СоздатьДокумент.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВКоманднойПанели;
	Элементы.СоздатьДокумент.Отображение = ОтображениеКнопки.Картинка;
	Элементы.СоздатьДокумент.Картинка = БиблиотекаКартинок.СоздатьЭлементСписка;
	...
	Для каждого Элемент из Элементы.ГруппаБыстрыеОтборы.ПодчиненныеЭлементы Цикл
		Если ТипЗнч(Элемент) = Тип("ПолеФормы") Тогда
			МК_ВыровнятьЭлементФормы(Элемент);
		КонецЕсли;
	КонецЦикла;
	
	// Вид просмотра.
	МК_НастроитьВидПросмотра();
	
	// Страницы к группам.
	// Страницы некоректно отображаются при добавлении туда всплывающей группы.
	Элементы.МК_ДеревоРазрезов.Видимость = Истина;
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаКатегории);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаВидыДокументов);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаВопросыДеятельности);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаКонтрагенты);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаНоменклатураДел);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаДелаТома);
	МК_НастроитьГруппуДляВидаПросмотра(Элементы.СтраницаПроекты);
	...
	Элементы.ГруппаИсходящийНомерИДата.Видимость = Ложь;
	...
	Элементы.ПоследнийЭтап.Видимость = Ложь;
	...
	Элементы.МК_ГруппаСписок.Видимость = Истина;
	Элементы.Переместить(Элементы.ГруппаБыстрыеОтборы, Элементы.МК_ГруппаСписок, Элементы.МК_ДеревоРазрезов);
	Элементы.Переместить(Элементы.Список, Элементы.МК_ГруппаСписок);
	
	Элементы.Переместить(Элементы.ГруппаФайлыЗадачи, Элементы.МК_Картинки);
	Элементы.Переместить(Элементы.ГруппаЭПКатегории, Элементы.МК_Картинки);
	Элементы.Переместить(Элементы.ГруппаСВДКонтроль, Элементы.ГруппаФайлыЗадачи);
	
	Элементы.Переместить(Элементы.Наименование, Элементы.МК_НаименованиеПодготовил, Элементы.МК_РегНомерИДата);
	Элементы.Переместить(Элементы.Отправитель, Элементы.МК_НаименованиеПодготовил);
	Элементы.Переместить(Элементы.РегистрационныйНомерИДата, Элементы.МК_РегНомерИДата);
	Элементы.Переместить(Элементы.ДатаРегистрации, Элементы.МК_РегНомерИДата);
	
	Элементы.Наименование.Высота = 2;
	Элементы.Отправитель.ЦветТекста = ЦветаСтиля.МК_ЦветАвтораИсполнителя;
	Элементы.Отправитель.Шрифт = ШрифтыСтиля.МК_ШрифтАвтораИсполнителя;
	Элементы.Отправитель.Высота = 1;
	Элементы.РегистрационныйНомерИДата.Высота = 1;
	
	// Проверка на миникарточку.
	ПриИзмененииПараметровЭкранаНаСервере();
...
&НаСервере
Процедура МК_НастроитьГруппуДляВидаПросмотра(Элемент)
	
	Элементы.Переместить(Элемент, Элементы.МК_ДеревоРазрезов);
...
&НаСервере
Процедура МК_НастроитьВидПросмотра()
	
	// По категории.
	Элементы.МК_ГруппаКатегории.Видимость = Истина;
	Элементы.Переместить(Элементы.ДеревоКатегорий, Элементы.МК_ГруппаКатегории);
	Элементы.Переместить(Элементы.ВыбранныеКатегории, Элементы.МК_ГруппаКатегории);
	Элементы.ДеревоКатегорий.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По виду документа.
	Элементы.МК_ГруппаВидыДокументов.Видимость = Истина;
	Элементы.Переместить(Элементы.ВидыДокументов, Элементы.МК_ГруппаВидыДокументов);
	Элементы.ВидыДокументов.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По вопросам деятельности.
	Элементы.МК_ГруппаВопросыДеятельности.Видимость = Истина;
	Элементы.Переместить(Элементы.ВопросыДеятельности, Элементы.МК_ГруппаВопросыДеятельности);
	Элементы.ВопросыДеятельности.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По контрагентам.
	Элементы.МК_ГруппаКонтрагенты.Видимость = Истина;
	Элементы.Переместить(Элементы.Контрагенты, Элементы.МК_ГруппаКонтрагенты);
	Элементы.Контрагенты.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По номенклатуре дел.
	Элементы.МК_ГруппаНоменклатураДел.Видимость = Истина;
	Элементы.Переместить(Элементы.СписокНоменклатураДел, Элементы.МК_ГруппаНоменклатураДел);
	Элементы.Переместить(Элементы.ГруппаОрганизацияГод, Элементы.МК_ГруппаНоменклатураДел);
	Элементы.СписокНоменклатураДел.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По делам томам.
	Элементы.МК_ГруппаДелаТома.Видимость = Истина;
	Элементы.Переместить(Элементы.ДелаТома, Элементы.МК_ГруппаДелаТома);
	Элементы.Переместить(Элементы.ГруппаОрганизацияГодДелТомов, Элементы.МК_ГруппаДелаТома);
	Элементы.ДелаТома.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
	
	// По проектам.
	Элементы.МК_ГруппаПроекты.Видимость = Истина;
	Элементы.Переместить(Элементы.ТолькоМоиПроекты, Элементы.МК_ГруппаПроекты);
	Элементы.Переместить(Элементы.Проекты, Элементы.МК_ГруппаПроекты);
	Элементы.Проекты.ИспользованиеТекущейСтроки = ИспользованиеТекущейСтрокиТаблицы.ОтображениеВыделенияИВыбор;
...
Процедура МК_НастроитьФормуВАльбомномРежиме()
	...
	// Страницы миникарточки.
	Элементы.СписокЗадачи.Шапка = Ложь;
	Элементы.ГруппаЗадачиСвязи.ЦветФона = ЦветаСтиля.МК_ЦветФонаГруппы;
	
	Элементы.МК_ГруппаФайлы.Видимость = Истина;
	Элементы.Переместить(Элементы.ФайлыДокумента, Элементы.МК_ГруппаФайлы);
	Элементы.ФайлыДокумента.Шапка = Ложь;
	
	Элементы.МК_ГруппаСвязи.Видимость = Истина;
	Элементы.Переместить(Элементы.ДеревоСвязей, Элементы.МК_ГруппаСвязи);
	Элементы.ДеревоСвязей.Шапка = Ложь;
