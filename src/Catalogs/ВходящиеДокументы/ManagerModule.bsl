...
Функция ПолучитьИмяПредметаПоУмолчанию(Ссылка) Экспорт
	...
	Если Не ЗначениеЗаполнено(ИмяПредмета) Тогда
		ИмяПредмета =  НСтр("ru='Входящий документ'")
	КонецЕсли;
...
Функция ИменаРеквизитовИзШаблона() Экспорт
	...
	Возврат "Адресат, ВидДокумента, Организация, ГрифДоступа, ВопросДеятельности, Ответственный, Отправитель,
	|НоменклатураДел, ВидОбращения, Подразделение, Проект, СпособПолучения";
