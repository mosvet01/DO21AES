...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ Объект.Ссылка.Пустая() Тогда
		...
		ЗаполнитьТекущийРазмер();
		
		МаксимальныйРазмерПриОткрытии = Объект.МаксимальныйРазмер;
	КонецЕсли;
...
&НаКлиенте
Процедура ОбновитьРазмер(Команда)
	ОбновитьРазмерНаСервере();
КонецПроцедуры

&НаСервере
Процедура ОбновитьРазмерНаСервере()
	
	ФайловыеФункции.ОбновитьРазмерТома(Объект.Ссылка);
	ЗаполнитьТекущийРазмер();
	
КонецПроцедуры

&НаСервере
Процедура ЗаполнитьТекущийРазмер()
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Если МаксимальныйРазмерПриОткрытии = 0
		И Объект.МаксимальныйРазмер <> 0 Тогда
