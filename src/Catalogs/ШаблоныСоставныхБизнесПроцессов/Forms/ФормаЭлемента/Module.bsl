...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	// Выполняем проверку заполнения вложенных шаблонов, только
	// если текущий не является шаблоном по умолчанию.
	ШаблоныПроцессовПоУмолчанию = ШаблоныБизнесПроцессовКлиентСервер.ШаблоныПроцессовПоУмолчанию();
	Если ШаблоныПроцессовПоУмолчанию.Найти(Объект.Ссылка) = Неопределено Тогда
...
Процедура НастроитьФормуДляШаблонаПоУмолчанию()
	...
	Если Объект.Ссылка <> Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаВнутреннегоДокументаПоУмолчанию
		И Объект.Ссылка <> Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаВходящегоДокументаПоУмолчанию
		И Объект.Ссылка <> Справочники.ШаблоныСоставныхБизнесПроцессов.ОбработкаИсходящегоДокументаПоУмолчанию Тогда
		Возврат;
	КонецЕсли;
	...
	Элементы.ТипШаблона.Видимость = Ложь;
...
Процедура ЗаполнитьОбъектПоУмолчанию() Экспорт
	...
	ШаблонОбъект = ДанныеФормыВЗначение(Объект, Тип("СправочникОбъект.ШаблоныСоставныхБизнесПроцессов"));
...
Процедура УстановитьУсловноеОформлениеИстекшихСроков()
	...
	СрокиИсполненияПроцессов.УстановитьУсловноеОформлениеИстекшегоСрока(
		ЭтаФорма,
		НСтр("ru = 'Срок исполнения действия истек (Шаблоны)'"),
...
Процедура ПоказатьНезаполненныеПоляНеобходимыеДляСтарта()
	...
	Если ОшибкиЗаполнения.Найти("Исполнители") <> Неопределено
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'В шаблонах вложенных процессов не заполнены обязательные поля.'"),,
			"Объект.Шаблоны");
	Иначе
