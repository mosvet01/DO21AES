...
Процедура ВосстановитьПоНачальномуЗаполнению(Команда)
	...
	Если Модифицированность ИЛИ ОбъектЗаписывался Тогда
		ПоказатьПредупреждение(,НСтр("ru = 'Для выполнения команды требуется переоткрыть форму.'"));
	Иначе
		ОписаниеОповещения = Новый ОписаниеОповещения(
		...
		ПоказатьВопрос(
			ОписаниеОповещения,
			НСтр("ru = 'Восстановить полномочия по содержимому начального заполнения?'"),
...
Процедура ВосстановитьПоНачальномуЗаполнениюПродолжение2(КодОтвета, Параметры) Экспорт
	...
	Если КодОтвета <> КодВозвратаДиалога.Отмена Тогда
		...
		Если ОбновитьГруппыДоступа Тогда
			Текст = НСтр("ru = 'Полномочия ""%1"" восстановлены по содержимому начального заполнения.'");
		Иначе
			Текст = НСтр("ru = 'Полномочия ""%1"" восстановлены по содержимому начального заполнения.'");
		КонецЕсли;
...
Процедура ОписаниеПоставляемогоПрофиля(Команда)
	...
	ТекстовыйДокумент.Показать(СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Описание поставляемых полномочий ""%1""'"), Объект.Наименование));
...
Процедура ПослеЗаписиКлиент(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ЕстьОшибки") 
		...
		ОписаниеОповещения = Новый ОписаниеОповещения(
			...
			ЭтотОбъект,
			ПараметрыЗаписи);
		ПоказатьПредупреждение(ОписаниеОповещения, НСтр("ru = 'При записи возникли ошибки (см. журнал регистрации)'"));
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьДеревоРолей()
	...
	ДеревоРолей = ПользователиСлужебныйПовтИсп.ДеревоРолей(ПоказатьПодсистемыРолей, Назначение).Получить().Скопировать();
