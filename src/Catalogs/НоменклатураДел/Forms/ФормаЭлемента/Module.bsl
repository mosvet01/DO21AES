...
Процедура ФормаДокументовПриИзменении(Элемент)
	...
	Если Объект.ФормаДокументов = ПредопределенноеЗначение("Перечисление.ВариантыФормДокументов.Электронная") Тогда 
		Если СтрНайти(Объект.Примечание, НСтр("ru = '(ЭД)'")) = 0 Тогда 
			Объект.Примечание = Объект.Примечание + НСтр("ru = '(ЭД)'");
		КонецЕсли;
	ИначеЕсли СтрНайти(Объект.Примечание, НСтр("ru = '(ЭД)'")) > 0 Тогда 
		Объект.Примечание = СтрЗаменить(Объект.Примечание, НСтр("ru = '(ЭД)'"), "");
	КонецЕсли;
