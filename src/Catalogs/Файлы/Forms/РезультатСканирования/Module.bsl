...
Процедура Повернуть180(Команда)
	ПовернутьКартинку(180);
КонецПроцедуры

&НаКлиенте
Процедура Повернуть90Влево(Команда)
	ПовернутьКартинку(-90);
КонецПроцедуры
...
#Если Не ВебКлиент Тогда	
	...
	КоличествоСтрок = ТаблицаФайлов.Количество();
	
	Если КоличествоСтрок > 1 Тогда
		НомерТекущейСтроки = Элементы.ТаблицаФайлов.ТекущаяСтрока;
		СтрокаТаблицы = Элементы.ТаблицаФайлов.ДанныеСтроки(НомерТекущейСтроки);
	Иначе            
		СтрокаТаблицы = ТаблицаФайлов[0];
	КонецЕсли;
	...
	ПовернутьКартинкуСервер(Угол, ДвоичныеДанные);
	
	ДвоичныеДанные.Записать(ПутьКВыбранномуФайлу);
	СтрокаТаблицы.АдресКартинки = ПоместитьВоВременноеХранилище(ДвоичныеДанные, УникальныйИдентификатор);
	АдресКартинки = СтрокаТаблицы.АдресКартинки;
