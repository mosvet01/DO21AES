	...
	Если ИзКарточкиФайла И ПустойТекстПриОткрытии Тогда
		...
		Если Не ЗаданВопросНеРаспознан Тогда
			...
			Если НЕ Распознан И СтатусРаспознаванияТекста = "НужноРаспознать" Тогда
				...
				ПоказатьВопрос(Обработчик, НСтр("ru = 'Внимание! Файл еще не распознан. Сделанные вами изменения будут потеряны. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
		...
		Если Не ЗаданВопросНеИзвлечен Тогда
			...
			Если НЕ ИзвлеченТекст И СтатусИзвлеченияТекста = "НеИзвлечен" Тогда
				...
				ПоказатьВопрос(Обработчик, НСтр("ru = 'Внимание! Текст файла еще не извлечен. Сделанные вами изменения будут потеряны. Продолжить?'"), РежимДиалогаВопрос.ДаНет);
