...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ДелопроизводствоКлиентСервер.ЭтоДокумент(ТекущийОбъект.ВладелецФайла) Тогда
		Если ЗначениеЗаполнено(ТекущийОбъект.ШаблонОснованиеДляСоздания)
			И Не РольДоступна("ПолныеПрава") Тогда 
		...
		Иначе
			...
			Если (ДелопроизводствоКлиентСервер.ЭтоВходящийДокумент(ТекущийОбъект.ВладелецФайла)
					Или ДелопроизводствоКлиентСервер.ЭтоИсходящийДокумент(ТекущийОбъект.ВладелецФайла))
				И ПолучитьФункциональнуюОпцию("ИспользоватьОтчетностьВКонтролирующиеОрганы") Тогда
				
				ДокументОтчетности =
					ОтчетностьВКонтролирующиеОрганы.ЭлектронныйДокументКОПоДокументу(ТекущийОбъект.ВладелецФайла);
				
				Если ДокументОтчетности <> Неопределено И ЗначениеЗаполнено(ДокументОтчетности) Тогда
...
Процедура ПродлитьДействиеПодписей(Команда)
	...
	Если ТекущиеДанные = Неопределено Тогда
		УИДПодписи = Неопределено;
	Иначе
		УИДПодписи = ТекущиеДанные.УникальныйИдентификатор;
	КонецЕсли;
	
	ОбработчикПродолжения = Новый ОписаниеОповещения("ОбработатьПродлениеПодписей", ЭтотОбъект);
	
	РаботаСЭПКлиент.ПродлитьДействиеПодписей(ЭтотОбъект, Объект.Ссылка, УИДПодписи, ОбработчикПродолжения);
