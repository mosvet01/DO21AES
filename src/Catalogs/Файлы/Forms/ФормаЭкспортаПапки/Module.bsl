...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	РасширениеДляЗашифрованныхФайлов = ЭлектроннаяПодпись.ПерсональныеНастройки().РасширениеДляЗашифрованныхФайлов;
КонецПроцедуры
...
Процедура ПриОткрытии(Отказ)
	...
	ПапкаДляЭкспорта = ФайловыеФункцииКлиент.НормализоватьКаталог(
		ПапкаДляЭкспорта);
		
	ПапкаДляЭкспортаПолная = ПапкаДляЭкспорта
	                       + Строка(ЧтоСохраняем);
		
	Если Прав(ПапкаДляЭкспортаПолная, 1) <> ПолучитьРазделительПути() Тогда
		ПапкаДляЭкспортаПолная = ПапкаДляЭкспортаПолная + ПолучитьРазделительПути();
	КонецЕсли;	
...
Процедура ОбойтиДеревоФайлов(ОбработчикРезультата, ТаблицаФайлов, БазовыйКаталогСохранения, РодительскаяПапка, ОбщиеПараметры)
	...
	//       * ФормаВопроса - ФормаКлиентскогоПриложения- Объект, который содержит ссылку на
...
Процедура ОбойтиДеревоФайловЗапускЦикла(ПараметрыВыполнения)
	...
	РаботаСФайламиКлиент.ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, ПараметрыВыполнения);
КонецПроцедуры
...
Процедура ОбойтиДеревоФайлов4(ПараметрыВыполнения)
	...
	Если ТекстОшибки <> "" Тогда
		РаботаСФайламиКлиент.ПодготовитьОбработчикДляДиалога(ПараметрыВыполнения);
	...
	Если ПараметрыВыполнения.ФайлНаДиске.Существует() Тогда
		...
		Иначе
			Если Не ПараметрыВыполнения.ОбщиеПараметры.ДляВсехФайлов Тогда
				...
				ТекстОкончание = СтрШаблон(
					НСтр("ru = '
					           |размер существующего файла = %1 байт, дата изменения %2.
					           |размер сохраняемого файла = %3 байт, дата изменения %4.
					           |
					           |Перезаписать или пропустить файл?'"),
				...
				ТекстСообщения = Новый ФорматированнаяСтрока(
					ТекстНачало,
					Новый ФорматированнаяСтрока(ПараметрыВыполнения.ИмяФайлаСРасширением, Новый Шрифт(,,Истина)),
					ТекстОкончание );	
				...
				ФормаВопроса = ПолучитьФорму("Справочник.Файлы.Форма.ФайлСуществует");//РаботаСФайламиКлиентПовтИсп.ПолучитьФормуЭкспортаПапкиФайлСуществует();
				...
				РаботаСФайламиКлиент.УстановитьОповещениеФормы(ФормаВопроса, Обработчик);
...
Процедура ОбойтиДеревоФайлов14(ПараметрыВыполнения)
	...
	СостояниеЗаголовок = СтрШаблон(
		НСтр("ru = 'Экспорт папки ""%1""'"),
		ПараметрыВыполнения.ЗаписьФайла.НаименованиеПапки);
	СостояниеТекст = СтрШаблон(
	...
	// Запишем файл заново
	ДополнительныеПараметры = Новый Структура("ЭкспортПапки, ПолныйПуть", Истина, ПараметрыВыполнения.ПолноеИмяФайла);
	АдресФайлаДляОткрытия = РаботаСФайламиВызовСервера.ПолучитьНавигационнуюСсылкуДляОткрытия(
...
Процедура ОбойтиДеревоФайлов16(Ответ, ПараметрыВыполнения) Экспорт
	Если Ответ = КодВозвратаДиалога.Прервать Тогда
		...
		ПараметрыВыполнения.Успех = Ложь;
		ПараметрыВыполнения.ТребуетсяЗапускЦикла = Ложь; // Цикл перезапускать не требуется.
		РаботаСФайламиКлиент.ВернутьРезультат(ПараметрыВыполнения.ОбработчикРезультата, ПараметрыВыполнения); // Ложь.
		Возврат;
	ИначеЕсли Ответ = КодВозвратаДиалога.Пропустить Тогда
