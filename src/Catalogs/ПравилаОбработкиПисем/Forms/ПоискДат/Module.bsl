...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не Параметры.Свойство("СписокЗначений")
		...
		ВызватьИсключение(НСтр("ru = 'Некорректные параметры'"));
...
Процедура Готово(Команда)
	...
	Если ЗначениеЗаполнено(ДатаДо) И ДатаДо < ДатаПосле Тогда
		Текст = НСтр("ru = 'Значение в поле ""Дата до"" должно быть больше значения в поле ""Дата после""'");
	...
	Если (ЗначениеЗаполнено(ДатаДо) Или ЗначениеЗаполнено(ДатаДо)) И ДатаДо = ДатаПосле Тогда
		Текст = НСтр("ru = 'Необходимо ввести разные значения в поля ""Дата до"" и ""Дата после""'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			Текст,
			,
			,
