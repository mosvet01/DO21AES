...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	ЧастотаПовторенияПолеВвода =
		Элементы.Добавить("ЧастотаПовторенияПолеВвода", Тип("ПолеФормы"), Элементы.ГруппаЧастотаПовторения);
	ЧастотаПовторенияПолеВвода.Вид = ВидПоляФормы.ПолеВвода;
	ЧастотаПовторенияПолеВвода.ПутьКДанным = "ЧастотаПовторения";
	ЧастотаПовторенияПолеВвода.УстановитьДействие("ПриИзменении", "ЧастотаПовторенияПриИзменении");
	ЧастотаПовторенияПолеВвода.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
	Элементы.ЧастотаПовторения.Видимость = Ложь;
	Для Каждого ЭлементВыбора Из Элементы.ЧастотаПовторения.СписокВыбора Цикл
		ЧастотаПовторенияПолеВвода.СписокВыбора.Добавить(ЭлементВыбора.Значение, ЭлементВыбора.Представление);
	КонецЦикла;
	
	Элементы.ДекорацияРазделительЧастотаПовторения.Видимость = Ложь;
	Элементы.ГруппаПравилоОкончанияПовторения.Группировка = ГруппировкаПодчиненныхЭлементовФормы.Вертикальная;
	Элементы.ПравилоОкончанияПовторения.Ширина = 0;
	Элементы.ПравилоОкончанияПовторения.РастягиватьПоГоризонтали = Истина;
