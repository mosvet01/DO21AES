...
Процедура НоменклатураДелНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ДелопроизводствоКлиент.НоменклатураДелНачалоВыбора(
		ЭтаФорма, Элемент, СтандартнаяОбработка, Объект.Отправитель);
...
Процедура ОтправительОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если УчитыватьКакОбращениеГраждан Тогда 
		...
		Если ДанныеОтправителя.КонтрагентЮрЛицо Тогда 
			ПоказатьПредупреждение(, НСтр("ru = 'Выбранный контрагент не является физическим лицом!'"));
			Объект.Отправитель = Неопределено;
		КонецЕсли;
	КонецЕсли;
...
Процедура ОбновитьВидимостьРеквизитов()
	...
	СписокДопРеквизитов = РаботаСШаблонамиДокументовСервер.ПолучитьНаборДопРеквизитовДокумента(
		"ВходящиеДокументы", Объект.ВидДокумента, Объект);
	...
	Если ПараметрыФункциональныхОпций.ВидВходящегоДокумента.ВестиУчетПоНоменклатуреДел 
		Или ПолучитьФункциональнуюОпцию("РаздельныйУчетДокументов") Тогда
		Элементы.ГруппаХранение.ОтображатьЗаголовок = Истина;
		Элементы.НоменклатураДел.Видимость = ПараметрыФункциональныхОпций.ВидВходящегоДокумента.ВестиУчетПоНоменклатуреДел;
	Иначе
	...
	Если ПараметрыФункциональныхОпций.ВидВходящегоДокумента.УчитыватьМестоХранения <> ВариантНеУчитывать Тогда
		Элементы.МестоХранения.Видимость = ПоказыватьПоляХранения;
	Иначе
		Элементы.МестоХранения.Видимость = Ложь;
	КонецЕсли;
	
	Если ПараметрыФункциональныхОпций.ВидВходящегоДокумента.УчитыватьОтветственногоЗаХранение <> ВариантНеУчитывать Тогда
