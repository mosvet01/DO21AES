...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт 
	...
	Если ТипЗнч(ОснованиеЗаполнения) = Тип("СправочникСсылка.ВходящиеДокументы") Тогда
		...
		Если Получатели.Количество() > 0 И Не ПолучательЗаполнен Тогда 
	...
	ИначеЕсли ТипЗнч(ОснованиеЗаполнения) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда
		...
		Если РеквизитыОснования.ВидДокументаВестиУчетПоКонтрагентам Тогда
			Для каждого Строка Из ОснованиеЗаполнения.Контрагенты Цикл
				...
				Если ЕстьПолучатели И Не ПолучательЗаполнен Тогда 
					НоваяСтрока = Получатели[0];
					ЕстьПолучатели = Ложь;
				Иначе 
					НоваяСтрока = Получатели.Добавить();
	...
	ИначеЕсли ОтчетностьВКонтролирующиеОрганыКлиентСервер.ЭтоДокументОбменаСКонтролирующимОрганом(
		...
		ЧистыйЗаголовок = СтрЗаменить(СведенияОДокументе.Наименование, Символы.ПС, " "); 
		Заголовок = СтрЗаменить(ЧистыйЗаголовок, Символы.ВК, "");
		...
		Иначе
			...
			Если Получатели.Количество() = 0 Тогда
				СтрокаПолучателя = Получатели.Добавить();
			Иначе
				СтрокаПолучателя = Получатели[0];
				Пока Получатели.Количество() > 1 Цикл
					Получатели.Удалить(1);
				КонецЦикла;
			КонецЕсли;
			
			СтрокаПолучателя.Получатель = СведенияОДокументе.Контрагент;
			Если Не ЗначениеЗаполнено(СтрокаПолучателя.СпособОтправки) Тогда
				СтрокаПолучателя.СпособОтправки = Справочники.СпособыДоставки.Сервис1СОтчетность;
			КонецЕсли;
