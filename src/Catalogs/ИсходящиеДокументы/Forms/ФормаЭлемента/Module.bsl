...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ИспользоватьСервисСПАРКРиски Тогда 
		...
		СПАРКРиски.ПриСозданииНаСервере(
			...
			Неопределено,
			Получатель,
			ОбщегоНазначенияДокументооборотКлиентСервер.ВидКонтрагентаСПАРК(Получатель),
			ПараметрыПроцедуры);
	Иначе 
	...
	КэшВидаДокумента.Вставить("ЯвляетсяВходящейКорреспонденцией", Ложь);
	КэшВидаДокумента.Вставить("ЯвляетсяИсходящейКорреспонденцией", Истина);
...
Процедура ОтправитьПоМЭДО(Команда)
	...
	МЭДОКлиент.НачатьОтправкуИсходящихДокументов(
		...
		МассивДокументов,
		ЭтотОбъект,
		Новый ОписаниеОповещения("ОтправитьПоМЭДОЗавершение", ЭтотОбъект));
	...
	Если ЗначениеЗаполнено(ПредметПереписки) И ТипЗнч(ПредметПереписки) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда 
		...
		Если СпособОтраженияПередачиКонтрагенту <> ПредопределенноеЗначение("Перечисление.СпособыОтраженияПередачиКонтрагенту.НеОтражать") Тогда 
			...
			Для Каждого Строка Из Объект.Получатели Цикл
				...
				ИндексПолучателя = Объект.Получатели.Индекс(Строка);
	
				Если Не ПараметрыЗаписи.Свойство("Передача" + Строка(ИндексПолучателя)) Тогда

					НоваяСтрока = СписокСтрокДляФиксацииПередачи.Добавить();
					НоваяСтрока.ИндексПолучателя = ИндексПолучателя;
					...
					Если СпособОтраженияПередачиКонтрагенту <> ПредопределенноеЗначение("Перечисление.СпособыОтраженияПередачиКонтрагенту.Автоматически") Тогда
						...
						ПараметрыОбработчика = Новый Структура;
						ПараметрыОбработчика.Вставить("ИндексПолучателя", Строка(
							НоваяСтрока.ИндексПолучателя));
						ПараметрыОбработчика.Вставить("НоваяСтрока", НоваяСтрока);
						ПараметрыОбработчика.Вставить("ПараметрыЗаписи", ПараметрыЗаписи);

						ОписаниеОповещения = Новый ОписаниеОповещения("ПередЗаписьюПослеОтветаОВнесенииЗаписиВЖурналПередачи",
							ЭтотОбъект, ПараметрыОбработчика);

						ТекстВопроса = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Внести запись в журнал передачи о передаче документа %1 контактному лицу %2?'"),
							Строка(ПредметПереписки), Строка(Строка.Адресат));
...
Процедура ПриИзмененииВидаДокумента()
	...
	РеквизитыВидДокумента = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Объект.ВидДокумента, 
		"ВестиУчетПоНоменклатуреДел, НеобходимаПечатьШтрихкода, ИспользоватьЭтапыОбработкиДокумента, ОбязательноУказаниеПроекта");
...
Функция ПолучитьВОтветНа(ВОтветНаСтрока, Отправитель)
	...
	Если ТипЗнч(Отправитель) = Тип("Массив") И Отправитель.Количество() > 0 Тогда
		Запрос.Текст = Запрос.Текст + "
		|	И ВходящиеДокументы.Отправитель В (&Отправитель)";
	ИначеЕсли ЗначениеЗаполнено(Отправитель) Тогда 
		Запрос.Текст = Запрос.Текст + "
...
Процедура УстановитьДоступностьПоОтчетностиВКО()
	...
	ЭлементыДляОграничения.Добавить(Элементы.Получатель);
...
Функция ПолучитьЗахваченныеФайлы()
	...
	ДелопроизводствоКлиент.ДобавитьЗанятыеФайлыСамойКарточки(ЭтотОбъект, МассивЗахваченныхФайлов);
...
&НаКлиенте
Процедура ОтправитьПоМЭДОЗавершение(Результат, ПараметрыОповещения) Экспорт
	...
	Иначе
		Для Каждого КЗ Из Результат Цикл
			Если Не КЗ.Значение.Успех Тогда
				ТекстОтвета = КЗ.Значение.СообщениеОбОшибке;
				Успех = Ложь;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
...
&НаСервере
Процедура ВидимостьЭлементовМЭДОВФорме()
...
Процедура МК_НастроитьЭлементыФормы()
	...
	Элементы.Переместить(Элементы.ВизыСогласованияТаблицаГруппа, Элементы.МК_ВизыГруппаРезультатИсполнитель);
