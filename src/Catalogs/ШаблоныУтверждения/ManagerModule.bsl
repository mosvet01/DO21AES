...
//     ПодписыватьЭП
//
Функция ПолучитьСтруктуруШаблонаУтверждения() Экспорт
...
//
Функция СоздатьШаблонУтверждения(СтруктураШаблона) Экспорт
...
Функция СинонимПроцесса(ИмяПроцесса, РеквизитыШаблона) Экспорт
	...
	Если РеквизитыШаблона.ВидПроцесса = Перечисления.ВидыБизнесПроцессаУтверждение.Подписание Тогда
		Возврат "Подписание";
	Иначе
		Возврат "Утверждение";
	КонецЕсли;
...
// Параметры:
//   HTMLТекст - Строка
//   Шаблон - СправочникСсылка.ШаблоныУтверждения - ссылка на шаблон
//
Процедура ЗаполнитьОбзорШаблона(HTMLТекст, Шаблон) Экспорт
	...
	Если ЗначениеЗаполнено(РеквизитыШаблона.СрокИсполнения) 
		...
		ОбзорОбъектовКлиентСервер.ДобавитьПодпись(HTMLТекст, НСтр("ru = 'Срок утверждения / подписания:'"));
...
//
// Параметры:
//  Шаблон - СправочникОбъект.ШаблоныУтверждения, СправочникСсылка.ШаблоныУтверждения - шаблон
//
// Возвращаемое значение:
//  ТаблицаЗначений
//   * Участник
...
Функция УчастникиДляПроверкиПрав(Шаблон) Экспорт
	...
	РаботаСБизнесПроцессами.ДобавитьУчастниковУтвержденияВТаблицу(ТаблицаУчастников, Шаблон);
...
//
// Параметры:
//  Процесс - СправочникСсылка.ШаблоныУтверждения - ссылка на шаблон
//
// Возвращаемое значение:
//  Структура
//
...
Функция ВыборкаДанныхДействий(ПараметрыДействий) Экспорт
	...
	Запрос.Текст = 
		...
		|	ШаблоныУтверждения.Наименование,
		|	ШаблоныУтверждения.Исполнитель,
		|	ШаблоныУтверждения.ВариантУстановкиСрокаИсполнения,
		|	ШаблоныУтверждения.СрокИсполнения,
		|	ШаблоныУтверждения.СрокИсполненияДни,
		|	ШаблоныУтверждения.СрокИсполненияЧасы,
		|	ШаблоныУтверждения.СрокИсполненияМинуты,
		|	ШаблоныУтверждения.Автор,
		|	ШаблоныУтверждения.ВариантУстановкиСрокаОбработкиРезультатов,
		|	ШаблоныУтверждения.СрокОбработкиРезультатов,
		|	ШаблоныУтверждения.СрокОбработкиРезультатовДни,
		|	ШаблоныУтверждения.СрокОбработкиРезультатовЧасы,
		|	ШаблоныУтверждения.СрокОбработкиРезультатовМинуты,
		|	ШаблоныУтверждения.КоличествоИтераций,
		|	ШаблоныУтверждения.СрокИсполненияПроцесса,
		|	ШаблоныУтверждения.ВидПроцесса
		|ИЗ
		|	ПараметрыДействий КАК ПараметрыДействий
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.ШаблоныУтверждения КАК ШаблоныУтверждения
		|		ПО ПараметрыДействий.ШаблонПроцесса = ШаблоныУтверждения.Ссылка";
...
//
// Параметры:
//  Объект - СправочникСсылка.ШаблоныУтверждения, ВыборкаДанных
//
// Возвращаемое значение:
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	...
	Если ВидФормы = "ФормаОбъекта" Тогда
		...
		ВыбраннаяФорма = "Справочник.ШаблоныУтверждения.Форма.ФормаЭлемента";
	...
	Если ВидФормы = "ФормаВыбора" Тогда
		...
		Параметры.Вставить("ТипШаблона", "Утверждение");
