...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ТипВладельцаФайла.СписокВыбора.Добавить(
		...
		НСтр("ru = 'Внутренний документ'"));
	Элементы.ТипВладельцаФайла.СписокВыбора.Добавить(
		Строка( Тип("СправочникСсылка.ВидыИсходящихДокументов")),
		НСтр("ru = 'Исходящий документ'"));
	...
	Если ЗначениеЗаполнено(Объект.ВладелецФайла) Тогда
		Если ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ВидыВнутреннихДокументов") Тогда
			ИмяТипаВладельцаФайла = НСтр("ru = 'Внутренний документ'");
		ИначеЕсли ТипЗнч(Объект.ВладелецФайла) = Тип("СправочникСсылка.ВидыИсходящихДокументов")  Тогда
			ИмяТипаВладельцаФайла = НСтр("ru = 'Исходящий документ'");
		КонецЕсли;
...
&НаКлиенте
Процедура ПередЗакрытиемПослеУдаления(ДополнительныеПараметры) Экспорт
КонецПроцедуры
	...
	#Если НЕ ВебКлиент И Не МобильныйКлиент Тогда
		Если НЕ ВыполнятьЗаполнениеФайловНаСервере Тогда
			...
			Если АвтозаполнениеШаблоновФайловКлиентСервер.ФорматMSWord(РасширениеШаблона) Тогда
				...
				Состояние(НСтр("ru = 'В шаблоне файла осуществляется поиск указанных полей и строк. Пожалуйста, подождите...'"));
				...
				Если НЕ ПустаяСтрока(ТекстОшибки) Тогда
					Отказ = Истина;
					Состояние(НСтр("ru = 'Поиск полей и строк завершен. Обнаружены ошибки.'"));
		...
		Если Стр <> Неопределено Тогда 
			Для Каждого НастройкаЗамены из ТекущийОбъект.ДанныеДляАвтозаполнения Цикл
				Если НЕ ПустаяСтрока(НастройкаЗамены.ТермДляЗамены) Тогда
					ТекстИсключения = НСтр("ru = 'Правила автозаполнения для текстовых файлов не могут содержать указания полей для замены. Допускается использование только строк для замены.'");
					ВызватьИсключение ТекстИсключения;
				ИначеЕсли ПустаяСтрока(НастройкаЗамены.ЗаменяемаяСтрока) Тогда
					ТекстИсключения = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
						НСтр("ru = 'Необходимо указать строку, которую заменяет значение ""%1"".'"),
	...
	ИначеЕсли РасширениеФайла = "odt" Тогда
		...
		Для Каждого НастройкаЗамены Из ТекущийОбъект.ДанныеДляАвтозаполнения Цикл
			Если ПустаяСтрока(НастройкаЗамены.ТермДляЗамены) И
				...
				ВызватьИсключение(НСтр("ru = 'Необходимо указать либо строку для замены, либо поле для замены'"));
			КонецЕсли;
			Если НЕ ПустаяСтрока(НастройкаЗамены.ТермДляЗамены) Тогда
...
Процедура ДанныеДляАвтозаполненияВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если ТипВладельцаФайла = Строка(Тип("СправочникСсылка.ВидыВнутреннихДокументов")) Тогда
		ВыбранныйТип = НСтр("ru = 'Внутренний документ'");
	Иначе
		ВыбранныйТип = НСтр("ru = 'Исходящий документ'");
	КонецЕсли;
...
Процедура ДанныеДляАвтозаполненияПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ШаблонФайла) Тогда
		ТекстСообщения = НСтр("ru = 'Поле ""Шаблон файла"" не заполнено'");
	...
	Если НЕ ЗначениеЗаполнено(ТипВладельцаФайла) Тогда
		ТекстСообщения = НСтр("ru = 'Поле ""Тип документа"" не заполнено'");
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВладелецФайла) 
		...
		ТекстСообщения = НСтр("ru = 'Поле ""Вид документа"" не заполнено'");
...
Процедура ДанныеДляАвтозаполненияПередУдалением(Элемент, Отказ)
	...
	ТекстВопроса = НСтр("ru = 'Удалить выбранные настройки автозаполнения (" + Элемент.ВыделенныеСтроки.Количество() + ")?'");
	Режим = Новый СписокЗначений;
	Режим.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Да, удалить'"));
	Режим.Добавить(КодВозвратаДиалога.Нет, НСтр("ru = 'Нет, не удалять'"));
...
Процедура ПроверитьПравилоПродолжениеПослеВопросаОЗаписи(Ответ, Параметры) Экспорт
	...
	ИначеЕсли Ответ = КодВозвратаДиалога.Нет Тогда
		Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Текст = НСтр("ru = 'Новое правило перед проверкой должно быть записано. Записать?'");
			ОписаниеОповещения = Новый ОписаниеОповещения(
...
Процедура ПроверитьПравилоПродолжениеПослеВыбораДокумента(Результат, Параметры) Экспорт
	...
	Если РезультатЗаполненияФайла.Свойство("ДвоичныеДанныеЗаполненногоФайла")
		...
		Если РезультатЗаполненияФайла.ДвоичныеДанныеЗаполненногоФайла = "Linux" Тогда
			ОписаниеОшибки = НСтр("ru = 'Автозаполнение файлов Microsoft Word 2003 возможно только в Windows. Обратитесь к администратору.'");
...
Процедура СформироватьЗаголовокФормы(Форма)
	...
	Если Объект.Ссылка.Пустая() Тогда 
		Если ЗначениеЗаполнено(Объект.ШаблонФайла) Тогда 
			Форма.Заголовок = СтрШаблон(НСтр("ru = 'Настройка заполнения файла ""%1"" (создание)'"),
				Объект.ШаблонФайла);
		Иначе 
			Форма.Заголовок = НСтр("ru = 'Настройка заполнения файла (создание)'");
		КонецЕсли;
	Иначе 
		Если ЗначениеЗаполнено(Объект.ШаблонФайла) Тогда 
			Форма.Заголовок = СтрШаблон(НСтр("ru = 'Настройка заполнения файла ""%1""'"),
				Объект.ШаблонФайла);
		Иначе 
			Форма.Заголовок = НСтр("ru = 'Настройка заполнения файла'");
		КонецЕсли;
	КонецЕсли;
