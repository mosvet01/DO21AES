...
// Заполняет схему шаблона процесса по умолчанию.
//
Процедура ЗаполнитьСхемуШаблонаПроцессаПоУмолчанию() Экспорт
	
	Если Ссылка <> Справочники.СхемыКомплексныхПроцессов.СхемаШаблонаПроцессаПоУмолчанию Тогда
		ВызватьИсключение НСтр("ru = 'Процедура ЗаполнитьСхемуШаблонаПроцессаПоУмолчанию предназначена для вызова из объекта предопределенной схемы СхемаШаблонаПроцессаПоУмолчанию.'");
	КонецЕсли;
	
	ВладелецСхемы = Справочники.ШаблоныКомплексныхБизнесПроцессов.ПоУмолчанию;
	КомплексныйПроцесс = БизнесПроцессы.КомплексныйПроцесс.ПустаяСсылка();
	
	ГрафическаяСхема = Справочники.СхемыКомплексныхПроцессов.ПолучитьМакет("СхемаПоУмолчанию");
	Схема = Новый ХранилищеЗначения(ГрафическаяСхема);
	
	РаботаСКомплекснымиБизнесПроцессамиКлиентСервер.ЗаполнитьСхемуПроцессаПоГрафическойСхеме(
		ЭтотОбъект, ГрафическаяСхема);
...
Процедура СтартоватьПроцессПоСхеме(ФормируемыеБизнесПроцессы, Процесс) Экспорт
	...
	Если НайденныеЭлементы.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'В процессе нет ни одного текущего элемента. Запуск процесса невозможен.'");
	КонецЕсли;
...
Функция СхемаКорректна() Экспорт
	...
	ИначеЕсли СтруктураДляПроверки.ЕстьЦиклыБезДействий Тогда
		...
		РезультатПроверки.НетОшибок = Ложь;
		РезультатПроверки.ОписаниеОшибки = 
			НСтр("ru = 'Есть циклы не имеющие ни одного действия/вложенного процесса.'");
...
Функция РезультатУсловия(ИмяУсловия, Процесс)
	...
	Исключение
		...
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'Проверка условия схемы процесса'", ОбщегоНазначенияКлиентСервер.КодОсновногоЯзыка()),
