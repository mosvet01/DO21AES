...
Функция ПеренестиНаСервере()
	...
	Если ПеренестиИзДела.ДелоЗакрыто Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Нельзя перенести документы из закрытого дела'"),,,"ПеренестиИзДела");
		Возврат Ложь;
	КонецЕсли;
	
	Если ПеренестиВДело.ДелоЗакрыто Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Нельзя перенести документы в закрытое дело'"),,,"ПеренестиВДело");
		Возврат Ложь;
	КонецЕсли; 
	...
	Если Список.НайтиСтроки(ПараметрыОтбора).Количество() = 0 Тогда 
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
			НСтр("ru = 'Не отмечены документы, которые должны быть перенесены'"),,,"Список");
		Возврат Ложь;
	КонецЕсли;
