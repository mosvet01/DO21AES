...
Процедура Создать(Команда)
	...
	ПоказатьПредупреждение(ОписаниеОповещение, НСтр("ru = 'Дела по выделенным элементам номенклатуры созданы.'"));
	...
	Пока Выборка.Следующий() Цикл
		...
		Если СтрокаРодитель <> Неопределено Тогда 
			...
			Если Выборка.Переходящее Тогда 
				НоваяСтрока.Подробности = НСтр("ru = 'Есть переходящее дело'");
			ИначеЕсли Выборка.ДелоУжеСоздано Тогда 
				НоваяСтрока.Подробности = НСтр("ru = 'Дело уже создано'");
			Иначе 
				НоваяСтрока.Подробности = НСтр("ru = 'Будет создано новое дело'");
				НоваяСтрока.Пометка = Истина;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
