...
Процедура ПриЧтенииНаСервере(ТекущийОбъект)
	...
	Если ПолучатьФотографии Тогда
		...
		ФотографияЗаданаПриОткрытии = ФотографияЗадана;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ПолучатьФотографии Или ПараметрыЗаписи.ЭтоНовыйОбъект Тогда
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	// Передадим фотографию в физ. лицо
	Если ФотографияЗаданаПриОткрытии <> ФотографияЗадана
		Или ПараметрыЗаписи.ЭтоНовыйОбъект Тогда
		Если ФотографияЗадана И ЭтоАдресВременногоХранилища(АдресФотографии) Тогда
			РаботаСФотографиями.ЗаписатьИзображение(АдресФотографии, 
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ФотографияЗаданаПриОткрытии <> ФотографияЗадана
		Или ПараметрыЗаписи.ЭтоНовыйОбъект
		И ФотографияЗадана И ЭтоАдресВременногоХранилища(АдресФотографии) Тогда
		...
		Для Каждого Строка Из ЭлементыДерева Цикл
			...
			Если Строка.ЭтоНесуществующаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					"Роли[%1].РолиСиноним",
					СтрШаблон(НСтр("ru = 'Несуществующая роль ""%1"".'"), Строка.Синоним),
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтрШаблон(НСтр("ru = 'Несуществующая роль ""%1"" в строке %2.'"), Строка.Синоним, "%1"));
			КонецЕсли;
			Если Строка.ЭтоНедоступнаяРоль Тогда
				ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,
					...
					"Роли",
					ЭлементыДерева.Индекс(Строка),
					СтрШаблон(НСтр("ru = 'Роль ""%1"" в строке %2 недоступна для пользователей.'"), Строка.Синоним, "%1"));
			КонецЕсли;
		КонецЦикла;
...
Процедура ФотографияНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Не ФотографияЗадана И Не ЕстьПравоРедактирования Тогда
	...
	ФайловыеФункцииКлиент.ВыбратьКартинкуИПоместитьВХранилище(
		ОповещениеОЗавершение, УникальныйИдентификатор,, Истина);
...
Процедура ФотографияНажатиеЗавершение(Результат, ДополнительныеПараметры) Экспорт
	...
	АдресФотографии = ДополнительныеПараметры.АдресВременногоХранилищаФайла;
	Модифицированность = Истина;
	ФотографияЗадана = Истина;
	// Обновим данные в физ. лице
	ФотографияЗаданаПриОткрытии = Не ФотографияЗадана;
		...
		Если УровеньДоступа.ИзменениеТекущего Тогда
			Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
				...
				Если СтрокаОписания = Неопределено Тогда
					...
					УточнениеСвойств.Вставить(0, СтрШаблон(НСтр("ru = 'Почта для восстановления пароля ""%1""'"),
						ПользовательИБАдресЭлектроннойПочты));
					КонецЕсли;
				КонецЕсли;
		КонецЕсли;
