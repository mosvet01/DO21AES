...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Для Каждого Строка Из НастройкиНумерации Цикл
		Если Не ЗначениеЗаполнено(Строка.ТипДокумента) Тогда 
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(
				НСтр("ru = 'Не указан ""Тип документа""'"),,"НастройкиНумерации["+Формат(НомерСтроки, "ЧН=; ЧГ=")+"].ТипДокумента",,Отказ);
		КонецЕсли;
	КонецЦикла;	
...
Процедура НастройкиНумерацииВидДокументаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(ТекущиеДанные.ТипДокумента) Или ТекущиеДанные.ВидДокумента = Неопределено Тогда 
		...
		Список.Добавить("СправочникСсылка.ВидыВходящихДокументов",   НСтр("ru = 'Вид входящего документа'"));
		Список.Добавить("СправочникСсылка.ВидыИсходящихДокументов",  НСтр("ru = 'Вид исходящего документа'"));
		Список.Добавить("СправочникСсылка.ВидыВнутреннихДокументов", НСтр("ru = 'Вид внутреннего документа'"));
