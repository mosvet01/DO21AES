...
Процедура ВыбратьПапку()
	...
	Иначе
		ПоказатьПредупреждение(, НСтр("ru = 'Не выбрана папка для переноса.'"));
		Возврат;
	КонецЕсли;
...
Процедура НастроитьЭлементыФормыДляМобильногоУстройства()
	...
	//Элементы.ПапкиКонтекстноеМенюДобавить.Видимость = Ложь;
	Элементы.КоманднаяПанельПеренос.Видимость = Ложь;
	Элементы.ВыберитеПапку.Видимость = Ложь;
	Элементы.ПапкиКонтекстноеМенюРежимМоиПапки.ПоложениеВКоманднойПанели = ПоложениеКнопкиВКоманднойПанели.ВДополнительномПодменю;
	Заголовок = НСтр("ru='Переместить в папку'");
