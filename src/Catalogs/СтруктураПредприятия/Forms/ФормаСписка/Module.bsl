...
Процедура СтруктураПредприятияПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если ПодразделениеПомеченоНаУдаление Тогда
		ШаблонДействия = НСтр("ru = 'Подразделение ""%1"" помечено на удаление. %2'");
	...
	Если КоличествоПользователей = 1 Тогда
		Если ДействиеИсключитьПользователя Тогда
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'Исключить пользователя ""%1"" из подразделения ""%2""?'"),
		...
		ИначеЕсли Не ПодразделениеПомеченоНаУдаление Тогда
			ТекстВопроса = СтрШаблон(
				НСтр("ru = '%1 пользователя ""%2"" в подразделение ""%3""?'"),
				ДействиеСПользователем,
				Строка(ПараметрыПеретаскивания.Значение[0]),
				Строка(Строка));
		Иначе
			ТекстВопроса = СтрШаблон(
				НСтр("ru = '%1 пользователя ""%2"" в это подразделение?'"),
				ДействиеСПользователем,
				Строка(ПараметрыПеретаскивания.Значение[0]));
		КонецЕсли;
	Иначе
		Если ДействиеИсключитьПользователя Тогда
			ТекстВопроса = СтрШаблон(
				НСтр("ru = 'Исключить пользователей (%1) из подразделения ""%2""?'"),
				КоличествоПользователей,
				Строка(Элементы.СтруктураПредприятия.ТекущаяСтрока));
			
		ИначеЕсли Не ПодразделениеПомеченоНаУдаление Тогда
			ТекстВопроса = СтрШаблон(
				НСтр("ru = '%1 пользователей (%2) в подразделение ""%3""?'"),
				ДействиеСПользователем,
				КоличествоПользователей,
				Строка(Строка));
		Иначе
			ТекстВопроса = СтрШаблон(
				НСтр("ru = '%1 пользователей (%2) в это подразделение?'"),
				ДействиеСПользователем,
				КоличествоПользователей);
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьОтборОформленияПодразделенияТекущегоПользователя()
	...
	Если Элементы.ПользователиСписок.ТекущаяСтрока <> АктивныйПользователь Тогда
		АктивныйПользователь = Элементы.ПользователиСписок.ТекущаяСтрока;
...
Процедура ПоказатьВопросыОбОткрытииКарточек(Вопросы) Экспорт
	...
	Ответы.Добавить("ОткрытьКарточку", НСтр("ru = 'Открыть карточку'"));
	Ответы.Добавить("НеОткрывать", НСтр("ru = 'Не открывать'"));
...
Процедура ПеренестиВПодразделениеСервер(ПереносимыеПользователи, Подразделение, ВопросыОбОткрытииКарточек)
	...
	Для Каждого ПереносимыйПользователь Из ПереносимыеПользователи Цикл
		РаботаСПользователями.ПеренестиПользователяВПодразделение(ПереносимыйПользователь, Подразделение);
	КонецЦикла;
