...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка) Экспорт 
	...
	Если ТипЗнч(ОснованиеЗаполнения) = Тип("СправочникСсылка.ВнутренниеДокументы") Тогда
		РеквизитыОснования = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ОснованиеЗаполнения, 
			...
			|Организация, Сумма, Валюта, СуммаНДС, Товары");
		
		ЯвляетсяЗаявкойНаОплату = Ложь; ВестиУчетТоваровИУслуг = Ложь;
		Если ЗначениеЗаполнено(ВидДокумента) Тогда
			РеквизитыВида = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВидДокумента, "ЯвляетсяЗаявкойНаОплату, ВестиУчетТоваровИУслуг");
		...
		Если Товары.Количество() = 0 И ВестиУчетТоваровИУслуг Тогда
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ЗначениеЗаполнено(ВидДокумента) Тогда 
		...
		Если РеквизитыВида.ВестиУчетСторон Тогда
			РаботаСПодписямиДокументов.ПроверитьЗаполнениеСторон(ЭтотОбъект, Отказ, РеквизитыВида.ЯвляетсяЗаявкойНаОплату);
...
Процедура ПриЗаписи(Отказ)
	...
	// Установнка начального состояния ЭДО, если документ может быть отправлен
	Если Не ДополнительныеСвойства.Свойство("НеЗаписыватьНачальноеСостояниеЭДО")
		Или ДополнительныеСвойства.НеЗаписыватьНачальноеСостояниеЭДО <> Истина Тогда
		
		ОбменСКонтрагентамиДОСлужебный.ЗаписатьНачальноеСостояниеЭДОДокументаДО(Ссылка);
	КонецЕсли;
